...
final class SnippetFormatter
{
    ...
    public function format(\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, string $snippetRegex, string $kind) : string
    {
        ...
        return (string) \_PhpScoperb26833cc184d\Nette\Utils\Strings::replace($fileInfo->getContents(), $snippetRegex, function ($match) use($kind) : string {
            if (\_PhpScoperb26833cc184d\Nette\Utils\Strings::contains($match['content'], '-----')) {
                // do nothing
    ...
    private function fixContent(string $content, string $kind) : string
    {
        ...
        if (!\_PhpScoperb26833cc184d\Nette\Utils\Strings::startsWith($this->isPhp73OrAbove ? \trim($content) : $content, '<?php')) {
        ...
        return \_PhpScoperb26833cc184d\Nette\Utils\Strings::replace($fileContent, self::OPENING_TAG_HERENOWDOC_REGEX, '$1');
    }
    /**
    ...
    {
        $content = \_PhpScoperb26833cc184d\Nette\Utils\Strings::replace($content, self::DECLARE_REGEX, '');
    ...
    {
        return \_PhpScoperb26833cc184d\Nette\Utils\Strings::replace($fileContent, self::OPENING_TAG_REGEX, '$1');
    }
}
