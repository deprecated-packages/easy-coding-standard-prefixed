 ...
 */
namespace _PhpScoper96382aaac118\SebastianBergmann\Diff\Utils;
...
trait UnifiedDiffAssertTrait
{
    ...
    public function assertValidUnifiedDiffFormat(string $diff) : void
    {
        ...
        for (; $lineNumber <= $lineCount; ++$lineNumber) {
            ...
            // line numbers start by 1, array index at 0
            ...
            if ('-' === $type) {
                ...
                $endOfLineTypes[$previousType] = \true;
                $diffClosed = \count($endOfLineTypes) > 1;
            } else {
                // internal state error
                throw new \RuntimeException(\sprintf('Unexpected line type "%s" Line %d.', $type, $lineNumber));
            }
            $expectHunkHeader = $diffLineFromNumber === $fromStart + $fromTillOffset && $diffLineToNumber === $toStart + $toTillOffset;
        }
        if ($diffLineFromNumber !== $fromStart + $fromTillOffset && $diffLineToNumber !== $toStart + $toTillOffset) {
    ...
    {
        $this->unifiedDiffAssertStrLength($line, 2, $message);
        // 2: line type indicator ('+', '-', ' ' or '\') and a line break
        $firstChar = $line[0];
    ...
    private function unifiedDiffAssertHeaderDate(string $date, string $message) : void
    {
        ...
        $match = \preg_match('/^([\\d]{4})-([01]?[\\d])-([0123]?[\\d])(:? [\\d]{1,2}:[\\d]{1,2}(?::[\\d]{1,2}(:?\\.[\\d]+)?)?(?: ([\\+\\-][\\d]{4}))?)?$/', $date, $matches);
    ...
    private function unifiedDiffAssertHunkHeader(string $line, string $message) : array
    {
        if (1 !== \preg_match('#^@@ -([\\d]+)((?:,[\\d]+)?) \\+([\\d]+)((?:,[\\d]+)?) @@\\n$#', $line, $matches)) {
            throw new \UnexpectedValueException(\sprintf('Hunk header line does not match expected pattern, got "%s". %s', $line, $message));
        }
        return [(int) $matches[1], empty($matches[2]) ? 1 : (int) \substr($matches[2], 1), (int) $matches[3], empty($matches[4]) ? 1 : (int) \substr($matches[4], 1)];
    }
}
