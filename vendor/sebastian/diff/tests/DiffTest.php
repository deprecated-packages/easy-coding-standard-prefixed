 ...
 */
final class DiffTest extends \_PhpScoperbaf90856897c\PHPUnit\Framework\TestCase
{
    public function testGettersAfterConstructionWithDefault() : void
    {
        $from = 'line1a';
        $to = 'line2a';
        $diff = new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Diff($from, $to);
    ...
    public function testGettersAfterConstructionWithChunks() : void
    {
        $from = 'line1b';
        $to = 'line2b';
        $chunks = [new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Chunk(), new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Chunk(2, 3)];
        $diff = new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Diff($from, $to, $chunks);
        $this->assertSame($from, $diff->getFrom());
        $this->assertSame($to, $diff->getTo());
    ...
    {
        $diff = new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Diff('line1c', 'line2c');
        $this->assertSame([], $diff->getChunks(), 'Expect chunks to be default value "array()".');
        $chunks = [new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Chunk(), new \_PhpScoperbaf90856897c\SebastianBergmann\Diff\Chunk(2, 3)];
