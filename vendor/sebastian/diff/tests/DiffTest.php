 ...
 */
final class DiffTest extends \_PhpScoperaac5f7c652e4\PHPUnit\Framework\TestCase
{
    public function testGettersAfterConstructionWithDefault() : void
    {
        $from = 'line1a';
        $to = 'line2a';
        $diff = new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Diff($from, $to);
    ...
    public function testGettersAfterConstructionWithChunks() : void
    {
        $from = 'line1b';
        $to = 'line2b';
        $chunks = [new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Chunk(), new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Chunk(2, 3)];
        $diff = new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Diff($from, $to, $chunks);
        $this->assertSame($from, $diff->getFrom());
        $this->assertSame($to, $diff->getTo());
    ...
    {
        $diff = new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Diff('line1c', 'line2c');
        $this->assertSame([], $diff->getChunks(), 'Expect chunks to be default value "array()".');
        $chunks = [new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Chunk(), new \_PhpScoperaac5f7c652e4\SebastianBergmann\Diff\Chunk(2, 3)];
