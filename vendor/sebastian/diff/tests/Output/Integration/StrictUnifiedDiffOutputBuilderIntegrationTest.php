...
use _PhpScoperb458b528613f\SebastianBergmann\Diff\Utils\FileUtils;
use _PhpScoperb458b528613f\SebastianBergmann\Diff\Utils\UnifiedDiffAssertTrait;
 ...
 * @requires OS Linux
 */
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends \_PhpScoperb458b528613f\PHPUnit\Framework\TestCase
{
    ...
    {
        $from = \_PhpScoperb458b528613f\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileFrom);
        $to = \_PhpScoperb458b528613f\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileTo);
        $diff = (new \_PhpScoperb458b528613f\SebastianBergmann\Diff\Differ(new \_PhpScoperb458b528613f\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilder(['fromFile' => 'Original', 'toFile' => 'New'])))->diff($from, $to);
        if ('' === $diff && $from === $to) {
    ...
    {
        return \_PhpScoperb458b528613f\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideOutputBuildingCases();
    }
    public function provideSample() : array
    {
        return \_PhpScoperb458b528613f\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideSample();
    }
    public function provideBasicDiffGeneration() : array
    {
        return \_PhpScoperb458b528613f\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideBasicDiffGeneration();
    }
    ...
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperb458b528613f\Symfony\Component\Process\Process(\sprintf('diff -u %s %s', \escapeshellarg($this->fileFrom), \escapeshellarg($this->fileTo)));
        $p->run();
    ...
    private function doIntegrationTestGitApply(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperb458b528613f\Symfony\Component\Process\Process(\sprintf('git --git-dir %s apply --check -v --unsafe-paths --ignore-whitespace %s', \escapeshellarg($this->dir), \escapeshellarg($this->filePatch)));
        $p->run();
    ...
    private function doIntegrationTestPatch(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperb458b528613f\Symfony\Component\Process\Process($command);
        $p->run();
        $this->assertProcessSuccessful($p);
        $this->assertStringEqualsFile($this->fileFrom, $to, \sprintf('Patch command "%s".', $command));
    }
    private function assertProcessSuccessful(\_PhpScoperb458b528613f\Symfony\Component\Process\Process $p) : void
    {
