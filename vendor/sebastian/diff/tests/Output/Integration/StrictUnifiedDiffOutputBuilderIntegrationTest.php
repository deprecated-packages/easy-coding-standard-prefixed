 ...
 */
namespace _PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Output;

use _PhpScoperb73f9e44f4eb\PHPUnit\Framework\TestCase;
use _PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Differ;
use _PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Utils\FileUtils;
use _PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Utils\UnifiedDiffAssertTrait;
 ...
 * @requires OS Linux
 */
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends \_PhpScoperb73f9e44f4eb\PHPUnit\Framework\TestCase
{
    ...
    {
        $from = \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileFrom);
        $to = \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileTo);
        $diff = (new \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Differ(new \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilder(['fromFile' => 'Original', 'toFile' => 'New'])))->diff($from, $to);
        if ('' === $diff && $from === $to) {
    ...
    {
        return \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideOutputBuildingCases();
    }
    public function provideSample() : array
    {
        return \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideSample();
    }
    public function provideBasicDiffGeneration() : array
    {
        return \_PhpScoperb73f9e44f4eb\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideBasicDiffGeneration();
    }
    ...
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperb73f9e44f4eb\Symfony\Component\Process\Process(\sprintf('diff -u %s %s', \escapeshellarg($this->fileFrom), \escapeshellarg($this->fileTo)));
        $p->run();
    ...
    private function doIntegrationTestGitApply(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperb73f9e44f4eb\Symfony\Component\Process\Process(\sprintf('git --git-dir %s apply --check -v --unsafe-paths --ignore-whitespace %s', \escapeshellarg($this->dir), \escapeshellarg($this->filePatch)));
        $p->run();
    ...
    private function doIntegrationTestPatch(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperb73f9e44f4eb\Symfony\Component\Process\Process($command);
        $p->run();
        $this->assertProcessSuccessful($p);
        $this->assertStringEqualsFile($this->fileFrom, $to, \sprintf('Patch command "%s".', $command));
    }
    private function assertProcessSuccessful(\_PhpScoperb73f9e44f4eb\Symfony\Component\Process\Process $p) : void
    {
