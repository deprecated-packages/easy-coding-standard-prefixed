 ...
 */
namespace _PhpScoper544eb478a6f6\SebastianBergmann\Diff\Output;

use _PhpScoper544eb478a6f6\PHPUnit\Framework\TestCase;
use _PhpScoper544eb478a6f6\SebastianBergmann\Diff\Differ;
use _PhpScoper544eb478a6f6\SebastianBergmann\Diff\Utils\FileUtils;
use _PhpScoper544eb478a6f6\SebastianBergmann\Diff\Utils\UnifiedDiffAssertTrait;
 ...
 * @requires OS Linux
 */
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends \_PhpScoper544eb478a6f6\PHPUnit\Framework\TestCase
{
    ...
    {
        $from = \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileFrom);
        $to = \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileTo);
        $diff = (new \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Differ(new \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilder(['fromFile' => 'Original', 'toFile' => 'New'])))->diff($from, $to);
        if ('' === $diff && $from === $to) {
    ...
    {
        return \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideOutputBuildingCases();
    }
    public function provideSample() : array
    {
        return \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideSample();
    }
    public function provideBasicDiffGeneration() : array
    {
        return \_PhpScoper544eb478a6f6\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideBasicDiffGeneration();
    }
    ...
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoper544eb478a6f6\Symfony\Component\Process\Process(\sprintf('diff -u %s %s', \escapeshellarg($this->fileFrom), \escapeshellarg($this->fileTo)));
        $p->run();
    ...
    private function doIntegrationTestGitApply(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoper544eb478a6f6\Symfony\Component\Process\Process(\sprintf('git --git-dir %s apply --check -v --unsafe-paths --ignore-whitespace %s', \escapeshellarg($this->dir), \escapeshellarg($this->filePatch)));
        $p->run();
    ...
    private function doIntegrationTestPatch(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoper544eb478a6f6\Symfony\Component\Process\Process($command);
        $p->run();
        $this->assertProcessSuccessful($p);
        $this->assertStringEqualsFile($this->fileFrom, $to, \sprintf('Patch command "%s".', $command));
    }
    private function assertProcessSuccessful(\_PhpScoper544eb478a6f6\Symfony\Component\Process\Process $p) : void
    {
