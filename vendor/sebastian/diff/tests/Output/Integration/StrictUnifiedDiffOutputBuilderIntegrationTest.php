 ...
 */
namespace _PhpScoperc233426b15e0\SebastianBergmann\Diff\Output;

use _PhpScoperc233426b15e0\PHPUnit\Framework\TestCase;
use _PhpScoperc233426b15e0\SebastianBergmann\Diff\Differ;
use _PhpScoperc233426b15e0\SebastianBergmann\Diff\Utils\FileUtils;
use _PhpScoperc233426b15e0\SebastianBergmann\Diff\Utils\UnifiedDiffAssertTrait;
 ...
 * @requires OS Linux
 */
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends \_PhpScoperc233426b15e0\PHPUnit\Framework\TestCase
{
    ...
    {
        $from = \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileFrom);
        $to = \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileTo);
        $diff = (new \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Differ(new \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilder(['fromFile' => 'Original', 'toFile' => 'New'])))->diff($from, $to);
        if ('' === $diff && $from === $to) {
    ...
    {
        return \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideOutputBuildingCases();
    }
    public function provideSample() : array
    {
        return \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideSample();
    }
    public function provideBasicDiffGeneration() : array
    {
        return \_PhpScoperc233426b15e0\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideBasicDiffGeneration();
    }
    ...
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperc233426b15e0\Symfony\Component\Process\Process(\sprintf('diff -u %s %s', \escapeshellarg($this->fileFrom), \escapeshellarg($this->fileTo)));
        $p->run();
    ...
    private function doIntegrationTestGitApply(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperc233426b15e0\Symfony\Component\Process\Process(\sprintf('git --git-dir %s apply --check -v --unsafe-paths --ignore-whitespace %s', \escapeshellarg($this->dir), \escapeshellarg($this->filePatch)));
        $p->run();
    ...
    private function doIntegrationTestPatch(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperc233426b15e0\Symfony\Component\Process\Process($command);
        $p->run();
        $this->assertProcessSuccessful($p);
        $this->assertStringEqualsFile($this->fileFrom, $to, \sprintf('Patch command "%s".', $command));
    }
    private function assertProcessSuccessful(\_PhpScoperc233426b15e0\Symfony\Component\Process\Process $p) : void
    {
