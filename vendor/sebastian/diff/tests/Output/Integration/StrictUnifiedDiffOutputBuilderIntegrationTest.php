 ...
 */
namespace _PhpScoperedc2e0c967db\SebastianBergmann\Diff\Output;

use _PhpScoperedc2e0c967db\PHPUnit\Framework\TestCase;
use _PhpScoperedc2e0c967db\SebastianBergmann\Diff\Differ;
use _PhpScoperedc2e0c967db\SebastianBergmann\Diff\Utils\FileUtils;
use _PhpScoperedc2e0c967db\SebastianBergmann\Diff\Utils\UnifiedDiffAssertTrait;
 ...
 * @requires OS Linux
 */
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends \_PhpScoperedc2e0c967db\PHPUnit\Framework\TestCase
{
    ...
    {
        $from = \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileFrom);
        $to = \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($fileTo);
        $diff = (new \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Differ(new \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilder(['fromFile' => 'Original', 'toFile' => 'New'])))->diff($from, $to);
        if ('' === $diff && $from === $to) {
    ...
    {
        return \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideOutputBuildingCases();
    }
    public function provideSample() : array
    {
        return \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideSample();
    }
    public function provideBasicDiffGeneration() : array
    {
        return \_PhpScoperedc2e0c967db\SebastianBergmann\Diff\Output\StrictUnifiedDiffOutputBuilderDataProvider::provideBasicDiffGeneration();
    }
    ...
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperedc2e0c967db\Symfony\Component\Process\Process(\sprintf('diff -u %s %s', \escapeshellarg($this->fileFrom), \escapeshellarg($this->fileTo)));
        $p->run();
    ...
    private function doIntegrationTestGitApply(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperedc2e0c967db\Symfony\Component\Process\Process(\sprintf('git --git-dir %s apply --check -v --unsafe-paths --ignore-whitespace %s', \escapeshellarg($this->dir), \escapeshellarg($this->filePatch)));
        $p->run();
    ...
    private function doIntegrationTestPatch(string $diff, string $from, string $to) : void
    {
        ...
        $p = new \_PhpScoperedc2e0c967db\Symfony\Component\Process\Process($command);
        $p->run();
        $this->assertProcessSuccessful($p);
        $this->assertStringEqualsFile($this->fileFrom, $to, \sprintf('Patch command "%s".', $command));
    }
    private function assertProcessSuccessful(\_PhpScoperedc2e0c967db\Symfony\Component\Process\Process $p) : void
    {
