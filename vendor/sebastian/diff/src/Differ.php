...
use function substr;
use ECSPrefix20210507\SebastianBergmann\Diff\Output\DiffOutputBuilderInterface;
...
final class Differ
{
    ...
    public function __construct($outputBuilder = null)
    {
        if ($outputBuilder instanceof DiffOutputBuilderInterface) {
            ...
            throw new \ECSPrefix20210507\SebastianBergmann\Diff\InvalidArgumentException(sprintf('Expected builder to be an instance of DiffOutputBuilderInterface, <null> or a string, got %s.', is_object($outputBuilder) ? 'instance of "' . get_class($outputBuilder) . '"' : gettype($outputBuilder) . ' "' . $outputBuilder . '"'));
        }
    }
    /**
    ...
    public function diffToArray($from, $to, $lcs = null)
    {
        if (is_string($from)) {
            ...
            throw new \ECSPrefix20210507\SebastianBergmann\Diff\InvalidArgumentException('"from" must be an array or string.');
        }
        if (is_string($to)) {
    ...
    private function selectLcsImplementation(array $from, array $to)
    {
        ...
        if ($this->calculateEstimatedFootprint($from, $to) > $memoryLimit) {
            return new \ECSPrefix20210507\SebastianBergmann\Diff\MemoryEfficientLongestCommonSubsequenceCalculator();
        }
        return new \ECSPrefix20210507\SebastianBergmann\Diff\TimeEfficientLongestCommonSubsequenceCalculator();
    }
    /**
