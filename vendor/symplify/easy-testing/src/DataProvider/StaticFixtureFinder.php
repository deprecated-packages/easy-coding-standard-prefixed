...
final class StaticFixtureFinder
{
    ...
    {
        $finder = \_PhpScoper5e5ebcdbebdf\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $iterator = \_PhpScoper5e5ebcdbebdf\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \_PhpScoper5e5ebcdbebdf\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
