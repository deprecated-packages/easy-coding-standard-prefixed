...
final class StaticFixtureSplitter
{
    ...
    {
        $splitLineCount = \count(\_PhpScoper7574e8786845\Nette\Utils\Strings::matchAll($smartFileInfo->getContents(), \Symplify\EasyTesting\ValueObject\SplitLine::SPLIT_LINE_REGEX));
        // if more or less, it could be a test cases for monorepo line in it
        if ($splitLineCount === 1) {
            // input → expected
            [$input, $expected] = \_PhpScoper7574e8786845\Nette\Utils\Strings::split($smartFileInfo->getContents(), \Symplify\EasyTesting\ValueObject\SplitLine::SPLIT_LINE_REGEX);
    ...
    {
        $hash = \_PhpScoper7574e8786845\Nette\Utils\Strings::substring(\md5($smartFileInfo->getRealPath()), -20);
