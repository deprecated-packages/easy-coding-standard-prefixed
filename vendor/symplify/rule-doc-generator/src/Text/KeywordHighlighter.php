...
final class KeywordHighlighter
{
    ...
    {
        $words = \_PhpScoperba24099fc6fd\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScoperba24099fc6fd\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScoperba24099fc6fd\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScoperba24099fc6fd\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScoperba24099fc6fd\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScoperba24099fc6fd\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScoperba24099fc6fd\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScoperba24099fc6fd\Nette\Utils\Strings::contains($word, '\\')) {
