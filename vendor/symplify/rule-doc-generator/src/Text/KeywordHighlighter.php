...
final class KeywordHighlighter
{
    ...
    {
        $words = \_PhpScoperd9fcac9e904f\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScoperd9fcac9e904f\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScoperd9fcac9e904f\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScoperd9fcac9e904f\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScoperd9fcac9e904f\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScoperd9fcac9e904f\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScoperd9fcac9e904f\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScoperd9fcac9e904f\Nette\Utils\Strings::contains($word, '\\')) {
