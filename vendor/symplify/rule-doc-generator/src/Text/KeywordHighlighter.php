...
use _PhpScopera4be459e5e3d\Rector\NodeTypeResolver\ClassExistenceStaticHelper;
use Throwable;
/**
...
final class KeywordHighlighter
{
    ...
    {
        $words = \_PhpScopera4be459e5e3d\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScopera4be459e5e3d\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScopera4be459e5e3d\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScopera4be459e5e3d\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScopera4be459e5e3d\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScopera4be459e5e3d\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScopera4be459e5e3d\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        if (\function_exists($word) || \function_exists(\trim($word, '()'))) {
            return \true;
        }
        if (\_PhpScopera4be459e5e3d\Rector\NodeTypeResolver\ClassExistenceStaticHelper::doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScopera4be459e5e3d\Nette\Utils\Strings::contains($word, '\\')) {
