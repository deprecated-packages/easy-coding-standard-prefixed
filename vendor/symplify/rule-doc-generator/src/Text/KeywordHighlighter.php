...
final class KeywordHighlighter
{
    ...
    {
        $words = \_PhpScoper3d66e802e797\Nette\Utils\Strings::split($content, '# #');
        foreach ($words as $key => $word) {
            if (!$this->isKeywordToHighlight($word)) {
                continue;
            }
            $words[$key] = \_PhpScoper3d66e802e797\Nette\Utils\Strings::replace('`' . $word . '`', self::COMMA_SPLIT_REGEX, function (array $match) : string {
    ...
    {
        if (\_PhpScoper3d66e802e797\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScoper3d66e802e797\Nette\Utils\Strings::startsWith($word, '`')) {
            return \false;
        }
        if (\_PhpScoper3d66e802e797\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScoper3d66e802e797\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScoper3d66e802e797\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScoper3d66e802e797\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScoper3d66e802e797\Nette\Utils\Strings::contains($word, '\\')) {
