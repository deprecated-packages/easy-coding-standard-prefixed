...
, [self::KEYWORD => 'CODE_SNIPPET'])]);
    ...
    {
        $match = \_PhpScoper28625551a513\Nette\Utils\Strings::match($token->getContent(), self::START_HEREDOC_NOWDOC_NAME_REGEX);
        if (!isset($match['name'])) {
            return;
        }
        $newContent = \_PhpScoper28625551a513\Nette\Utils\Strings::replace($token->getContent(), self::START_HEREDOC_NOWDOC_NAME_REGEX, '$1' . $this->keyword . '$4');
