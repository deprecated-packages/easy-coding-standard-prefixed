...
final class RemoveUselessClassCommentFixer extends \Symplify\CodingStandard\Fixer\AbstractSymplifyFixer implements \Symplify\RuleDocGenerator\Contract\DocumentedRuleInterface
{
    ...
    public function fix(\SplFileInfo $file, \PhpCsFixer\Tokenizer\Tokens $tokens) : void
    {
        ...
        foreach ($reverseTokens as $index => $token) {
            ...
            $cleanedDocContent = \_PhpScopera23ebff5477f\Nette\Utils\Strings::replace($originalDocContent, self::COMMENT_CLASS_REGEX, '');
            $cleanedDocContent = \_PhpScopera23ebff5477f\Nette\Utils\Strings::replace($cleanedDocContent, self::COMMENT_CONSTRUCTOR_CLASS_REGEX, '');
            $cleanedDocContent = \_PhpScopera23ebff5477f\Nette\Utils\Strings::replace($cleanedDocContent, self::COMMENT_METHOD_CLASS_REGEX, '');
...
CODE_SAMPLE
    ...
    {
        $matchMethodClass = \_PhpScopera23ebff5477f\Nette\Utils\Strings::match($cleanedDocContent, self::COMMENT_METHOD_CLASS_REGEX);
        if (!$matchMethodClass && $this->isNextFunction($reverseTokens, $index)) {
            $matchAnyMethodClass = \_PhpScopera23ebff5477f\Nette\Utils\Strings::match($cleanedDocContent, self::COMMENT_ANY_METHOD_CLASS_REGEX);
            if ($matchAnyMethodClass && \strtolower(\_PhpScopera23ebff5477f\Nette\Utils\Strings::replace($matchAnyMethodClass[3], self::SPACE_STAR_SLASH_REGEX, '')) === \strtolower($reverseTokens[$index + 6]->getContent())) {
                return \_PhpScopera23ebff5477f\Nette\Utils\Strings::replace($cleanedDocContent, self::COMMENT_ANY_METHOD_CLASS_REGEX, '');
            }
        }
