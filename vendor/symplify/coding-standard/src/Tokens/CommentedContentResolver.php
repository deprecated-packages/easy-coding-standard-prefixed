...
final class CommentedContentResolver
{
    ...
    public function resolve(\PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : \Symplify\CodingStandard\ValueObject\StartAndEnd
    {
        ...
        for ($i = $position; $i < $tokens->count(); ++$i) {
            ...
            $hasBlockCommentCloser = \_PhpScoper7faa8deb0d3c\Nette\Utils\Strings::endsWith($tokenContent, '*/');
    ...
    {
        if ($lastLineSeen + 1 <= $tokenLine && \_PhpScoper7faa8deb0d3c\Nette\Utils\Strings::startsWith($token->getContent(), '/*')) {
            // First non-whitespace token on a new line is start of a different style comment.
            return \true;
        }
        // next line is not a comment
        if ($lastLineSeen < $tokenLine && !\_PhpScoper7faa8deb0d3c\Nette\Utils\Strings::startsWith($token->getContent(), '//')) {
            return \true;
        }
