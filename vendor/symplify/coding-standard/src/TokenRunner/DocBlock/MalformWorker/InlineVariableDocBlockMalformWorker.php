...
final class InlineVariableDocBlockMalformWorker extends \Symplify\CodingStandard\TokenRunner\DocBlock\MalformWorker\AbstractMalformWorker
{
    ...
    public function work(string $docContent, \PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : string
    {
        ...
        // asterisk start
        $docContent = \_PhpScoperc753ccca5a0c\Nette\Utils\Strings::replace($docContent, self::SINGLE_ASTERISK_START_REGEX, '/**$1');
        // inline
        $docContent = \_PhpScoperc753ccca5a0c\Nette\Utils\Strings::replace($docContent, self::SPACE_REGEX, ' ');
        // remove asterisk leftover
        return \_PhpScoperc753ccca5a0c\Nette\Utils\Strings::replace($docContent, self::ASTERISK_LEFTOVERS_REGEX, '$1');
    }
