...
final class SuperfluousReturnNameMalformWorker extends \Symplify\CodingStandard\TokenRunner\DocBlock\MalformWorker\AbstractMalformWorker
{
    ...
    public function work(string $docContent, \PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : string
    {
        ...
        foreach ($lines as $line) {
            $match = \_PhpScopere106f9fd4493\Nette\Utils\Strings::match($line->getContent(), self::RETURN_VARIABLE_NAME_REGEX);
            if ($this->shouldSkip($match, $line->getContent())) {
                continue;
            }
            $newLineContent = \_PhpScopere106f9fd4493\Nette\Utils\Strings::replace($line->getContent(), self::RETURN_VARIABLE_NAME_REGEX, function (array $match) {
                $replacement = $match[1];
                if ($match['type'] !== []) {
    ...
    private function shouldSkip(?array $match, string $content) : bool
    {
        ...
        return \count(\_PhpScopere106f9fd4493\Nette\Utils\Strings::matchAll($content, self::VARIABLE_NAME_REGEX)) >= 2;
    }
}
