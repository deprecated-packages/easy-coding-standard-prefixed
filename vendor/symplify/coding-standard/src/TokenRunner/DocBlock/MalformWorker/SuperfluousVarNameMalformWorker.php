...
final class SuperfluousVarNameMalformWorker extends \Symplify\CodingStandard\TokenRunner\DocBlock\MalformWorker\AbstractMalformWorker
{
    ...
    public function work(string $docContent, \PhpCsFixer\Tokenizer\Tokens $tokens, int $position) : string
    {
        ...
        foreach ($lines as $line) {
            $match = \_PhpScoper833c56a97273\Nette\Utils\Strings::match($line->getContent(), self::VAR_VARIABLE_NAME_REGEX);
            if ($match === null) {
                continue;
            }
            $newLineContent = \_PhpScoper833c56a97273\Nette\Utils\Strings::replace($line->getContent(), self::VAR_VARIABLE_NAME_REGEX, function (array $match) : string {
                $replacement = $match[1];
                if ($match['type'] !== []) {
                    $replacement .= $match['type'];
                }
                if (\_PhpScoper833c56a97273\Nette\Utils\Strings::match($match[0], self::THIS_VARIABLE_REGEX)) {
                    return \_PhpScoper833c56a97273\Nette\Utils\Strings::replace($match[0], self::THIS_VARIABLE_REGEX, 'self');
                }
                return $replacement;
            });
