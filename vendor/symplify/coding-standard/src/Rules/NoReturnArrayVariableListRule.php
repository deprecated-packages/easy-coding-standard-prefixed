...
final class NoReturnArrayVariableListRule extends \Symplify\CodingStandard\Rules\AbstractSymplifyRule
{
    ...
    {
        return [\_PhpScoper3d04c8135695\PhpParser\Node\Stmt\Return_::class];
    }
    /**
     * @param Return_ $node
     * @return string[]
    ...
    private function shouldSkip(\_PhpScoper3d04c8135695\PHPStan\Analyser\Scope $scope, $node) : bool
    {
        // skip tests
        if (\_PhpScoper3d04c8135695\Nette\Utils\Strings::match($scope->getFile(), self::TESTS_DIRECTORY_REGEX) && !\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return \true;
        }
        // skip value objects
        if (\_PhpScoper3d04c8135695\Nette\Utils\Strings::match($scope->getFile(), self::VALUE_OBJECT_REGEX)) {
            return \true;
        }
        ...
        if ($functionLike instanceof \_PhpScoper3d04c8135695\PHPStan\Reflection\MethodReflection) {
            return $this->parentMethodAnalyser->hasParentClassMethodWithSameName($scope, $functionLike->getName());
        }
        return \false;
    }
    private function resolveExprCount(\_PhpScoper3d04c8135695\PhpParser\Node\Expr\Array_ $array) : int
    {
        $exprCount = 0;
        foreach ((array) $array->items as $item) {
            if (!$item instanceof \_PhpScoper3d04c8135695\PhpParser\Node\Expr\ArrayItem) {
                continue;
            }
            if (!$item->value instanceof \_PhpScoper3d04c8135695\PhpParser\Node\Expr) {
                continue;
            }
            if ($item->value instanceof \_PhpScoper3d04c8135695\PhpParser\Node\Expr\New_) {
                continue;
            }
            ++$exprCount;
        }
