...
use _PhpScoper8de082cbb8c7\PHPStan\Reflection\Native\NativeParameterReflection;
...
final class ForbiddenComplexArrayConfigInSetRule extends \Symplify\CodingStandard\Rules\AbstractSymplifyRule
{
    ...
    {
        return [\_PhpScoper8de082cbb8c7\PhpParser\Node\Expr\ArrayItem::class];
    }
    /**
    ...
    public function process(\_PhpScoper8de082cbb8c7\PhpParser\Node $node, \_PhpScoper8de082cbb8c7\PHPStan\Analyser\Scope $scope) : array
    {
        ...
        if (!$node->key instanceof \_PhpScoper8de082cbb8c7\PhpParser\Node\Expr\ClassConstFetch) {
            return [];
        }
        ...
        // simple → skip
        if (!$node->value instanceof \_PhpScoper8de082cbb8c7\PhpParser\Node\Expr\Array_) {
            return [];
        }
        $valueArray = $node->value;
        foreach ($valueArray->items as $nestedItem) {
            if (!$nestedItem instanceof \_PhpScoper8de082cbb8c7\PhpParser\Node\Expr\ArrayItem) {
                continue;
            }
            // way too complex
            if ($nestedItem->value instanceof \_PhpScoper8de082cbb8c7\PhpParser\Node\Expr\Array_) {
                return [self::ERROR_MESSAGE];
            }
        }
        return [];
    }
    private function isInSymfonyPhpConfigClosure(\_PhpScoper8de082cbb8c7\PHPStan\Analyser\Scope $scope) : bool
    {
        // we are in a closure
        ...
        $containerConfiguratorObjectType = new \_PhpScoper8de082cbb8c7\PHPStan\Type\ObjectType(\Symfony\Component\DependencyInjection\Loader\Configurator\ContainerConfigurator::class);
