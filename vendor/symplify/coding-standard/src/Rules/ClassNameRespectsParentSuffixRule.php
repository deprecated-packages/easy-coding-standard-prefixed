...
final class ClassNameRespectsParentSuffixRule extends \Symplify\CodingStandard\Rules\AbstractSymplifyRule
{
    ...
    {
        if (\_PhpScoper8de082cbb8c7\Nette\Utils\Strings::endsWith($parentType, 'Interface')) {
            $parentType = \_PhpScoper8de082cbb8c7\Nette\Utils\Strings::substring($parentType, 0, -\strlen('Interface'));
        }
        if (\_PhpScoper8de082cbb8c7\Nette\Utils\Strings::endsWith($parentType, 'Abstract')) {
            $parentType = \_PhpScoper8de082cbb8c7\Nette\Utils\Strings::substring($parentType, 0, -\strlen('Abstract'));
        }
        if (\_PhpScoper8de082cbb8c7\Nette\Utils\Strings::startsWith($parentType, 'Abstract')) {
            $parentType = \_PhpScoper8de082cbb8c7\Nette\Utils\Strings::substring($parentType, \strlen('Abstract'));
        }
        return $parentType;
    }
    /**
     * @return string[]
     */
    private function processParent(\_PhpScoper8de082cbb8c7\PhpParser\Node\Stmt\Class_ $class, \_PhpScoper8de082cbb8c7\PhpParser\Node\Name $parentClassName) : array
    {
    ...
    private function processClassNameAndShort(string $class, string $currentShortClass) : array
    {
        ...
        foreach ($parentClassesToCheck as $parentSuffix => $expectedSuffix) {
            ...
            if (!\_PhpScoper8de082cbb8c7\Nette\Utils\Strings::endsWith($currentShortClass, $parentSuffix)) {
                continue;
            }
            if (\_PhpScoper8de082cbb8c7\Nette\Utils\Strings::endsWith($class, $expectedSuffix)) {
                return [];
            }
