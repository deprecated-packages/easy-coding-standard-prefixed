...
final class RegexSuffixInRegexConstantRule extends \Symplify\CodingStandard\Rules\AbstractRegexRule
{
     ...
     * @return string[]
     */
    private function processConstantName(\_PhpScoper2b44cb0c30af\PhpParser\Node\Expr $expr) : array
    {
        if (!$expr instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Expr\ClassConstFetch) {
            return [];
        }
        if ($expr->name instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Expr) {
            return [];
        }
        $constantName = (string) $expr->name;
        if (\_PhpScoper2b44cb0c30af\Nette\Utils\Strings::endsWith($constantName, '_REGEX')) {
            return [];
        }
