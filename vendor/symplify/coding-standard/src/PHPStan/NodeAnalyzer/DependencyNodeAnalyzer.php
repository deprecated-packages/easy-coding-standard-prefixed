...
final class DependencyNodeAnalyzer
{
    ...
    public function __construct(\_PhpScoper2b44cb0c30af\PhpParser\NodeFinder $nodeFinder, \Symplify\CodingStandard\PHPStan\Naming\SimpleNameResolver $simpleNameResolver)
    {
        $this->nodeFinder = $nodeFinder;
        $this->simpleNameResolver = $simpleNameResolver;
    }
    public function isInsideAbstractClassAndPassedAsDependencyViaConstructorOrSetUp(\_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\Property $property) : bool
    {
        ...
        /** @var Assign[] $assigns */
        $assigns = $this->nodeFinder->findInstanceOf($classMethod, \_PhpScoper2b44cb0c30af\PhpParser\Node\Expr\Assign::class);
        if ($assigns === []) {
            return \false;
        }
        return $this->isBeeingAssignedInAssigns($property, $assigns);
    }
    public function isInsideClassAndPassedAsDependencyViaAutowireMethod(\_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\Property $property) : bool
    {
        $classLike = $this->resolveCurrentClass($property);
        if (!$classLike instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\Class_ && !$classLike instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\Trait_) {
            return \false;
        }
     ...
     */
    private function isBeeingAssignedInAssigns(\_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\Property $property, array $assigns) : bool
    {
        foreach ($assigns as $assign) {
            if (!$assign->var instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Expr\PropertyFetch) {
                continue;
            }
    ...
    private function resolveCurrentClass(\_PhpScoper2b44cb0c30af\PhpParser\Node $node) : ?\_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\ClassLike
    {
        $class = $node->getAttribute(\Symplify\CodingStandard\ValueObject\PHPStanAttributeKey::PARENT);
        while ($class) {
            if ($class instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\ClassLike) {
                return $class;
            }
    ...
    private function isPropertyFetchAndPropertyMatch(\_PhpScoper2b44cb0c30af\PhpParser\Node\Expr\PropertyFetch $propertyFetch, \_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\Property $property) : bool
    {
    ...
    private function isLocalPropertyFetch(\_PhpScoper2b44cb0c30af\PhpParser\Node\Expr\PropertyFetch $propertyFetch) : bool
    {
        if (!$propertyFetch->var instanceof \_PhpScoper2b44cb0c30af\PhpParser\Node\Expr\Variable) {
            return \false;
        }
        $propertyVariableName = $this->simpleNameResolver->getName($propertyFetch->var);
        return $propertyVariableName === 'this';
    }
    private function hasRequiredAnnotation(\_PhpScoper2b44cb0c30af\PhpParser\Node\Stmt\ClassMethod $classMethod) : bool
    {
        $docComment = $classMethod->getDocComment();
        if ($docComment === null) {
            return \false;
        }
        return (bool) \_PhpScoper2b44cb0c30af\Nette\Utils\Strings::match($docComment->getText(), self::REQUIRED_DOCBLOCK_REGEX);
    }
}
