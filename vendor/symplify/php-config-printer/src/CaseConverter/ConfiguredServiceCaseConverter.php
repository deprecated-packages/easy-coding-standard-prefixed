...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperf3db63c305b2\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoperf3db63c305b2\PhpParser\Node\Expr\MethodCall(new \_PhpScoperf3db63c305b2\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoperf3db63c305b2\Nette\Utils\Strings::startsWith($values, '@');
    }
}
