...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperd301db66c80c\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoperd301db66c80c\PhpParser\Node\Expr\MethodCall(new \_PhpScoperd301db66c80c\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoperd301db66c80c\Nette\Utils\Strings::startsWith($values, '@');
    }
}
