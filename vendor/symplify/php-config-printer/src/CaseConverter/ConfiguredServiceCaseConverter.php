...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperf3d5f0921050\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoperf3d5f0921050\PhpParser\Node\Expr\MethodCall(new \_PhpScoperf3d5f0921050\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoperf3d5f0921050\Nette\Utils\Strings::startsWith($values, '@');
    }
}
