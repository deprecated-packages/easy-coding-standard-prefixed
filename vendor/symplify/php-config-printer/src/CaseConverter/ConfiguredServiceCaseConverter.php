...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper04022cd986ec\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper04022cd986ec\PhpParser\Node\Expr\MethodCall(new \_PhpScoper04022cd986ec\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoper04022cd986ec\Nette\Utils\Strings::startsWith($values, '@');
    }
}
