...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper9f8d5dcff860\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper9f8d5dcff860\PhpParser\Node\Expr\MethodCall(new \_PhpScoper9f8d5dcff860\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoper9f8d5dcff860\Nette\Utils\Strings::startsWith($values, '@');
    }
}
