...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperaba240c3d5f1\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoperaba240c3d5f1\PhpParser\Node\Expr\MethodCall(new \_PhpScoperaba240c3d5f1\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoperaba240c3d5f1\Nette\Utils\Strings::startsWith($values, '@');
    }
}
