...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScopere341acab57d4\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScopere341acab57d4\PhpParser\Node\Expr\MethodCall(new \_PhpScopere341acab57d4\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScopere341acab57d4\Nette\Utils\Strings::startsWith($values, '@');
    }
}
