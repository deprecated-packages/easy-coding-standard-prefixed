...
final class ConfiguredServiceCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperbaf90856897c\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoperbaf90856897c\PhpParser\Node\Expr\MethodCall(new \_PhpScoperbaf90856897c\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    private function isAlias($values) : bool
    {
        ...
        return \is_string($values) && \_PhpScoperbaf90856897c\Nette\Utils\Strings::startsWith($values, '@');
    }
}
