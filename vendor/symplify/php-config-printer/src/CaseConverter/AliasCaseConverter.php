...
final class AliasCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Expression
    {
        ...
        $servicesVariable = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES);
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $methodCall = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \_PhpScoperb2e2c0c42e71\Nette\Utils\Strings::before($key, ' $');
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoperb2e2c0c42e71\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        return \is_string($values) && $values[0] === '@';
    }
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoperb2e2c0c42e71\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \_PhpScoperb2e2c0c42e71\Nette\Utils\Strings::after($key, '$');
        $concat = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Arg(new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\MethodCall(new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
