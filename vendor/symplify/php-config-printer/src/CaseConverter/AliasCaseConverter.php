...
final class AliasCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper4f42ead57614\PhpParser\Node\Stmt\Expression
    {
        ...
        $servicesVariable = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES);
        ...
        $fullClassName = \_PhpScoper4f42ead57614\Nette\Utils\Strings::before($key, ' $');
        ...
        if (\is_string($values) && $values[0] === '@') {
            ...
            $methodCall = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoper4f42ead57614\PhpParser\Node\Stmt\Expression($methodCall);
        }
        if (\is_array($values)) {
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoper4f42ead57614\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        if (!\is_string($values)) {
            return \false;
    ...
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoper4f42ead57614\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \_PhpScoper4f42ead57614\Nette\Utils\Strings::after($key, '$');
        $concat = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoper4f42ead57614\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoper4f42ead57614\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoper4f42ead57614\PhpParser\Node\Arg(new \_PhpScoper4f42ead57614\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\MethodCall(new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
    /**
     * @param mixed $values
     */
    private function createFromClassLike(string $key, $values, \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Variable $servicesVariable) : \_PhpScoper4f42ead57614\PhpParser\Node\Stmt\Expression
    {
    ...
    private function createFromAlias(string $className, string $key, \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Variable $servicesVariable) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr\MethodCall
    {
        ...
        return new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
    /**
     * @param mixed[] $values
     */
    private function createFromArrayValues(array $values, string $key, \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Variable $servicesVariable) : \_PhpScoper4f42ead57614\PhpParser\Node\Stmt\Expression
    {
