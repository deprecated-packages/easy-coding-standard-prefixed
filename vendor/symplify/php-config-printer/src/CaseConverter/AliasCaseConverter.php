...
final class AliasCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Stmt\Expression
    {
        ...
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $methodCall = new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \_PhpScoperdc8fbcd7c69d\Nette\Utils\Strings::before($key, ' $');
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoperdc8fbcd7c69d\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        return \is_string($values) && $values[0] === '@';
    }
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoperdc8fbcd7c69d\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \_PhpScoperdc8fbcd7c69d\Nette\Utils\Strings::after($key, '$');
        $concat = new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Arg(new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall(new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
