...
final class AliasCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperbd5c5a045153\PhpParser\Node\Stmt\Expression
    {
        ...
        $servicesVariable = new \_PhpScoperbd5c5a045153\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES);
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $methodCall = new \_PhpScoperbd5c5a045153\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoperbd5c5a045153\PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \_PhpScoperbd5c5a045153\Nette\Utils\Strings::before($key, ' $');
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoperbd5c5a045153\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        return \is_string($values) && $values[0] === '@';
    }
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoperbd5c5a045153\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoperbd5c5a045153\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \_PhpScoperbd5c5a045153\Nette\Utils\Strings::after($key, '$');
        $concat = new \_PhpScoperbd5c5a045153\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoperbd5c5a045153\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoperbd5c5a045153\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoperbd5c5a045153\PhpParser\Node\Arg(new \_PhpScoperbd5c5a045153\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoperbd5c5a045153\PhpParser\Node\Expr\MethodCall(new \_PhpScoperbd5c5a045153\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
