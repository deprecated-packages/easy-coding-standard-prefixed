...
final class AliasCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperba24099fc6fd\PhpParser\Node\Stmt\Expression
    {
        ...
        $servicesVariable = new \_PhpScoperba24099fc6fd\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES);
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $methodCall = new \_PhpScoperba24099fc6fd\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoperba24099fc6fd\PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \_PhpScoperba24099fc6fd\Nette\Utils\Strings::before($key, ' $');
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoperba24099fc6fd\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        return \is_string($values) && $values[0] === '@';
    }
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoperba24099fc6fd\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoperba24099fc6fd\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \_PhpScoperba24099fc6fd\Nette\Utils\Strings::after($key, '$');
        $concat = new \_PhpScoperba24099fc6fd\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoperba24099fc6fd\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoperba24099fc6fd\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoperba24099fc6fd\PhpParser\Node\Arg(new \_PhpScoperba24099fc6fd\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoperba24099fc6fd\PhpParser\Node\Expr\MethodCall(new \_PhpScoperba24099fc6fd\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
