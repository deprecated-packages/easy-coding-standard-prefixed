...
final class InstanceOfNestedCaseConverter implements \Symplify\PhpConfigPrinter\Contract\NestedCaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper9613f3fac51d\PhpParser\Node\Stmt\Expression
    {
        ...
        $args = [new \_PhpScoper9613f3fac51d\PhpParser\Node\Arg($classConstFetch)];
        $instanceofMethodCall = new \_PhpScoper9613f3fac51d\PhpParser\Node\Expr\MethodCall($servicesVariable, \Symplify\PhpConfigPrinter\ValueObject\MethodName::INSTANCEOF, $args);
        $instanceofMethodCall = $this->serviceOptionNodeFactory->convertServiceOptionsToNodes($values, $instanceofMethodCall);
        return new \_PhpScoper9613f3fac51d\PhpParser\Node\Stmt\Expression($instanceofMethodCall);
    }
