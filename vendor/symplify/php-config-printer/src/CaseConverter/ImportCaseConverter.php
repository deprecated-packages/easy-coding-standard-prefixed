...
use _PhpScoper326bba7310a2\PhpParser\Node\Expr\Variable;
use _PhpScoper326bba7310a2\PhpParser\Node\Scalar\String_;
use _PhpScoper326bba7310a2\PhpParser\Node\Stmt\Expression;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper326bba7310a2\PhpParser\Node\Stmt\Expression
    {
        if (\is_array($values)) {
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper326bba7310a2\PhpParser\Node\Stmt\Expression
    {
        $containerConfiguratorVariable = new \_PhpScoper326bba7310a2\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $args = $this->createArgs($arguments);
        $methodCall = new \_PhpScoper326bba7310a2\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper326bba7310a2\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper326bba7310a2\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper326bba7310a2\PhpParser\Node\Expr
    {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper326bba7310a2\PhpParser\Node\Scalar\String_('not_found');
        }
