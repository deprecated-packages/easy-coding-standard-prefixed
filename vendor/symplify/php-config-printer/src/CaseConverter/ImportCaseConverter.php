...
use _PhpScoper13160cf3462c\PhpParser\Node\Expr\Variable;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper13160cf3462c\PhpParser\Node\Stmt\Expression
    {
        $containerConfiguratorVariable = new \_PhpScoper13160cf3462c\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $args = $this->createArgs($arguments);
        $methodCall = new \_PhpScoper13160cf3462c\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper13160cf3462c\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper13160cf3462c\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper13160cf3462c\PhpParser\Node\Expr
    {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper13160cf3462c\PhpParser\Node\Scalar\String_('not_found');
        }
