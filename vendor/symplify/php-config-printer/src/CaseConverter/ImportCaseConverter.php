...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoperef5048aa2573\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoperef5048aa2573\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoperef5048aa2573\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoperef5048aa2573\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoperef5048aa2573\PhpParser\Node\Scalar\String_('not_found');
        }
