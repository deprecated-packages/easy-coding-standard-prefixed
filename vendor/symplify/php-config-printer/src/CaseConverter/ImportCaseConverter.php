...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper0ba97041430d\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper0ba97041430d\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper0ba97041430d\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper0ba97041430d\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper0ba97041430d\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper0ba97041430d\PhpParser\Node\Scalar\String_('not_found');
        }
