...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper842c7347e6be\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper842c7347e6be\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper842c7347e6be\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper842c7347e6be\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper842c7347e6be\PhpParser\Node\Scalar\String_('not_found');
        }
