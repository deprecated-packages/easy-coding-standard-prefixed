...
use _PhpScoper7c0f822a05e1\PhpParser\Node\Expr\Variable;
use _PhpScoper7c0f822a05e1\PhpParser\Node\Scalar\String_;
use _PhpScoper7c0f822a05e1\PhpParser\Node\Stmt\Expression;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper7c0f822a05e1\PhpParser\Node\Stmt\Expression
    {
        if (\is_array($values)) {
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper7c0f822a05e1\PhpParser\Node\Stmt\Expression
    {
        $containerConfiguratorVariable = new \_PhpScoper7c0f822a05e1\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $args = $this->createArgs($arguments);
        $methodCall = new \_PhpScoper7c0f822a05e1\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper7c0f822a05e1\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper7c0f822a05e1\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper7c0f822a05e1\PhpParser\Node\Expr
    {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper7c0f822a05e1\PhpParser\Node\Scalar\String_('not_found');
        }
