...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper4fc0030e9d22\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper4fc0030e9d22\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper4fc0030e9d22\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper4fc0030e9d22\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper4fc0030e9d22\PhpParser\Node\Scalar\String_('not_found');
        }
