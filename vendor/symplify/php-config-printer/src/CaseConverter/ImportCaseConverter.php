...
use _PhpScoper5f6e904600e7\PhpParser\Node\Scalar\String_;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper5f6e904600e7\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper5f6e904600e7\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper5f6e904600e7\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper5f6e904600e7\PhpParser\Node\Scalar\String_('not_found');
        }
