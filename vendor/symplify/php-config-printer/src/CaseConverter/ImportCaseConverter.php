...
use _PhpScoper4e2df00556a9\PhpParser\Node\Scalar\String_;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper4e2df00556a9\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper4e2df00556a9\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper4e2df00556a9\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper4e2df00556a9\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper4e2df00556a9\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper4e2df00556a9\PhpParser\Node\Scalar\String_('not_found');
        }
