...
use _PhpScoper776637f3d3c3\PhpParser\Node\Stmt\Expression;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper776637f3d3c3\PhpParser\Node\Stmt\Expression
    {
        if (\is_array($values)) {
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScoper776637f3d3c3\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScoper776637f3d3c3\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScoper776637f3d3c3\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScoper776637f3d3c3\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScoper776637f3d3c3\PhpParser\Node\Expr
    {
        ...
        if ($value === 'not_found') {
            return new \_PhpScoper776637f3d3c3\PhpParser\Node\Scalar\String_('not_found');
        }
