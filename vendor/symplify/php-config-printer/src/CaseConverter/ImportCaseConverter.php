...
use _PhpScopera1a51450b61d\PhpParser\Node\Expr\Variable;
use _PhpScopera1a51450b61d\PhpParser\Node\Scalar\String_;
use _PhpScopera1a51450b61d\PhpParser\Node\Stmt\Expression;
...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \_PhpScopera1a51450b61d\PhpParser\Node\Stmt\Expression
    {
        if (\is_array($values)) {
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScopera1a51450b61d\PhpParser\Node\Stmt\Expression
    {
        $containerConfiguratorVariable = new \_PhpScopera1a51450b61d\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $args = $this->createArgs($arguments);
        $methodCall = new \_PhpScopera1a51450b61d\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScopera1a51450b61d\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScopera1a51450b61d\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScopera1a51450b61d\PhpParser\Node\Expr
    {
        ...
        if ($value === 'not_found') {
            return new \_PhpScopera1a51450b61d\PhpParser\Node\Scalar\String_('not_found');
        }
