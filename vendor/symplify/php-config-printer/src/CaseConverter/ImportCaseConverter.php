...
final class ImportCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
     ...
     */
    private function createImportMethodCall(array $arguments) : \_PhpScopere050faf861e6\PhpParser\Node\Stmt\Expression
    {
        ...
        $methodCall = new \_PhpScopere050faf861e6\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, 'import', $args);
        return new \_PhpScopere050faf861e6\PhpParser\Node\Stmt\Expression($methodCall);
    }
    /**
     * @param mixed[] $arguments
     * @return Arg[]
    ...
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \_PhpScopere050faf861e6\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
    private function resolveExpr($value) : \_PhpScopere050faf861e6\PhpParser\Node\Expr
    {
        if (\is_bool($value)) {
        ...
        if ($value === 'not_found') {
            return new \_PhpScopere050faf861e6\PhpParser\Node\Scalar\String_('not_found');
        }
