...
final class ServicesPhpNodeFactory
{
    ...
    public function createResource(string $serviceKey, array $serviceValues) : \_PhpScopera37d6fb0b1ab\PhpParser\Node\Stmt\Expression
    {
        ...
        if (!isset($serviceValues[self::EXCLUDE])) {
            return new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Stmt\Expression($servicesLoadMethodCall);
        }
        ...
        $excludeMethodCall = new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Expr\MethodCall($servicesLoadMethodCall, self::EXCLUDE, $args);
        return new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Stmt\Expression($excludeMethodCall);
    }
    private function createServicesLoadMethodCall(string $serviceKey, $serviceValues) : \_PhpScopera37d6fb0b1ab\PhpParser\Node\Expr\MethodCall
    {
        ...
        $args = [];
        $args[] = new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Arg(new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Scalar\String_($serviceKey));
        $args[] = new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Arg($this->commonNodeFactory->createAbsoluteDirExpr($resource));
        return new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Expr\MethodCall($servicesVariable, 'load', $args);
    }
}
