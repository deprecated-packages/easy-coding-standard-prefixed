...
final class ServicesPhpNodeFactory
{
    ...
    public function createResource(string $serviceKey, array $serviceValues) : \_PhpScoperac4e86be08e5\PhpParser\Node\Stmt\Expression
    {
        ...
        if (!isset($serviceValues[self::EXCLUDE])) {
            return new \_PhpScoperac4e86be08e5\PhpParser\Node\Stmt\Expression($servicesLoadMethodCall);
        }
        ...
        $excludeMethodCall = new \_PhpScoperac4e86be08e5\PhpParser\Node\Expr\MethodCall($servicesLoadMethodCall, self::EXCLUDE, $args);
        return new \_PhpScoperac4e86be08e5\PhpParser\Node\Stmt\Expression($excludeMethodCall);
    }
    private function createServicesLoadMethodCall(string $serviceKey, $serviceValues) : \_PhpScoperac4e86be08e5\PhpParser\Node\Expr\MethodCall
    {
        ...
        $args = [];
        $args[] = new \_PhpScoperac4e86be08e5\PhpParser\Node\Arg(new \_PhpScoperac4e86be08e5\PhpParser\Node\Scalar\String_($serviceKey));
        $args[] = new \_PhpScoperac4e86be08e5\PhpParser\Node\Arg($this->commonNodeFactory->createAbsoluteDirExpr($resource));
        return new \_PhpScoperac4e86be08e5\PhpParser\Node\Expr\MethodCall($servicesVariable, 'load', $args);
    }
}
