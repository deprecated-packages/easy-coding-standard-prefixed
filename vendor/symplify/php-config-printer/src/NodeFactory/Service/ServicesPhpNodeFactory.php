...
final class ServicesPhpNodeFactory
{
    ...
    public function createResource(string $serviceKey, array $serviceValues) : \_PhpScoperab9510cd5d97\PhpParser\Node\Stmt\Expression
    {
        ...
        if (!isset($serviceValues[self::EXCLUDE])) {
            return new \_PhpScoperab9510cd5d97\PhpParser\Node\Stmt\Expression($servicesLoadMethodCall);
        }
        ...
        $excludeMethodCall = new \_PhpScoperab9510cd5d97\PhpParser\Node\Expr\MethodCall($servicesLoadMethodCall, self::EXCLUDE, $args);
        return new \_PhpScoperab9510cd5d97\PhpParser\Node\Stmt\Expression($excludeMethodCall);
    }
    private function createServicesLoadMethodCall(string $serviceKey, $serviceValues) : \_PhpScoperab9510cd5d97\PhpParser\Node\Expr\MethodCall
    {
        ...
        $args = [];
        $args[] = new \_PhpScoperab9510cd5d97\PhpParser\Node\Arg(new \_PhpScoperab9510cd5d97\PhpParser\Node\Scalar\String_($serviceKey));
        $args[] = new \_PhpScoperab9510cd5d97\PhpParser\Node\Arg($this->commonNodeFactory->createAbsoluteDirExpr($resource));
        return new \_PhpScoperab9510cd5d97\PhpParser\Node\Expr\MethodCall($servicesVariable, 'load', $args);
    }
}
