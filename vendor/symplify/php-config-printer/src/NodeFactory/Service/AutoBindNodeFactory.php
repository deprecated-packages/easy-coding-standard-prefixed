...
final class AutoBindNodeFactory
{
     ...
     * ->bind()
     */
    public function createAutoBindCalls(array $yaml, \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall
    {
    ...
    private function createBindMethodCall(\_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall $methodCall, array $bindValues) : \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall
    {
        foreach ($bindValues as $key => $value) {
            $args = $this->argsNodeFactory->createFromValues([$key, $value]);
            $methodCall = new \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND, $args);
        }
        return $methodCall;
    }
    private function createAutowire($value, \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall
    {
        if ($value === \true) {
            return new \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        }
        $args = [new \_PhpScoper59ccd3f8e121\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE, $args);
    }
    private function createAutoconfigure($value, \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall $methodCall, string $type)
    {
        if ($value === \true) {
            return new \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        ...
        $args = [new \_PhpScoper59ccd3f8e121\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE, $args);
    }
    private function createPublicPrivate($value, \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall
    {
        if ($value !== \false) {
        ...
        $args = [new \_PhpScoper59ccd3f8e121\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall($methodCall, 'public', $args);
    }
    private function createTagsMethodCall(\_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall $methodCall, $value) : \_PhpScoper59ccd3f8e121\PhpParser\Node\Expr\MethodCall
    {
