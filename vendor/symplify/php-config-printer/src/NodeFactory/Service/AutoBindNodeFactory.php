...
final class AutoBindNodeFactory
{
     ...
     * ->bind()
     */
    public function createAutoBindCalls(array $yaml, \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall
    {
    ...
    private function createBindMethodCall(\_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall $methodCall, array $bindValues) : \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall
    {
        foreach ($bindValues as $key => $value) {
            $args = $this->argsNodeFactory->createFromValues([$key, $value]);
            $methodCall = new \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND, $args);
        }
        return $methodCall;
    }
    private function createAutowire($value, \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall
    {
        if ($value === \true) {
            return new \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        }
        $args = [new \_PhpScoperca8ca183ac38\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE, $args);
    }
    private function createAutoconfigure($value, \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall $methodCall, string $type)
    {
        if ($value === \true) {
            return new \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        ...
        $args = [new \_PhpScoperca8ca183ac38\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE, $args);
    }
    private function createPublicPrivate($value, \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall
    {
        if ($value !== \false) {
        ...
        $args = [new \_PhpScoperca8ca183ac38\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall($methodCall, 'public', $args);
    }
    private function createTagsMethodCall(\_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall $methodCall, $value) : \_PhpScoperca8ca183ac38\PhpParser\Node\Expr\MethodCall
    {
