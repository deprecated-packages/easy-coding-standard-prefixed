...
final class AutoBindNodeFactory
{
     ...
     * ->bind()
     */
    public function createAutoBindCalls(array $yaml, \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall
    {
    ...
    private function createBindMethodCall(\_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall $methodCall, array $bindValues) : \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall
    {
        foreach ($bindValues as $key => $value) {
            $args = $this->argsNodeFactory->createFromValues([$key, $value]);
            $methodCall = new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND, $args);
        }
        return $methodCall;
    }
    private function createAutowire($value, \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall
    {
        if ($value === \true) {
            return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        }
        $args = [new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE, $args);
    }
    private function createAutoconfigure($value, \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall $methodCall, string $type)
    {
        if ($value === \true) {
            return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        ...
        $args = [new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE, $args);
    }
    private function createPublicPrivate($value, \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall
    {
        if ($value !== \false) {
            return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, 'public');
        }
        // default value
        if ($type === self::TYPE_DEFAULTS) {
            ...
            return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, 'private');
        }
        $args = [new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall($methodCall, 'public', $args);
    }
    private function createTagsMethodCall(\_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall $methodCall, $value) : \_PhpScoperf3f1be0d8a30\PhpParser\Node\Expr\MethodCall
    {
