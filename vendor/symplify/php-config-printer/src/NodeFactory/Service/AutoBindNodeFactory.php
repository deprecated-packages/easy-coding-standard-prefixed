...
final class AutoBindNodeFactory
{
     ...
     * ->bind()
     */
    public function createAutoBindCalls(array $yaml, \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall
    {
    ...
    private function createBindMethodCall(\_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall $methodCall, array $bindValues) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall
    {
        foreach ($bindValues as $key => $value) {
            $args = $this->argsNodeFactory->createFromValues([$key, $value]);
            $methodCall = new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND, $args);
        }
        return $methodCall;
    }
    private function createAutowire($value, \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall
    {
        if ($value === \true) {
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        }
        $args = [new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE, $args);
    }
    private function createAutoconfigure($value, \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall $methodCall, string $type)
    {
        if ($value === \true) {
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        ...
        $args = [new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, \Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE, $args);
    }
    private function createPublicPrivate($value, \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall
    {
        if ($value !== \false) {
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, 'public');
        }
        // default value
        if ($type === self::TYPE_DEFAULTS) {
            ...
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, 'private');
        }
        $args = [new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall($methodCall, 'public', $args);
    }
    private function createTagsMethodCall(\_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall $methodCall, $value) : \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\MethodCall
    {
