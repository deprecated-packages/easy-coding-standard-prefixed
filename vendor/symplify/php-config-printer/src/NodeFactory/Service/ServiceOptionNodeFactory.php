...
final class ServiceOptionNodeFactory
{
    ...
    public function convertServiceOptionsToNodes(array $servicesValues, \_PhpScoper9613f3fac51d\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoper9613f3fac51d\PhpParser\Node\Expr\MethodCall
    {
        $servicesValues = $this->unNestArguments($servicesValues);
        foreach ($servicesValues as $key => $value) {
            // options started by decoration_<option> are used as options of the method decorate().
            if (\_PhpScoper9613f3fac51d\Nette\Utils\Strings::startsWith($key, 'decoration_') || $key === 'alias') {
                continue;
            }
    ...
    private function isNestedArguments(array $servicesValues) : bool
    {
        ...
        foreach (\array_keys($servicesValues) as $key) {
            if (!\_PhpScoper9613f3fac51d\Nette\Utils\Strings::startsWith((string) $key, '$')) {
                return \false;
            }
        }
        return \true;
