...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopera6f918786d5c\PhpParser\Node\Expr
    {
        if (\_PhpScopera6f918786d5c\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopera6f918786d5c\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopera6f918786d5c\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopera6f918786d5c\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopera6f918786d5c\PhpParser\Node\Expr\ConstFetch(new \_PhpScopera6f918786d5c\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
