...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper0ba97041430d\PhpParser\Node\Expr
    {
        if (\_PhpScoper0ba97041430d\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper0ba97041430d\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper0ba97041430d\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper0ba97041430d\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper0ba97041430d\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper0ba97041430d\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
