...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper842c7347e6be\PhpParser\Node\Expr
    {
        if (\_PhpScoper842c7347e6be\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper842c7347e6be\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper842c7347e6be\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper842c7347e6be\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper842c7347e6be\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper842c7347e6be\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
