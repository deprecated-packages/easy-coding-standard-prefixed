...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr
    {
        if (\_PhpScoperc4ea0f0bd23f\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperc4ea0f0bd23f\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
