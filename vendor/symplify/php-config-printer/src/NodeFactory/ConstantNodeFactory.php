...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper971ef29294dd\PhpParser\Node\Expr
    {
        if (\_PhpScoper971ef29294dd\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper971ef29294dd\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper971ef29294dd\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper971ef29294dd\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper971ef29294dd\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper971ef29294dd\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
