...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperc75fd40d7a6e\PhpParser\Node\Expr
    {
        if (\_PhpScoperc75fd40d7a6e\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperc75fd40d7a6e\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperc75fd40d7a6e\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperc75fd40d7a6e\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperc75fd40d7a6e\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperc75fd40d7a6e\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
