...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper79449c4e744b\PhpParser\Node\Expr
    {
        if (\_PhpScoper79449c4e744b\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper79449c4e744b\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper79449c4e744b\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper79449c4e744b\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper79449c4e744b\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper79449c4e744b\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
