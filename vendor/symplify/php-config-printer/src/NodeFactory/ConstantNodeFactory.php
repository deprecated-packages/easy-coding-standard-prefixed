...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopera37d6fb0b1ab\PhpParser\Node\Expr
    {
        if (\_PhpScopera37d6fb0b1ab\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopera37d6fb0b1ab\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Expr\ConstFetch(new \_PhpScopera37d6fb0b1ab\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
