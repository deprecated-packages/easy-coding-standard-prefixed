...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopera1a51450b61d\PhpParser\Node\Expr
    {
        if (\_PhpScopera1a51450b61d\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopera1a51450b61d\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopera1a51450b61d\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopera1a51450b61d\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopera1a51450b61d\PhpParser\Node\Expr\ConstFetch(new \_PhpScopera1a51450b61d\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
