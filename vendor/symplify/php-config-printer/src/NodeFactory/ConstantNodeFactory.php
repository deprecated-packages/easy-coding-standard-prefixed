...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperfd70a7e8e84f\PhpParser\Node\Expr
    {
        if (\_PhpScoperfd70a7e8e84f\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperfd70a7e8e84f\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperfd70a7e8e84f\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperfd70a7e8e84f\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperfd70a7e8e84f\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperfd70a7e8e84f\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
