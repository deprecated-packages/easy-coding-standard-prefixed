...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopera88a8b9f064a\PhpParser\Node\Expr
    {
        if (\_PhpScopera88a8b9f064a\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopera88a8b9f064a\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopera88a8b9f064a\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopera88a8b9f064a\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopera88a8b9f064a\PhpParser\Node\Expr\ConstFetch(new \_PhpScopera88a8b9f064a\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
