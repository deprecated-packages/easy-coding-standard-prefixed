...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper58a0a169dcfb\PhpParser\Node\Expr
    {
        if (\_PhpScoper58a0a169dcfb\Nette\Utils\Strings::contains($value, '::')) {
            ...
            return new \_PhpScoper58a0a169dcfb\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper58a0a169dcfb\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper58a0a169dcfb\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper58a0a169dcfb\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper58a0a169dcfb\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
