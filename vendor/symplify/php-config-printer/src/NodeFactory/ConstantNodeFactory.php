...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopera04bf8e97c06\PhpParser\Node\Expr
    {
        if (\_PhpScopera04bf8e97c06\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopera04bf8e97c06\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopera04bf8e97c06\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopera04bf8e97c06\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopera04bf8e97c06\PhpParser\Node\Expr\ConstFetch(new \_PhpScopera04bf8e97c06\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
