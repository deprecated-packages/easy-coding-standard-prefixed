...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperdf15f2b748e9\PhpParser\Node\Expr
    {
        if (\_PhpScoperdf15f2b748e9\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperdf15f2b748e9\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperdf15f2b748e9\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperdf15f2b748e9\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperdf15f2b748e9\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperdf15f2b748e9\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
