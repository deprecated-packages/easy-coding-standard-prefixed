...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper28ab463fc3ba\PhpParser\Node\Expr
    {
        if (\_PhpScoper28ab463fc3ba\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper28ab463fc3ba\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper28ab463fc3ba\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper28ab463fc3ba\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper28ab463fc3ba\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper28ab463fc3ba\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
