...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper9d73a84b09ad\PhpParser\Node\Expr
    {
        if (\_PhpScoper9d73a84b09ad\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper9d73a84b09ad\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper9d73a84b09ad\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper9d73a84b09ad\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper9d73a84b09ad\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper9d73a84b09ad\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
