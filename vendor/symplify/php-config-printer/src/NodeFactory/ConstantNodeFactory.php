...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperef870243cfdb\PhpParser\Node\Expr
    {
        if (\_PhpScoperef870243cfdb\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperef870243cfdb\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperef870243cfdb\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperef870243cfdb\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperef870243cfdb\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperef870243cfdb\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
