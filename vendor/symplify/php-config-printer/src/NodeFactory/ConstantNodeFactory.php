...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper7c1f54fd2f3a\PhpParser\Node\Expr
    {
        if (\_PhpScoper7c1f54fd2f3a\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper7c1f54fd2f3a\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper7c1f54fd2f3a\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper7c1f54fd2f3a\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper7c1f54fd2f3a\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper7c1f54fd2f3a\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
