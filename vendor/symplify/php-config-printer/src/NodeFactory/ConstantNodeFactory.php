...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperda2604e33acb\PhpParser\Node\Expr
    {
        if (\_PhpScoperda2604e33acb\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperda2604e33acb\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperda2604e33acb\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperda2604e33acb\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperda2604e33acb\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperda2604e33acb\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
