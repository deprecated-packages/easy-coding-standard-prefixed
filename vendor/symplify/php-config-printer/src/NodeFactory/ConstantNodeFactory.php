...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper89ec3c69e67d\PhpParser\Node\Expr
    {
        if (\_PhpScoper89ec3c69e67d\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper89ec3c69e67d\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper89ec3c69e67d\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper89ec3c69e67d\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper89ec3c69e67d\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper89ec3c69e67d\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
