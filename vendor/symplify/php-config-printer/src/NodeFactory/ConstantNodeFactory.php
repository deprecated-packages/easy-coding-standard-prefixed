...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperfab1bfb7ec99\PhpParser\Node\Expr
    {
        if (\_PhpScoperfab1bfb7ec99\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperfab1bfb7ec99\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperfab1bfb7ec99\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperfab1bfb7ec99\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperfab1bfb7ec99\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperfab1bfb7ec99\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
