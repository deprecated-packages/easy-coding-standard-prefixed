...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr
    {
        if (\_PhpScoperf65af7a6d9a0\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperf65af7a6d9a0\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
