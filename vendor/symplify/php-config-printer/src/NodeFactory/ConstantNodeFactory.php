...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr
    {
        if (\_PhpScoperdc8fbcd7c69d\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperdc8fbcd7c69d\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperdc8fbcd7c69d\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
