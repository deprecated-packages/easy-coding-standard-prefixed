...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper4fc0030e9d22\PhpParser\Node\Expr
    {
        if (\_PhpScoper4fc0030e9d22\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper4fc0030e9d22\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper4fc0030e9d22\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper4fc0030e9d22\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper4fc0030e9d22\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper4fc0030e9d22\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
