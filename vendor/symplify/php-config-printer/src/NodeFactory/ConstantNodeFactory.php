...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopera23ebff5477f\PhpParser\Node\Expr
    {
        if (\_PhpScopera23ebff5477f\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopera23ebff5477f\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopera23ebff5477f\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopera23ebff5477f\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopera23ebff5477f\PhpParser\Node\Expr\ConstFetch(new \_PhpScopera23ebff5477f\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
