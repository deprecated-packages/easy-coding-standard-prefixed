...
final class ConstantNodeFactory
{
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper807f8e74693b\PhpParser\Node\Expr
    {
        if (\_PhpScoper807f8e74693b\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper807f8e74693b\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper807f8e74693b\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper807f8e74693b\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper807f8e74693b\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper807f8e74693b\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
