...
use _PhpScoperaba240c3d5f1\PhpParser\Node\Expr\Assign;
...
use _PhpScoperaba240c3d5f1\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperaba240c3d5f1\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoperaba240c3d5f1\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperaba240c3d5f1\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperaba240c3d5f1\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScoperaba240c3d5f1\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperaba240c3d5f1\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperaba240c3d5f1\PhpParser\Node\Stmt\Expression($assign);
    }
