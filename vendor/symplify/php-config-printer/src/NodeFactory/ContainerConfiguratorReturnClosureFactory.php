...
use _PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\Assign;
use _PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\MethodCall;
use _PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\Variable;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper06c5fb6c14ed\PhpParser\Node\Stmt\Return_
    {
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \_PhpScoper06c5fb6c14ed\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper06c5fb6c14ed\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper06c5fb6c14ed\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper06c5fb6c14ed\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\_PhpScoper06c5fb6c14ed\PhpParser\Node\Stmt\Expression
    {
