...
use _PhpScoper967d20dce97a\PhpParser\Node\Expr\Assign;
...
use _PhpScoper967d20dce97a\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper967d20dce97a\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper967d20dce97a\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper967d20dce97a\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper967d20dce97a\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoper967d20dce97a\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoper967d20dce97a\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper967d20dce97a\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper967d20dce97a\PhpParser\Node\Stmt\Expression($assign);
    }
