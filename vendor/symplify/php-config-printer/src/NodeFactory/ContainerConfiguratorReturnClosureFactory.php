...
use _PhpScoper5f6e904600e7\PhpParser\Node\Expr\Assign;
...
use _PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper5f6e904600e7\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScoper5f6e904600e7\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper5f6e904600e7\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\_PhpScoper5f6e904600e7\PhpParser\Node\Stmt\Expression
    {
