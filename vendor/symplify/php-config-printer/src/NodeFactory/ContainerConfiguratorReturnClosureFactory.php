...
use _PhpScoper9f8d5dcff860\PhpParser\Node\Expr\Assign;
...
use _PhpScoper9f8d5dcff860\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper9f8d5dcff860\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper9f8d5dcff860\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper9f8d5dcff860\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper9f8d5dcff860\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScoper9f8d5dcff860\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper9f8d5dcff860\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper9f8d5dcff860\PhpParser\Node\Stmt\Expression($assign);
    }
