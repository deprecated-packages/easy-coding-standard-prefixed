...
use _PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\Assign;
use _PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\MethodCall;
use _PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\Variable;
use _PhpScoperf65af7a6d9a0\PhpParser\Node\Stmt\Expression;
use _PhpScoperf65af7a6d9a0\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperf65af7a6d9a0\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperf65af7a6d9a0\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperf65af7a6d9a0\PhpParser\Node\Stmt\Expression($assign);
    }
