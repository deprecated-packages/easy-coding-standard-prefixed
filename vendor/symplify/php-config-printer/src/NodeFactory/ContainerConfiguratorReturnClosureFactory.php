...
use _PhpScoperef870243cfdb\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperef870243cfdb\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoperef870243cfdb\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperef870243cfdb\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperef870243cfdb\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScoperef870243cfdb\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperef870243cfdb\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperef870243cfdb\PhpParser\Node\Stmt\Expression($assign);
    }
