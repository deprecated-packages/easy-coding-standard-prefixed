...
use _PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Assign;
use _PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\MethodCall;
use _PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Variable;
use _PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Expression;
use _PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperb2e2c0c42e71\PhpParser\Node\Stmt\Expression($assign);
    }
