...
use _PhpScopera4fc793dae73\PhpParser\Node\Expr\Assign;
use _PhpScopera4fc793dae73\PhpParser\Node\Expr\MethodCall;
use _PhpScopera4fc793dae73\PhpParser\Node\Expr\Variable;
use _PhpScopera4fc793dae73\PhpParser\Node\Stmt\Expression;
use _PhpScopera4fc793dae73\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScopera4fc793dae73\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScopera4fc793dae73\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScopera4fc793dae73\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScopera4fc793dae73\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScopera4fc793dae73\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScopera4fc793dae73\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScopera4fc793dae73\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScopera4fc793dae73\PhpParser\Node\Stmt\Expression($assign);
    }
