...
use _PhpScoper18bd934c069f\PhpParser\Node\Expr\Assign;
...
use _PhpScoper18bd934c069f\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper18bd934c069f\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper18bd934c069f\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper18bd934c069f\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper18bd934c069f\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoper18bd934c069f\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoper18bd934c069f\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper18bd934c069f\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper18bd934c069f\PhpParser\Node\Stmt\Expression($assign);
    }
