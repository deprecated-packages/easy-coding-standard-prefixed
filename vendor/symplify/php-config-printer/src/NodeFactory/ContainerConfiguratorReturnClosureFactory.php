...
use _PhpScopercb217fd4e736\PhpParser\Node\Expr\Assign;
use _PhpScopercb217fd4e736\PhpParser\Node\Expr\MethodCall;
use _PhpScopercb217fd4e736\PhpParser\Node\Expr\Variable;
use _PhpScopercb217fd4e736\PhpParser\Node\Stmt\Expression;
use _PhpScopercb217fd4e736\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScopercb217fd4e736\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScopercb217fd4e736\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScopercb217fd4e736\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScopercb217fd4e736\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScopercb217fd4e736\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScopercb217fd4e736\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScopercb217fd4e736\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScopercb217fd4e736\PhpParser\Node\Stmt\Expression($assign);
    }
