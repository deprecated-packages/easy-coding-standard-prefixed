...
use _PhpScopera1f11cc38772\PhpParser\Node\Expr\Assign;
...
use _PhpScopera1f11cc38772\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScopera1f11cc38772\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScopera1f11cc38772\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \_PhpScopera1f11cc38772\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScopera1f11cc38772\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScopera1f11cc38772\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScopera1f11cc38772\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScopera1f11cc38772\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScopera1f11cc38772\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScopera1f11cc38772\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\_PhpScopera1f11cc38772\PhpParser\Node\Stmt\Expression
    {
