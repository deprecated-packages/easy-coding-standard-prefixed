...
use _PhpScoperc4b135661b3a\PhpParser\Node\Expr\Assign;
...
use _PhpScoperc4b135661b3a\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperc4b135661b3a\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoperc4b135661b3a\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperc4b135661b3a\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperc4b135661b3a\PhpParser\Node\Stmt\Expression($assign);
    }
