...
use _PhpScoperd4c5032f0671\PhpParser\Node\Expr\Assign;
...
use _PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperd4c5032f0671\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoperd4c5032f0671\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoperd4c5032f0671\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperd4c5032f0671\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Expression($assign);
    }
