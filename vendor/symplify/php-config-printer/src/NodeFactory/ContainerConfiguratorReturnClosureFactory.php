...
use _PhpScoper89c09b8e7101\PhpParser\Node\Expr\Assign;
use _PhpScoper89c09b8e7101\PhpParser\Node\Expr\MethodCall;
use _PhpScoper89c09b8e7101\PhpParser\Node\Expr\Variable;
use _PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Expression;
use _PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \_PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper89c09b8e7101\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoper89c09b8e7101\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoper89c09b8e7101\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper89c09b8e7101\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\_PhpScoper89c09b8e7101\PhpParser\Node\Stmt\Expression
    {
