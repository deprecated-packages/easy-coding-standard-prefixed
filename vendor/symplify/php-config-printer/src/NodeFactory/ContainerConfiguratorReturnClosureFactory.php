...
use _PhpScoper6db4fde00cda\PhpParser\Node\Expr\Assign;
use _PhpScoper6db4fde00cda\PhpParser\Node\Expr\MethodCall;
use _PhpScoper6db4fde00cda\PhpParser\Node\Expr\Variable;
use _PhpScoper6db4fde00cda\PhpParser\Node\Stmt\Expression;
use _PhpScoper6db4fde00cda\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper6db4fde00cda\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper6db4fde00cda\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper6db4fde00cda\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper6db4fde00cda\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoper6db4fde00cda\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoper6db4fde00cda\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper6db4fde00cda\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper6db4fde00cda\PhpParser\Node\Stmt\Expression($assign);
    }
