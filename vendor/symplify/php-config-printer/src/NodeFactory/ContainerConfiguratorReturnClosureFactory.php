...
use _PhpScopera23ebff5477f\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScopera23ebff5477f\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScopera23ebff5477f\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScopera23ebff5477f\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScopera23ebff5477f\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScopera23ebff5477f\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScopera23ebff5477f\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScopera23ebff5477f\PhpParser\Node\Stmt\Expression($assign);
    }
