...
use _PhpScoper8acb416c2f5a\PhpParser\Node\Expr\Assign;
use _PhpScoper8acb416c2f5a\PhpParser\Node\Expr\MethodCall;
use _PhpScoper8acb416c2f5a\PhpParser\Node\Expr\Variable;
use _PhpScoper8acb416c2f5a\PhpParser\Node\Stmt\Expression;
use _PhpScoper8acb416c2f5a\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper8acb416c2f5a\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper8acb416c2f5a\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper8acb416c2f5a\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper8acb416c2f5a\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoper8acb416c2f5a\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoper8acb416c2f5a\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper8acb416c2f5a\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper8acb416c2f5a\PhpParser\Node\Stmt\Expression($assign);
    }
