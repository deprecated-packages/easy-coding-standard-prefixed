...
use _PhpScoper246d3630afdd\PhpParser\Node\Stmt\Return_;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoper246d3630afdd\PhpParser\Node\Stmt\Return_
    {
        $stmts = $this->createClosureStmts($arrayData);
        $closure = $this->configuratorClosureNodeFactory->createContainerClosureFromStmts($stmts);
        return new \_PhpScoper246d3630afdd\PhpParser\Node\Stmt\Return_($closure);
    }
    /**
     * @return Node[]
     */
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \_PhpScoper246d3630afdd\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoper246d3630afdd\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoper246d3630afdd\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScoper246d3630afdd\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoper246d3630afdd\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoper246d3630afdd\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\_PhpScoper246d3630afdd\PhpParser\Node\Stmt\Expression
    {
