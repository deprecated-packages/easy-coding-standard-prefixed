...
use _PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\Variable;
...
final class ContainerConfiguratorReturnClosureFactory
{
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Stmt\Return_
    {
    ...
    private function createNodesFromCaseConverters(array $yamlData) : array
    {
        ...
        foreach ($yamlData as $key => $values) {
            ...
            foreach ($values as $nestedKey => $nestedValues) {
                ...
                if (!$expression instanceof \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Stmt\Expression) {
                    continue;
                }
                $nodes[] = $expression;
            }
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\Variable($variableName);
        $containerConfiguratorVariable = new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::CONTAINER_CONFIGURATOR);
        $assign = new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperc4ea0f0bd23f\PhpParser\Node\Stmt\Expression($assign);
    }
    /**
     * @return mixed[]
     */
     ...
     */
    private function resolveExpression(string $key, $nestedKey, $nestedValues) : ?\_PhpScoperc4ea0f0bd23f\PhpParser\Node\Stmt\Expression
    {
