...
use _PhpScoper4f42ead57614\PhpParser\Node\Expr\ArrayItem;
use _PhpScoper4f42ead57614\PhpParser\Node\Expr\FuncCall;
...
final class ArgsNodeFactory
{
    ...
    public function createFromValuesAndWrapInArray($values) : array
    {
        if (\is_array($values)) {
            ...
            $items = [new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\ArrayItem($expr)];
            $array = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Array_($items);
        }
        return [new \_PhpScoper4f42ead57614\PhpParser\Node\Arg($array)];
    }
    /**
     * @return Arg[]
     */
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : array
    {
        if (\is_array($values)) {
            ...
            foreach ($values as $value) {
                ...
                $args[] = new \_PhpScoper4f42ead57614\PhpParser\Node\Arg($expr);
            }
            return $args;
        }
        if ($values instanceof \_PhpScoper4f42ead57614\PhpParser\Node) {
            if ($values instanceof \_PhpScoper4f42ead57614\PhpParser\Node\Arg) {
                return [$values];
            }
            if ($values instanceof \_PhpScoper4f42ead57614\PhpParser\Node\Expr) {
                return [new \_PhpScoper4f42ead57614\PhpParser\Node\Arg($values)];
            }
        }
        if (\is_string($values)) {
            $expr = $this->resolveExpr($values);
            return [new \_PhpScoper4f42ead57614\PhpParser\Node\Arg($expr)];
        }
        throw new \Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    public function resolveExpr($value, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr
    {
        if (\is_string($value)) {
            return $this->stringExprResolver->resolve($value, $skipServiceReference, $skipClassesToConstantReference);
        }
        if ($value instanceof \_PhpScoper4f42ead57614\PhpParser\Node\Expr) {
            return $value;
        }
        ...
        if (\is_array($value)) {
            ...
            return new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Array_($arrayItems);
        }
        return \_PhpScoper4f42ead57614\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function resolveExprFromArray(array $values) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Array_
    {
        $arrayItems = [];
        foreach ($values as $key => $value) {
            ...
            if (!\is_int($key)) {
                ...
                $arrayItem = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\ArrayItem($expr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\ArrayItem($expr);
            }
            $arrayItems[] = $arrayItem;
        }
        return new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\Array_($arrayItems);
    }
    private function createServiceReferenceFromTaggedValue(\_PhpScoper4f42ead57614\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr
    {
        ...
        return new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\FuncCall(new \_PhpScoper4f42ead57614\PhpParser\Node\Name($taggedValue->getTag()), $args);
    }
    /**
     * @param mixed[] $value
     * @return ArrayItem[]
    ...
    private function resolveArrayItems(array $value, bool $skipClassesToConstantReference) : array
    {
        ...
        foreach ($value as $nestedKey => $nestedValue) {
            ...
            if (!\is_int($nestedKey) || $nestedKey !== $naturalKey) {
                ...
                $arrayItem = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\ArrayItem($valueExpr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\ArrayItem($valueExpr);
            }
            $arrayItems[] = $arrayItem;
            ++$naturalKey;
        }
