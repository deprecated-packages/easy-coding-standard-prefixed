...
use _PhpScoperc8b83ee8976a\PhpParser\Node\Expr\Array_;
use _PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem;
use _PhpScoperc8b83ee8976a\PhpParser\Node\Expr\FuncCall;
...
final class ArgsNodeFactory
{
    ...
    public function createFromValuesAndWrapInArray($values) : array
    {
        if (\is_array($values)) {
            ...
            $items = [new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem($expr)];
            $array = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\Array_($items);
        }
        return [new \_PhpScoperc8b83ee8976a\PhpParser\Node\Arg($array)];
    }
    /**
     * @return Arg[]
     */
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : array
    {
        if (\is_array($values)) {
            ...
            foreach ($values as $value) {
                ...
                $args[] = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Arg($expr);
            }
            return $args;
        }
        if ($values instanceof \_PhpScoperc8b83ee8976a\PhpParser\Node) {
            if ($values instanceof \_PhpScoperc8b83ee8976a\PhpParser\Node\Arg) {
                return [$values];
            }
            if ($values instanceof \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr) {
                return [new \_PhpScoperc8b83ee8976a\PhpParser\Node\Arg($values)];
            }
        }
        if (\is_string($values)) {
            $expr = $this->resolveExpr($values);
            return [new \_PhpScoperc8b83ee8976a\PhpParser\Node\Arg($expr)];
        }
        throw new \Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    public function resolveExpr($value, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr
    {
        if (\is_string($value)) {
            return $this->resolveStringExpr($value, $skipServiceReference, $skipClassesToConstantReference);
        }
        if ($value instanceof \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr) {
            return $value;
        }
        ...
        if (\is_array($value)) {
            ...
            return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\Array_($arrayItems);
        }
        return \_PhpScoperc8b83ee8976a\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
        ...
        $args = [new \_PhpScoperc8b83ee8976a\PhpParser\Node\Arg($expr)];
        return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\FuncCall(new \_PhpScoperc8b83ee8976a\PhpParser\Node\Name\FullyQualified($functionName), $args);
    }
    private function resolveExprFromArray(array $values) : \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\Array_
    {
        $arrayItems = [];
        foreach ($values as $key => $value) {
            ...
            if (!\is_int($key)) {
                ...
                $arrayItem = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem($expr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem($expr);
            }
            $arrayItems[] = $arrayItem;
        }
        return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\Array_($arrayItems);
    }
    private function createServiceReferenceFromTaggedValue(\_PhpScoperc8b83ee8976a\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr
    {
        ...
        if ($taggedValue->getTag() === self::TAG_RETURNS_CLONE) {
            ...
            return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\FuncCall(new \_PhpScoperc8b83ee8976a\PhpParser\Node\Name($taggedValue->getTag()), $args);
        }
        $funcCall = $this->resolveServiceReferenceExpr($serviceName, \false, $functionName);
        if ($shouldWrapInArray) {
            return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\Array_([new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem($funcCall)]);
        }
        return $funcCall;
    }
    private function resolveStringExpr(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\in_array($value, ["\r", "\n", "\r\n"], \true)) {
            $string = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Scalar\String_($value);
            $string->setAttribute(self::KIND, \_PhpScoperc8b83ee8976a\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
            return $string;
        }
        $value = \ltrim($value, '\\');
        ...
        if (\_PhpScoperc8b83ee8976a\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            $args = $this->createFromValues($value);
            return new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\FuncCall(new \_PhpScoperc8b83ee8976a\PhpParser\Node\Name\FullyQualified(\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\_PhpScoperc8b83ee8976a\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    private function resolveArrayItems(array $value, bool $skipClassesToConstantReference) : array
    {
        ...
        foreach ($value as $nestedKey => $nestedValue) {
            ...
            if (!\is_int($nestedKey) || $nestedKey !== $naturalKey) {
                ...
                $arrayItem = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem($valueExpr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoperc8b83ee8976a\PhpParser\Node\Expr\ArrayItem($valueExpr);
            }
            $arrayItems[] = $arrayItem;
            ++$naturalKey;
        }
    ...
    {
        return (bool) \_PhpScoperc8b83ee8976a\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
