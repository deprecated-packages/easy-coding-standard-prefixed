...
use _PhpScoper0c0702cca4ac\PhpParser\Node\Expr\Array_;
use _PhpScoper0c0702cca4ac\PhpParser\Node\Expr\ArrayItem;
...
final class ArgsNodeFactory
{
    ...
    public function createFromValuesAndWrapInArray($values) : array
    {
        if (\is_array($values)) {
            ...
            $items = [new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\ArrayItem($expr)];
            $array = new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\Array_($items);
        }
        return [new \_PhpScoper0c0702cca4ac\PhpParser\Node\Arg($array)];
    }
    /**
     * @return Arg[]
     */
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : array
    {
        ...
        if ($values instanceof \_PhpScoper0c0702cca4ac\PhpParser\Node) {
            if ($values instanceof \_PhpScoper0c0702cca4ac\PhpParser\Node\Arg) {
                return [$values];
            }
            if ($values instanceof \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr) {
                return [new \_PhpScoper0c0702cca4ac\PhpParser\Node\Arg($values)];
            }
        }
        if (\is_string($values)) {
            $expr = $this->resolveExpr($values);
            return [new \_PhpScoper0c0702cca4ac\PhpParser\Node\Arg($expr)];
        }
        throw new \Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    public function resolveExpr($value, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr
    {
        if (\is_string($value)) {
            return $this->stringExprResolver->resolve($value, $skipServiceReference, $skipClassesToConstantReference);
        }
        if ($value instanceof \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr) {
            return $value;
        }
        ...
        if (\is_array($value)) {
            ...
            return new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\Array_($arrayItems);
        }
        return \_PhpScoper0c0702cca4ac\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function resolveExprFromArray(array $values) : \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\Array_
    {
        $arrayItems = [];
        foreach ($values as $key => $value) {
            ...
            if (!\is_int($key)) {
                ...
                $arrayItem = new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\ArrayItem($expr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\ArrayItem($expr);
            }
            $arrayItems[] = $arrayItem;
        }
        return new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\Array_($arrayItems);
    }
    private function createServiceReferenceFromTaggedValue(\_PhpScoper0c0702cca4ac\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr
    {
        ...
        return new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\FuncCall(new \_PhpScoper0c0702cca4ac\PhpParser\Node\Name($taggedValue->getTag()), $args);
    }
    /**
     * @param mixed[] $value
     * @return ArrayItem[]
    ...
    private function resolveArrayItems(array $value, bool $skipClassesToConstantReference) : array
    {
        ...
        foreach ($value as $nestedKey => $nestedValue) {
            ...
            if (!\is_int($nestedKey) || $nestedKey !== $naturalKey) {
                ...
                $arrayItem = new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\ArrayItem($valueExpr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoper0c0702cca4ac\PhpParser\Node\Expr\ArrayItem($valueExpr);
            }
            $arrayItems[] = $arrayItem;
            ++$naturalKey;
        }
