...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoper6a1dd9b8a650\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoper6a1dd9b8a650\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoper6a1dd9b8a650\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoper6a1dd9b8a650\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoper6a1dd9b8a650\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\_PhpScoper6a1dd9b8a650\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
