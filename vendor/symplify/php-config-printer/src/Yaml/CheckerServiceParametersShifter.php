...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoperb6a8e65b492c\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoperb6a8e65b492c\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoperb6a8e65b492c\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoperb6a8e65b492c\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoperb6a8e65b492c\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\_PhpScoperb6a8e65b492c\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
