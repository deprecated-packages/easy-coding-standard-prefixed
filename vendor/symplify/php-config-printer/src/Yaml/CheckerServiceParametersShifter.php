...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoper8163b0b2b8f3\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoper8163b0b2b8f3\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        if (\_PhpScoper8163b0b2b8f3\Nette\Utils\Strings::endsWith($checker, 'Fixer')) {
            return \true;
        }
        return \_PhpScoper8163b0b2b8f3\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoper8163b0b2b8f3\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
