...
final class CheckerServiceParametersShifter
{
    ...
    public function __construct()
    {
        ...
        $reflectionClass = new \ReflectionClass(\_PhpScoperf3db63c305b2\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoperf3db63c305b2\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoperf3db63c305b2\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoperf3db63c305b2\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoperf3db63c305b2\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoperf3db63c305b2\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
