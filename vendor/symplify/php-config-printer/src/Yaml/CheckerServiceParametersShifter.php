...
final class CheckerServiceParametersShifter
{
    ...
    public function __construct()
    {
        ...
        $reflectionClass = new \ReflectionClass(\_PhpScopere341acab57d4\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScopere341acab57d4\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScopere341acab57d4\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScopere341acab57d4\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScopere341acab57d4\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScopere341acab57d4\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
