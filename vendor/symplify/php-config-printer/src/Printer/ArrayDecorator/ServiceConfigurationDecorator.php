...
final class ServiceConfigurationDecorator
{
    ...
    private function decorateValueObject(object $value) : \_PhpScoper528afa732cbd\PhpParser\Node\Expr\FuncCall
    {
        $new = $this->newValueObjectFactory->create($value);
        $args = [new \_PhpScoper528afa732cbd\PhpParser\Node\Arg($new)];
    ...
    private function decorateValueObjects(array $values) : \_PhpScoper528afa732cbd\PhpParser\Node\Expr\FuncCall
    {
        $arrayItems = [];
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \_PhpScoper528afa732cbd\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \_PhpScoper528afa732cbd\PhpParser\Node\Expr\Array_($arrayItems);
        $args = [new \_PhpScoper528afa732cbd\PhpParser\Node\Arg($array)];
