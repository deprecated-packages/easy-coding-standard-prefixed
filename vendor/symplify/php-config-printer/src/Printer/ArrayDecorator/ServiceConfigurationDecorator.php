...
final class ServiceConfigurationDecorator
{
    ...
    private function decorateValueObject(object $value) : \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\FuncCall
    {
        $new = $this->newValueObjectFactory->create($value);
        $args = [new \_PhpScoperc4b135661b3a\PhpParser\Node\Arg($new)];
    ...
    private function decorateValueObjects(array $values) : \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\FuncCall
    {
        $arrayItems = [];
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \_PhpScoperc4b135661b3a\PhpParser\Node\Expr\Array_($arrayItems);
        $args = [new \_PhpScoperc4b135661b3a\PhpParser\Node\Arg($array)];
