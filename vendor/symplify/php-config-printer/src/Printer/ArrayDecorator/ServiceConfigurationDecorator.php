...
final class ServiceConfigurationDecorator
{
    ...
    private function decorateValueObject(object $value) : \_PhpScoperf3d5f0921050\PhpParser\Node\Expr\FuncCall
    {
        $new = $this->newValueObjectFactory->create($value);
        $args = [new \_PhpScoperf3d5f0921050\PhpParser\Node\Arg($new)];
    ...
    private function decorateValueObjects(array $values) : \_PhpScoperf3d5f0921050\PhpParser\Node\Expr\FuncCall
    {
        $arrayItems = [];
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \_PhpScoperf3d5f0921050\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \_PhpScoperf3d5f0921050\PhpParser\Node\Expr\Array_($arrayItems);
        $args = [new \_PhpScoperf3d5f0921050\PhpParser\Node\Arg($array)];
