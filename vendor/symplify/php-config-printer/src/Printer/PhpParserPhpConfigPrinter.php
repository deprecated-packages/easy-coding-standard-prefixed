...
use _PhpScoperd4c5032f0671\PhpParser\Node\Scalar\LNumber;
use _PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Declare_;
use _PhpScoperd4c5032f0671\PhpParser\Node\Stmt\DeclareDeclare;
use _PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Nop;
use _PhpScoperd4c5032f0671\PhpParser\PrettyPrinter\Standard;
...
final class PhpParserPhpConfigPrinter extends \_PhpScoperd4c5032f0671\PhpParser\PrettyPrinter\Standard
{
    /**
    ...
    public function prettyPrintFile(array $stmts) : string
    {
        ...
        $printedContent = \_PhpScoperd4c5032f0671\Nette\Utils\Strings::replace($printedContent, self::START_WITH_SPACE_REGEX, "\n");
        // remove space before " :" in main closure
        $printedContent = \_PhpScoperd4c5032f0671\Nette\Utils\Strings::replace($printedContent, self::VOID_AFTER_FUNC_REGEX, '): void');
        // remove space between declare strict types
        $printedContent = \_PhpScoperd4c5032f0671\Nette\Utils\Strings::replace($printedContent, self::DECLARE_SPACE_STRICT_REGEX, 'declare(strict');
    ...
    {
        return "'" . \_PhpScoperd4c5032f0671\Nette\Utils\Strings::replace($string, self::QUOTE_SLASH_REGEX, '\\\\$0') . "'";
    }
    protected function pExpr_Array(\_PhpScoperd4c5032f0671\PhpParser\Node\Expr\Array_ $array) : string
    {
        $array->setAttribute(self::KIND, \_PhpScoperd4c5032f0671\PhpParser\Node\Expr\Array_::KIND_SHORT);
        return parent::pExpr_Array($array);
    }
    protected function pExpr_MethodCall(\_PhpScoperd4c5032f0671\PhpParser\Node\Expr\MethodCall $methodCall) : string
    {
    ...
    {
        $nextCallIndentReplacement = ')' . \PHP_EOL . \_PhpScoperd4c5032f0671\Nette\Utils\Strings::indent('->', 8, ' ');
        return \_PhpScoperd4c5032f0671\Nette\Utils\Strings::replace($content, '#\\)->#', $nextCallIndentReplacement);
    }
    /**
     * @param Node[] $stmts
     * @return Node[]
    ...
    private function prependStrictTypesDeclare(array $stmts) : array
    {
        ...
        return \array_merge([$strictTypesDeclare, new \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Nop()], $stmts);
    }
    private function createStrictTypesDeclare() : \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Declare_
    {
        $declareDeclare = new \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\DeclareDeclare('strict_types', new \_PhpScoperd4c5032f0671\PhpParser\Node\Scalar\LNumber(1));
        return new \_PhpScoperd4c5032f0671\PhpParser\Node\Stmt\Declare_([$declareDeclare]);
    }
}
