...
use _PhpScoper7f5523334c1b\PhpParser\NodeTraverser;
...
final class ImportFullyQualifiedNamesNodeTraverser
{
    ...
    public function __construct(\Symplify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor $importFullyQualifiedNamesNodeVisitor, \_PhpScoper7f5523334c1b\PhpParser\BuilderFactory $builderFactory)
    {
    ...
    private function addUseImportsToNamespace(array $nodes, array $nameImports) : array
    {
        ...
        $useImports[] = new \_PhpScoper7f5523334c1b\PhpParser\Node\Stmt\Nop();
    ...
    {
        $nodeTraverser = new \_PhpScoper7f5523334c1b\PhpParser\NodeTraverser();
    ...
    private function createUses(array $nameImports) : array
    {
        ...
        foreach ($nameImports as $nameImport) {
            $shortNameImport = \_PhpScoper7f5523334c1b\Nette\Utils\Strings::after($nameImport, '\\', -1);
            if (\function_exists($nameImport) || $shortNameImport === 'ref') {
                $useBuilder = $this->builderFactory->useFunction(new \_PhpScoper7f5523334c1b\PhpParser\Node\Name($nameImport));
                /** @var Use_ $use */
                $use = $useBuilder->getNode();
                $useImports[] = $use;
            } else {
                $useBuilder = $this->builderFactory->use(new \_PhpScoper7f5523334c1b\PhpParser\Node\Name($nameImport));
                /** @var Use_ $use */
                $use = $useBuilder->getNode();
                $useImports[] = $use;
            }
