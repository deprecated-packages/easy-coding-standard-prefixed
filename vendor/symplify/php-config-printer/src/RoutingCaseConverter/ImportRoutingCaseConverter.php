...
final class ImportRoutingCaseConverter implements \Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    ...
    public function convertToMethodCall(string $key, $values) : \_PhpScoper21fff473f90a\PhpParser\Node\Stmt\Expression
    {
        $variable = new \_PhpScoper21fff473f90a\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        $args = $this->createAddArgs(self::IMPORT_ARGS, $values);
        $methodCall = new \_PhpScoper21fff473f90a\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \_PhpScoper21fff473f90a\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \_PhpScoper21fff473f90a\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
