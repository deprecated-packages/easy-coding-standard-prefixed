...
final class ImportRoutingCaseConverter implements \Symplify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    ...
    {
        $variable = new \_PhpScoper5c006f5f032f\PhpParser\Node\Expr\Variable(\Symplify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        $args = $this->createAddArgs(self::IMPORT_ARGS, $values);
        $methodCall = new \_PhpScoper5c006f5f032f\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \_PhpScoper5c006f5f032f\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \_PhpScoper5c006f5f032f\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
