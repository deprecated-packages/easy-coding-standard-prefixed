...
final class TagsServiceOptionKeyYamlToPhpFactory implements \Symplify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \_PhpScoper326bba7310a2\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoper326bba7310a2\PhpParser\Node\Expr\MethodCall
    {
        ...
        if (\count($yaml) === 1 && \is_string($yaml[0])) {
            $string = new \_PhpScoper326bba7310a2\PhpParser\Node\Scalar\String_($yaml[0]);
            return new \_PhpScoper326bba7310a2\PhpParser\Node\Expr\MethodCall($methodCall, self::TAG, [new \_PhpScoper326bba7310a2\PhpParser\Node\Arg($string)]);
        }
        foreach ($yaml as $singleValue) {
            $args = [];
            foreach ($singleValue as $singleNestedKey => $singleNestedValue) {
                if ($singleNestedKey === 'name') {
                    $args[] = new \_PhpScoper326bba7310a2\PhpParser\Node\Arg(\_PhpScoper326bba7310a2\PhpParser\BuilderHelpers::normalizeValue($singleNestedValue));
                    unset($singleValue[$singleNestedKey]);
                }
            }
            $restArgs = $this->argsNodeFactory->createFromValuesAndWrapInArray($singleValue);
            $args = \array_merge($args, $restArgs);
            $methodCall = new \_PhpScoper326bba7310a2\PhpParser\Node\Expr\MethodCall($methodCall, self::TAG, $args);
        }
        return $methodCall;
    }
