...
final class TagsServiceOptionKeyYamlToPhpFactory implements \Symplify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \_PhpScoperd35c27cd4b09\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoperd35c27cd4b09\PhpParser\Node\Expr\MethodCall
    {
        ...
        if (\count($yaml) === 1 && \is_string($yaml[0])) {
            $string = new \_PhpScoperd35c27cd4b09\PhpParser\Node\Scalar\String_($yaml[0]);
            return new \_PhpScoperd35c27cd4b09\PhpParser\Node\Expr\MethodCall($methodCall, self::TAG, [new \_PhpScoperd35c27cd4b09\PhpParser\Node\Arg($string)]);
        }
        foreach ($yaml as $singleValue) {
            $args = [];
            foreach ($singleValue as $singleNestedKey => $singleNestedValue) {
                if ($singleNestedKey === 'name') {
                    $args[] = new \_PhpScoperd35c27cd4b09\PhpParser\Node\Arg(\_PhpScoperd35c27cd4b09\PhpParser\BuilderHelpers::normalizeValue($singleNestedValue));
                    unset($singleValue[$singleNestedKey]);
                }
            }
            $restArgs = $this->argsNodeFactory->createFromValuesAndWrapInArray($singleValue);
            $args = \array_merge($args, $restArgs);
            $methodCall = new \_PhpScoperd35c27cd4b09\PhpParser\Node\Expr\MethodCall($methodCall, self::TAG, $args);
        }
        return $methodCall;
    }
