...
final class BindAutowireAutoconfigureServiceOptionKeyYamlToPhpFactory implements \Symplify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \_PhpScoper528afa732cbd\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoper528afa732cbd\PhpParser\Node\Expr\MethodCall
    {
        ...
        $methodCall = new \_PhpScoper528afa732cbd\PhpParser\Node\Expr\MethodCall($methodCall, $method);
        if ($yaml === \false) {
            $methodCall->args[] = new \_PhpScoper528afa732cbd\PhpParser\Node\Arg($this->commonNodeFactory->createFalse());
        }
        return $methodCall;
    }
