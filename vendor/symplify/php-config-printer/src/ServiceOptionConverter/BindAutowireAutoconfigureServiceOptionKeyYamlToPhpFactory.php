...
final class BindAutowireAutoconfigureServiceOptionKeyYamlToPhpFactory implements \Symplify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \_PhpScopere106f9fd4493\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScopere106f9fd4493\PhpParser\Node\Expr\MethodCall
    {
        ...
        $methodCall = new \_PhpScopere106f9fd4493\PhpParser\Node\Expr\MethodCall($methodCall, $method);
        if ($yaml === \false) {
            $methodCall->args[] = new \_PhpScopere106f9fd4493\PhpParser\Node\Arg($this->commonNodeFactory->createFalse());
        }
        return $methodCall;
    }
