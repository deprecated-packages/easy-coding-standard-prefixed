...
use _PhpScoperef5048aa2573\Rector\NodeTypeResolver\Node\AttributeKey;
...
final class StringExprResolver
{
    ...
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \_PhpScoperef5048aa2573\PhpParser\Node\Expr
    {
        if ($value === '') {
        ...
        if (\_PhpScoperef5048aa2573\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            ...
            return new \_PhpScoperef5048aa2573\PhpParser\Node\Expr\FuncCall(new \_PhpScoperef5048aa2573\PhpParser\Node\Name\FullyQualified(\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\_PhpScoperef5048aa2573\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    private function keepNewline(string $value) : \_PhpScoperef5048aa2573\PhpParser\Node\Scalar\String_
    {
        $string = new \_PhpScoperef5048aa2573\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\_PhpScoperef5048aa2573\Rector\NodeTypeResolver\Node\AttributeKey::KIND, \_PhpScoperef5048aa2573\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \_PhpScoperef5048aa2573\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \_PhpScoperef5048aa2573\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
