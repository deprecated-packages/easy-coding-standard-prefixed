...
use _PhpScoper4f42ead57614\PhpParser\Node\Expr\ClassConstFetch;
...
use _PhpScoper4f42ead57614\Rector\NodeTypeResolver\Node\AttributeKey;
...
final class StringExprResolver
{
    ...
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr
    {
        if ($value === '') {
        ...
        if (\_PhpScoper4f42ead57614\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            ...
            return new \_PhpScoper4f42ead57614\PhpParser\Node\Expr\FuncCall(new \_PhpScoper4f42ead57614\PhpParser\Node\Name\FullyQualified(\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\_PhpScoper4f42ead57614\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    private function keepNewline(string $value) : \_PhpScoper4f42ead57614\PhpParser\Node\Scalar\String_
    {
        $string = new \_PhpScoper4f42ead57614\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\_PhpScoper4f42ead57614\Rector\NodeTypeResolver\Node\AttributeKey::KIND, \_PhpScoper4f42ead57614\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \_PhpScoper4f42ead57614\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
     * @return String_|ClassConstFetch
     */
    private function resolveClassType(bool $skipClassesToConstantReference, string $value) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr
    {
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \_PhpScoper4f42ead57614\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
