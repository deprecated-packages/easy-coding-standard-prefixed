...
use _PhpScoper0ba97041430d\PhpParser\Node\Expr\ClassConstFetch;
...
use _PhpScoper0ba97041430d\Rector\NodeTypeResolver\Node\AttributeKey;
...
final class StringExprResolver
{
    ...
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \_PhpScoper0ba97041430d\PhpParser\Node\Expr
    {
        if ($value === '') {
        ...
        if (\_PhpScoper0ba97041430d\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            ...
            return new \_PhpScoper0ba97041430d\PhpParser\Node\Expr\FuncCall(new \_PhpScoper0ba97041430d\PhpParser\Node\Name\FullyQualified(\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\_PhpScoper0ba97041430d\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    private function keepNewline(string $value) : \_PhpScoper0ba97041430d\PhpParser\Node\Scalar\String_
    {
        $string = new \_PhpScoper0ba97041430d\PhpParser\Node\Scalar\String_($value);
        $string->setAttribute(\_PhpScoper0ba97041430d\Rector\NodeTypeResolver\Node\AttributeKey::KIND, \_PhpScoper0ba97041430d\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
        return $string;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \_PhpScoper0ba97041430d\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
     * @return String_|ClassConstFetch
     */
    private function resolveClassType(bool $skipClassesToConstantReference, string $value) : \_PhpScoper0ba97041430d\PhpParser\Node\Expr
    {
    ...
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \_PhpScoper0ba97041430d\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
