...
final class JsonInliner
{
    ...
    public function inlineSections(string $jsonContent) : string
    {
        ...
        foreach ($inlineSections as $inlineSection) {
            ...
            $jsonContent = \_PhpScopera189153e1f79\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \_PhpScopera189153e1f79\Nette\Utils\Strings::replace($match[2], self::SPACE_REGEX, ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
