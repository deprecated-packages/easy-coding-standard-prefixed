...
final class JsonInliner
{
    ...
    public function inlineSections(string $jsonContent) : string
    {
        ...
        foreach ($inlineSections as $inlineSection) {
            ...
            $jsonContent = \_PhpScoper2f75f00bf6fa\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \_PhpScoper2f75f00bf6fa\Nette\Utils\Strings::replace($match[2], self::SPACE_REGEX, ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
