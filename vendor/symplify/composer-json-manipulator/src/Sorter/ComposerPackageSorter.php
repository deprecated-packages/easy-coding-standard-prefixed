...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority(string $requirementName) : string
    {
        if ($this->isPlatformPackage($requirementName)) {
            return (string) \_PhpScoperb2e2c0c42e71\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
    ...
    {
        return (bool) \_PhpScoperb2e2c0c42e71\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
