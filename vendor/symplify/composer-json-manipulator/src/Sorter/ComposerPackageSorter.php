...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority(string $requirementName) : string
    {
        if ($this->isPlatformPackage($requirementName)) {
            return (string) \_PhpScoper6c2f17c43d2d\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
    ...
    {
        return (bool) \_PhpScoper6c2f17c43d2d\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
