...
use _PhpScoperaac5f7c652e4\Psr\Log\NullLogger;
 ...
 */
abstract class AbstractAdapter implements \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\AdapterInterface, \_PhpScoperaac5f7c652e4\Symfony\Contracts\Cache\CacheInterface, \_PhpScoperaac5f7c652e4\Psr\Log\LoggerAwareInterface, \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\ResettableInterface
{
    /**
     * @internal
     */
    ...
    {
        $this->namespace = '' === $namespace ? '' : \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\CacheItem::validateKey($namespace) . static::NS_SEPARATOR;
     ...
     */
    public static function createSystemCache($namespace, $defaultLifetime, $version, $directory, \_PhpScoperaac5f7c652e4\Psr\Log\LoggerInterface $logger = null)
    {
        $opcache = new \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\PhpFilesAdapter($namespace, $defaultLifetime, $directory, \true);
        if (null !== $logger) {
            $opcache->setLogger($logger);
        }
        if (!(self::$apcuSupported = self::$apcuSupported ?? \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\ApcuAdapter::isSupported())) {
            return $opcache;
        }
        $apcu = new \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\ApcuAdapter($namespace, (int) $defaultLifetime / 5, $version);
        if ('cli' === \PHP_SAPI && !\filter_var(\ini_get('apc.enable_cli'), \FILTER_VALIDATE_BOOLEAN)) {
            $apcu->setLogger(new \_PhpScoperaac5f7c652e4\Psr\Log\NullLogger());
        } elseif (null !== $logger) {
            $apcu->setLogger($logger);
        }
        return new \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\ChainAdapter([$apcu, $opcache]);
    }
    public static function createConnection($dsn, array $options = [])
    {
        if (!\is_string($dsn)) {
            throw new \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('The %s() method expect argument #1 to be string, %s given.', __METHOD__, \gettype($dsn)));
        }
        if (0 === \strpos($dsn, 'redis:') || 0 === \strpos($dsn, 'rediss:')) {
            return \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\RedisAdapter::createConnection($dsn, $options);
        }
        if (0 === \strpos($dsn, 'memcached:')) {
            return \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Adapter\MemcachedAdapter::createConnection($dsn, $options);
        }
        throw new \_PhpScoperaac5f7c652e4\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported DSN: %s.', $dsn));
    }
    /**
     * {@inheritdoc}
     *
