 ...
 */
class ArrayAdapter implements \_PhpScoper326af2119eba\Symfony\Component\Cache\Adapter\AdapterInterface, \_PhpScoper326af2119eba\Symfony\Contracts\Cache\CacheInterface, \_PhpScoper326af2119eba\Psr\Log\LoggerAwareInterface, \_PhpScoper326af2119eba\Symfony\Component\Cache\ResettableInterface
{
    ...
    {
        if (0 > $maxLifetime) {
            throw new \_PhpScoper326af2119eba\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxLifetime must be positive, %F passed.', $maxLifetime));
        }
        if (0 > $maxItems) {
            throw new \_PhpScoper326af2119eba\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxItems must be a positive integer, %d passed.', $maxItems));
        }
    ...
    private function unfreeze(string $key, bool &$isHit)
    {
        ...
        if (\is_string($value) && isset($value[2]) && ':' === $value[1]) {
            try {
                ...
                \_PhpScoper326af2119eba\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to unserialize key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
                $value = \false;
            }
            if (\false === $value) {
                $value = null;
