 ...
 */
class ChainAdapter implements \_PhpScoper653866602a9e\Symfony\Component\Cache\Adapter\AdapterInterface, CacheInterface, PruneableInterface, ResettableInterface
{
    use ContractsTrait;
    private $adapters = [];
    ...
    public function __construct(array $adapters, int $defaultLifetime = 0)
    {
        ...
        foreach ($adapters as $adapter) {
            ...
            if (\in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true) && $adapter instanceof \_PhpScoper653866602a9e\Symfony\Component\Cache\Adapter\ApcuAdapter && !\filter_var(\ini_get('apc.enable_cli'), \FILTER_VALIDATE_BOOLEAN)) {
                continue;
                // skip putting APCu in the chain when the backend is disabled
            }
            if ($adapter instanceof \_PhpScoper653866602a9e\Symfony\Component\Cache\Adapter\AdapterInterface) {
                $this->adapters[] = $adapter;
            } else {
                $this->adapters[] = new \_PhpScoper653866602a9e\Symfony\Component\Cache\Adapter\ProxyAdapter($adapter);
            }
        }
    ...
    public function clear(string $prefix = '')
    {
        ...
        $i = $this->adapterCount;
        while ($i--) {
            if ($this->adapters[$i] instanceof \_PhpScoper653866602a9e\Symfony\Component\Cache\Adapter\AdapterInterface) {
