...
use _PhpScoperf5f75c22067b\Predis\Connection\Aggregate\PredisCluster;
...
use _PhpScoperf5f75c22067b\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use _PhpScoperf5f75c22067b\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use _PhpScoperf5f75c22067b\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use _PhpScoperf5f75c22067b\Symfony\Component\Cache\Traits\RedisTrait;
/**
 ...
 */
class RedisTagAwareAdapter extends \_PhpScoperf5f75c22067b\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    use RedisTrait;
    /**
     ...
     */
    public function __construct($redisClient, string $namespace = '', int $defaultLifetime = 0, \_PhpScoperf5f75c22067b\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if ($redisClient instanceof \_PhpScoperf5f75c22067b\Predis\ClientInterface && $redisClient->getConnection() instanceof \_PhpScoperf5f75c22067b\Predis\Connection\Aggregate\ClusterInterface && !$redisClient->getConnection() instanceof \_PhpScoperf5f75c22067b\Predis\Connection\Aggregate\PredisCluster) {
            throw new \_PhpScoperf5f75c22067b\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \_PhpScoperf5f75c22067b\Predis\Connection\Aggregate\PredisCluster::class, \get_class($redisClient->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redisClient instanceof \Redis || $redisClient instanceof \RedisArray || $redisClient instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \_PhpScoperf5f75c22067b\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', \get_class($this), \_PhpScoperf5f75c22067b\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redisClient, $namespace, $defaultLifetime, new \_PhpScoperf5f75c22067b\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
     */
    protected function doSave(array $values, ?int $lifetime, array $addTagData = [], array $delTagData = []) : array
    {
        ...
        if ('noeviction' !== $eviction && 0 !== \strpos($eviction, 'volatile-')) {
            \_PhpScoperf5f75c22067b\Symfony\Component\Cache\CacheItem::log($this->logger, \sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or  "volatile-*" eviction policies', $eviction));
            return \false;
        }
        // serialize values
...
EOLUA;
    ...
    {
        if (!$this->redis instanceof \_PhpScoperf5f75c22067b\Predis\ClientInterface || !$this->redis->getConnection() instanceof \_PhpScoperf5f75c22067b\Predis\Connection\Aggregate\PredisCluster) {
