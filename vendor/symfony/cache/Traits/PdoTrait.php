...
use _PhpScoperf053e888b664\Doctrine\DBAL\DBALException;
use _PhpScoperf053e888b664\Doctrine\DBAL\Driver\ServerInfoAwareConnection;
use _PhpScoperf053e888b664\Doctrine\DBAL\DriverManager;
use _PhpScoperf053e888b664\Doctrine\DBAL\Exception\TableNotFoundException;
use _PhpScoperf053e888b664\Doctrine\DBAL\Schema\Schema;
...
trait PdoTrait
{
    ...
    private $namespace;
    private function init($connOrDsn, string $namespace, int $defaultLifetime, array $options, ?\_PhpScoperf053e888b664\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller)
    {
        if (isset($namespace[0]) && \preg_match('#[^-+.A-Za-z0-9]#', $namespace, $match)) {
            throw new \_PhpScoperf053e888b664\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Namespace contains "%s" but only characters in [-+.A-Za-z0-9] are allowed.', $match[0]));
        }
        if ($connOrDsn instanceof \PDO) {
            if (\PDO::ERRMODE_EXCEPTION !== $connOrDsn->getAttribute(\PDO::ATTR_ERRMODE)) {
                throw new \_PhpScoperf053e888b664\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO error mode attribute be set to throw Exceptions (i.e. $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION))', __CLASS__));
            }
            $this->conn = $connOrDsn;
        } elseif ($connOrDsn instanceof \_PhpScoperf053e888b664\Doctrine\DBAL\Connection) {
            $this->conn = $connOrDsn;
        } elseif (\is_string($connOrDsn)) {
            $this->dsn = $connOrDsn;
        } else {
            throw new \_PhpScoperf053e888b664\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" requires PDO or Doctrine\\DBAL\\Connection instance or DSN string as first argument, "%s" given.', __CLASS__, \is_object($connOrDsn) ? \get_class($connOrDsn) : \gettype($connOrDsn)));
        }
    ...
    public function createTable()
    {
        ...
        if ($conn instanceof \_PhpScoperf053e888b664\Doctrine\DBAL\Connection) {
            $types = ['mysql' => 'binary', 'sqlite' => 'text', 'pgsql' => 'string', 'oci' => 'string', 'sqlsrv' => 'string'];
            if (!isset($types[$this->driver])) {
                throw new \DomainException(\sprintf('Creating the cache table is currently not implemented for PDO driver "%s".', $this->driver));
            }
            $schema = new \_PhpScoperf053e888b664\Doctrine\DBAL\Schema\Schema();
    ...
    public function prune()
    {
        ...
        } catch (\_PhpScoperf053e888b664\Doctrine\DBAL\Exception\TableNotFoundException $e) {
            return \true;
    ...
    protected function doSave(array $values, $lifetime)
    {
        ...
        foreach ($values as $id => $data) {
            ...
            if (null === $driver && !$stmt->rowCount()) {
                try {
                    $insertStmt->execute();
                } catch (\_PhpScoperf053e888b664\Doctrine\DBAL\DBALException $e) {
                } catch (\PDOException $e) {
        ...
        if (null === $this->conn) {
            if (\strpos($this->dsn, '://')) {
                if (!\class_exists(\_PhpScoperf053e888b664\Doctrine\DBAL\DriverManager::class)) {
                    throw new \_PhpScoperf053e888b664\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Failed to parse the DSN "%s". Try running "composer require doctrine/dbal".', $this->dsn));
                }
                $this->conn = \_PhpScoperf053e888b664\Doctrine\DBAL\DriverManager::getConnection(['url' => $this->dsn]);
            } else {
    ...
    private function getServerVersion() : string
    {
        if (null === $this->serverVersion) {
            ...
            } elseif ($conn instanceof \_PhpScoperf053e888b664\Doctrine\DBAL\Driver\ServerInfoAwareConnection) {
