...
use _PhpScoper1ddca3f92d7d\Predis\Command\Redis\UNLINK;
use _PhpScoper1ddca3f92d7d\Predis\Connection\Aggregate\ClusterInterface;
use _PhpScoper1ddca3f92d7d\Predis\Connection\Aggregate\RedisCluster;
use _PhpScoper1ddca3f92d7d\Predis\Connection\Aggregate\ReplicationInterface;
use _PhpScoper1ddca3f92d7d\Predis\Response\Status;
...
trait RedisTrait
{
    ...
    private function init($redisClient, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redisClient instanceof \Redis && !$redisClient instanceof \RedisArray && !$redisClient instanceof \RedisCluster && !$redisClient instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface && !$redisClient instanceof \_PhpScoper1ddca3f92d7d\Symfony\Component\Cache\Traits\RedisProxy && !$redisClient instanceof \_PhpScoper1ddca3f92d7d\Symfony\Component\Cache\Traits\RedisClusterProxy) {
        ...
        if ($redisClient instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface && $redisClient->getOptions()->exceptions) {
    ...
    public static function createConnection($dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\_PhpScoper1ddca3f92d7d\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\_PhpScoper1ddca3f92d7d\Predis\Client::class)) {
        ...
        if (null === $params['class'] && !isset($params['redis_sentinel']) && \extension_loaded('redis')) {
            ...
            $class = null === $params['class'] ? \_PhpScoper1ddca3f92d7d\Predis\Client::class : $params['class'];
        }
        if (\is_a($class, \Redis::class, \true)) {
            ...
            if ($params['lazy']) {
                $redis = new \_PhpScoper1ddca3f92d7d\Symfony\Component\Cache\Traits\RedisProxy($redis, $initializer);
            } else {
                $initializer($redis);
            }
            ...
            $redis = $params['lazy'] ? new \_PhpScoper1ddca3f92d7d\Symfony\Component\Cache\Traits\RedisClusterProxy($initializer) : $initializer();
        } elseif (\is_a($class, \_PhpScoper1ddca3f92d7d\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    {
        $cleared = \true;
        if ($this->redis instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \_PhpScoper1ddca3f92d7d\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $namespace . '*', 'COUNT', 1000) : $host->scan($cursor, $namespace . '*', 1000);
    ...
    private function pipeline(\Closure $generator, $redis = null) : \Generator
    {
        ...
        if ($redis instanceof \_PhpScoper1ddca3f92d7d\Symfony\Component\Cache\Traits\RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface && $redis->getConnection() instanceof RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface) {
    ...
    private function getHosts() : array
    {
        ...
        if ($this->redis instanceof \_PhpScoper1ddca3f92d7d\Predis\ClientInterface) {
            ...
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \_PhpScoper1ddca3f92d7d\Predis\Client($c);
                }
            }
        ...
        } elseif ($this->redis instanceof \_PhpScoper1ddca3f92d7d\Symfony\Component\Cache\Traits\RedisClusterProxy || $this->redis instanceof \RedisCluster) {
            $hosts = [];
