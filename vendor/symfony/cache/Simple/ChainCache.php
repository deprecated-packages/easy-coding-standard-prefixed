 ...
 */
class ChainCache implements \_PhpScoperb458b528613f\Psr\SimpleCache\CacheInterface, \_PhpScoperb458b528613f\Symfony\Component\Cache\PruneableInterface, \_PhpScoperb458b528613f\Symfony\Component\Cache\ResettableInterface
{
    private $miss;
    private $caches = [];
    ...
    {
        if (!$caches) {
            throw new \_PhpScoperb458b528613f\Symfony\Component\Cache\Exception\InvalidArgumentException('At least one cache must be specified.');
        }
        foreach ($caches as $cache) {
            if (!$cache instanceof \_PhpScoperb458b528613f\Psr\SimpleCache\CacheInterface) {
                throw new \_PhpScoperb458b528613f\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('The class "%s" does not implement the "%s" interface.', \get_class($cache), \_PhpScoperb458b528613f\Psr\SimpleCache\CacheInterface::class));
            }
        }
    ...
    public function prune()
    {
        $pruned = \true;
        foreach ($this->caches as $cache) {
            if ($cache instanceof \_PhpScoperb458b528613f\Symfony\Component\Cache\PruneableInterface) {
    ...
    {
        foreach ($this->caches as $cache) {
            if ($cache instanceof \_PhpScoperb458b528613f\Symfony\Contracts\Service\ResetInterface) {
                $cache->reset();
            }
        }
    }
