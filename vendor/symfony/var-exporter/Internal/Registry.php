...
use _PhpScoperbc5235eb86f3\Symfony\Component\VarExporter\Exception\ClassNotFoundException;
...
class Registry
{
    ...
    public static function getClassReflector($class, $instantiableWithoutConstructor = \false, $cloneable = null)
    {
        if (!\class_exists($class) && !\interface_exists($class, \false) && !\trait_exists($class, \false)) {
            throw new \_PhpScoperbc5235eb86f3\Symfony\Component\VarExporter\Exception\ClassNotFoundException($class);
        }
        $reflector = new \ReflectionClass($class);
        if ($instantiableWithoutConstructor) {
            $proto = $reflector->newInstanceWithoutConstructor();
        } elseif (!$reflector->isInstantiable()) {
            throw new \_PhpScoperbc5235eb86f3\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException($class);
            ...
            if (null !== $proto && !$proto instanceof \Throwable) {
                try {
                    ...
                    throw new \_PhpScoperbc5235eb86f3\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException($class, $e);
                }
            }
        }
        if (null === $cloneable) {
