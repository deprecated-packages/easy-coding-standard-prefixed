...
use _PhpScoper3a22e8e0bd94\Symfony\Component\HttpFoundation\Exception\BadRequestException;
/**
 * InputBag is a container for user input values such as $_GET, $_POST, $_REQUEST, and $_COOKIE.
 *
 * @author Saif Eddin Gmati <saif.gmati@symfony.com>
 */
final class InputBag extends \_PhpScoper3a22e8e0bd94\Symfony\Component\HttpFoundation\ParameterBag
{
    /**
    ...
    public function get(string $key, $default = null)
    {
        ...
        if (null !== $value && $this !== $value && !\is_scalar($value) && !(\is_object($value) && \method_exists($value, '__toString'))) {
            trigger_deprecation('symfony/http-foundation', '5.1', 'Retrieving a non-string value from "%s()" is deprecated, and will throw a "%s" exception in Symfony 6.0, use "%s::all()" instead.', __METHOD__, \_PhpScoper3a22e8e0bd94\Symfony\Component\HttpFoundation\Exception\BadRequestException::class, __CLASS__);
        }
    ...
    public function all(string $key = null) : array
    {
        ...
        if (!\is_array($value)) {
            throw new \_PhpScoper3a22e8e0bd94\Symfony\Component\HttpFoundation\Exception\BadRequestException(\sprintf('Unexpected value for "%s" input, expecting "array", got "%s".', $key, \get_debug_type($value)));
        }
        return $value;
    }
    /**
