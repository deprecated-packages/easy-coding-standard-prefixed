 ...
 */
class ResolveBindingsPass extends \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process(\_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        try {
            ...
            foreach ($this->unusedBindings as list($key, $serviceId, $bindingType, $file)) {
                ...
                if (\_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Argument\BoundArgument::DEFAULTS_BINDING === $bindingType) {
                    $message .= 'under "_defaults"';
                } elseif (\_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Argument\BoundArgument::INSTANCEOF_BINDING === $bindingType) {
    ...
    {
        if ($value instanceof \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\TypedReference && $value->getType() === (string) $value) {
            // Already checked
        ...
        if (!$value instanceof \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Definition || !($bindings = $value->getBindings())) {
            return parent::processValue($value, $isRoot);
        }
        $bindingNames = [];
        foreach ($bindings as $key => $binding) {
            ...
            if (null !== $bindingValue && !$bindingValue instanceof \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Reference && !$bindingValue instanceof \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Definition && !$bindingValue instanceof \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument && !$bindingValue instanceof \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                throw new \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value for binding key "%s" for service "%s": expected null, "%s", "%s", "%s" or ServiceLocatorArgument, "%s" given.', $key, $this->currentId, \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Reference::class, \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Definition::class, \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument::class, \get_debug_type($bindingValue)));
            }
        }
        if ($value->isAbstract()) {
            return parent::processValue($value, $isRoot);
        ...
        foreach ($calls as $i => $call) {
            ...
            foreach ($reflectionMethod->getParameters() as $key => $parameter) {
                ...
                $typeHint = \_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter);
     ...
     * @return mixed
     */
    private function getBindingValue(\_PhpScoperc75fd40d7a6e\Symfony\Component\DependencyInjection\Argument\BoundArgument $binding)
    {
