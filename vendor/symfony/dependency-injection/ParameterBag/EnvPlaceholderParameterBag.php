 ...
 */
class EnvPlaceholderParameterBag extends \_PhpScoper3fa05b4669af\Symfony\Component\DependencyInjection\ParameterBag\ParameterBag
{
    ...
    public function get($name)
    {
        if (0 === \strpos($name, 'env(') && ')' === \substr($name, -1) && 'env()' !== $name) {
            ...
            if (!\preg_match('/^(?:\\w*+:)*+\\w++$/', $env)) {
                throw new \_PhpScoper3fa05b4669af\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid %s name: only "word" characters are allowed.', $name));
            }
            if ($this->has($name)) {
                ...
                if (null !== $defaultValue && !\is_scalar($defaultValue)) {
                    ...
                    throw new \_PhpScoper3fa05b4669af\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('The default value of an env() parameter must be scalar or null, but "%s" given to "%s".', \gettype($defaultValue), $name));
    ...
    public function resolve()
    {
        ...
        foreach ($this->envPlaceholders as $env => $placeholders) {
            ...
            if (\is_numeric($default = $this->parameters[$name])) {
                ...
                throw new \_PhpScoper3fa05b4669af\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('The default value of env parameter "%s" must be scalar or null, %s given.', $env, \gettype($default)));
