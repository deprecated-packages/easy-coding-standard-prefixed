...
use _PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException;
/**
...
class Definition
{
    ...
    public function setFactory($factory)
    {
        ...
        } elseif ($factory instanceof \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Reference) {
    ...
    public function setDecoratedService($id, $renamedId = null, $priority = 0)
    {
        if ($renamedId && $id === $renamedId) {
            throw new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The decorated service inner name for "%s" must be different than the service name itself.', $id));
        }
        $invalidBehavior = 3 < \func_num_args() ? (int) \func_get_arg(3) : \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE;
        $this->changes['decorated_service'] = \true;
        if (null === $id) {
            $this->decoratedService = null;
        } else {
            $this->decoratedService = [$id, $renamedId, (int) $priority];
            if (\_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE !== $invalidBehavior) {
    ...
    {
        if ($class instanceof \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Parameter) {
            @\trigger_error(\sprintf('Passing an instance of %s as class name to %s in deprecated in Symfony 4.4 and will result in a TypeError in 5.0. Please pass the string "%%%s%%" instead.', \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Parameter::class, __CLASS__, (string) $class), \E_USER_DEPRECATED);
        }
    ...
    public function replaceArgument($index, $argument)
    {
        if (0 === \count($this->arguments)) {
            throw new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException('Cannot replace arguments if none have been configured yet.');
        }
        if (\is_int($index) && ($index < 0 || $index > \count($this->arguments) - 1)) {
            throw new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The index "%d" is not in the range [0, %d].', $index, \count($this->arguments) - 1));
        }
        if (!\array_key_exists($index, $this->arguments)) {
            throw new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The argument "%s" doesn\'t exist.', $index));
        }
    ...
    {
        if (empty($method)) {
            throw new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Method name cannot be empty.');
        }
    ...
    public function setDeprecated($status = \true, $template = null)
    {
        if (null !== $template) {
            ...
            if (\false === \strpos($template, '%service_id%')) {
                throw new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%service_id%" placeholder.');
            }
    ...
    public function setConfigurator($configurator)
    {
        ...
        } elseif ($configurator instanceof \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Reference) {
    ...
    public function setBindings(array $bindings)
    {
        foreach ($bindings as $key => $binding) {
            ...
            if (!$binding instanceof \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Argument\BoundArgument) {
                $bindings[$key] = new \_PhpScoperecb978830f1e\Symfony\Component\DependencyInjection\Argument\BoundArgument($binding);
            }
        }
