 ...
 */
class StopwatchTest extends \_PhpScopere5e7dca8c031\PHPUnit_Framework_TestCase
{
    const DELTA = 20;
    public function testStart()
    {
        $stopwatch = new \_PhpScopere5e7dca8c031\Symfony\Component\Stopwatch\Stopwatch();
        $event = $stopwatch->start('foo', 'cat');
        $this->assertInstanceOf('_PhpScopere5e7dca8c031\\Symfony\\Component\\Stopwatch\\StopwatchEvent', $event);
    ...
    {
        $stopwatch = new \_PhpScopere5e7dca8c031\Symfony\Component\Stopwatch\Stopwatch();
        $sections = new \ReflectionProperty('_PhpScopere5e7dca8c031\\Symfony\\Component\\Stopwatch\\Stopwatch', 'sections');
        $sections->setAccessible(\true);
        $section = $sections->getValue($stopwatch);
        $events = new \ReflectionProperty('_PhpScopere5e7dca8c031\\Symfony\\Component\\Stopwatch\\Section', 'events');
        $events->setAccessible(\true);
        $stopwatchMockEvent = $this->getMockBuilder('_PhpScopere5e7dca8c031\\Symfony\\Component\\Stopwatch\\StopwatchEvent')->setConstructorArgs(array(\microtime(\true) * 1000))->getMock();
