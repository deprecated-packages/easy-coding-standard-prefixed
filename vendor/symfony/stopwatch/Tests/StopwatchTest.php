 ...
 */
class StopwatchTest extends \_PhpScoper61bdb485188f\PHPUnit_Framework_TestCase
{
    const DELTA = 20;
    public function testStart()
    {
        $stopwatch = new \_PhpScoper61bdb485188f\Symfony\Component\Stopwatch\Stopwatch();
        $event = $stopwatch->start('foo', 'cat');
        $this->assertInstanceOf('_PhpScoper61bdb485188f\\Symfony\\Component\\Stopwatch\\StopwatchEvent', $event);
    ...
    {
        $stopwatch = new \_PhpScoper61bdb485188f\Symfony\Component\Stopwatch\Stopwatch();
        $sections = new \ReflectionProperty('_PhpScoper61bdb485188f\\Symfony\\Component\\Stopwatch\\Stopwatch', 'sections');
        $sections->setAccessible(\true);
        $section = $sections->getValue($stopwatch);
        $events = new \ReflectionProperty('_PhpScoper61bdb485188f\\Symfony\\Component\\Stopwatch\\Section', 'events');
        $events->setAccessible(\true);
        $stopwatchMockEvent = $this->getMockBuilder('_PhpScoper61bdb485188f\\Symfony\\Component\\Stopwatch\\StopwatchEvent')->setConstructorArgs(array(\microtime(\true) * 1000))->getMock();
