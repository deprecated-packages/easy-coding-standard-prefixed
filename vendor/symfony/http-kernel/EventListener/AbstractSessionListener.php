...
use _PhpScopera749ac204cd2\Psr\Container\ContainerInterface;
use _PhpScopera749ac204cd2\Symfony\Component\EventDispatcher\EventSubscriberInterface;
use _PhpScopera749ac204cd2\Symfony\Component\HttpFoundation\Session\Session;
use _PhpScopera749ac204cd2\Symfony\Component\HttpFoundation\Session\SessionInterface;
use _PhpScopera749ac204cd2\Symfony\Component\HttpKernel\Event\FilterResponseEvent;
 ...
 */
abstract class AbstractSessionListener implements \_PhpScopera749ac204cd2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    public function __construct(\_PhpScopera749ac204cd2\Psr\Container\ContainerInterface $container = null)
    {
    ...
    public function onKernelRequest(\_PhpScopera749ac204cd2\Symfony\Component\HttpKernel\Event\GetResponseEvent $event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \_PhpScopera749ac204cd2\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    public function onKernelResponse(\_PhpScopera749ac204cd2\Symfony\Component\HttpKernel\Event\FilterResponseEvent $event)
    {
        if (!$event->isMasterRequest()) {
            return;
        }
        ...
        if ($session instanceof \_PhpScopera749ac204cd2\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() !== \end($this->sessionUsageStack) : $session->isStarted()) {
            if ($autoCacheControl) {
     ...
     * @internal
     */
    public function onFinishRequest(\_PhpScopera749ac204cd2\Symfony\Component\HttpKernel\Event\FinishRequestEvent $event)
    {
        if ($event->isMasterRequest()) {
    ...
    {
        return [
            \_PhpScopera749ac204cd2\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \_PhpScopera749ac204cd2\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \_PhpScopera749ac204cd2\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**
