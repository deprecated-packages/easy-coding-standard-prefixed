...
use _PhpScoperfcee700af3df\Symfony\Component\HttpFoundation\Session\Session;
use _PhpScoperfcee700af3df\Symfony\Component\HttpFoundation\Session\SessionInterface;
use _PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Event\FinishRequestEvent;
use _PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Event\RequestEvent;
use _PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Event\ResponseEvent;
use _PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException;
 ...
 * @internal
 */
abstract class AbstractSessionListener implements \_PhpScoperfcee700af3df\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    public const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    private $debug;
    public function __construct(\_PhpScoperfcee700af3df\Psr\Container\ContainerInterface $container = null, bool $debug = \false)
    {
        $this->container = $container;
        $this->debug = $debug;
    }
    public function onKernelRequest(\_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Event\RequestEvent $event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \_PhpScoperfcee700af3df\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    public function onKernelResponse(\_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Event\ResponseEvent $event)
    {
        ...
        if ($session instanceof \_PhpScoperfcee700af3df\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() === \end($this->sessionUsageStack) : !$session->isStarted()) {
            return;
        }
        if ($autoCacheControl) {
        ...
        if ($this->debug) {
            throw new \_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException('Session was used while the request was declared stateless.');
        }
        if ($this->container->has('logger')) {
            $this->container->get('logger')->warning('Session was used while the request was declared stateless.');
        }
    }
    public function onFinishRequest(\_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\Event\FinishRequestEvent $event)
    {
        if ($event->isMasterRequest()) {
    ...
    {
        return [
            \_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \_PhpScoperfcee700af3df\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**
