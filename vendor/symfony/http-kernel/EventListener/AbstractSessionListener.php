...
use _PhpScoperaba240c3d5f1\Psr\Container\ContainerInterface;
use _PhpScoperaba240c3d5f1\Symfony\Component\EventDispatcher\EventSubscriberInterface;
use _PhpScoperaba240c3d5f1\Symfony\Component\HttpFoundation\Session\Session;
 ...
 */
abstract class AbstractSessionListener implements \_PhpScoperaba240c3d5f1\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    public function __construct(\_PhpScoperaba240c3d5f1\Psr\Container\ContainerInterface $container = null)
    {
    ...
    public function onKernelRequest(\_PhpScoperaba240c3d5f1\Symfony\Component\HttpKernel\Event\GetResponseEvent $event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \_PhpScoperaba240c3d5f1\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    public function onKernelResponse(\_PhpScoperaba240c3d5f1\Symfony\Component\HttpKernel\Event\FilterResponseEvent $event)
    {
        ...
        if ($session instanceof \_PhpScoperaba240c3d5f1\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() !== \end($this->sessionUsageStack) : $session->isStarted()) {
            if ($autoCacheControl) {
     ...
     * @internal
     */
    public function onFinishRequest(\_PhpScoperaba240c3d5f1\Symfony\Component\HttpKernel\Event\FinishRequestEvent $event)
    {
        if ($event->isMasterRequest()) {
    ...
    {
        return [
            \_PhpScoperaba240c3d5f1\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \_PhpScoperaba240c3d5f1\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \_PhpScoperaba240c3d5f1\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**
