...
use _PhpScoperb6361033cf41\Symfony\Component\VarDumper\Cloner\Data;
...
use _PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\DataDumperInterface;
use _PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\HtmlDumper;
use _PhpScoperb6361033cf41\Symfony\Component\VarDumper\Server\Connection;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 *
 * @final
 */
class DumpDataCollector extends \_PhpScoperb6361033cf41\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
     ...
     */
    public function __construct(\_PhpScoperb6361033cf41\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, string $charset = null, \_PhpScoperb6361033cf41\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    public function __clone()
    {
        $this->clonesIndex = ++$this->clonesCount;
    }
    public function dump(\_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Cloner\Data $data)
    {
        if ($this->stopwatch) {
            $this->stopwatch->start('dump');
        }
        ['name' => $name, 'file' => $file, 'line' => $line, 'file_excerpt' => $fileExcerpt] = $this->sourceContextProvider->getContext();
        if ($this->dumper instanceof \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect(\_PhpScoperb6361033cf41\Symfony\Component\HttpFoundation\Request $request, \_PhpScoperb6361033cf41\Symfony\Component\HttpFoundation\Response $response, \Throwable $exception = null)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $dumper = new \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function __wakeup()
    {
        ...
        self::__construct($this->stopwatch, \is_string($fileLinkFormat) || $fileLinkFormat instanceof \_PhpScoperb6361033cf41\Symfony\Component\HttpKernel\Debug\FileLinkFormatter ? $fileLinkFormat : null, \is_string($charset) ? $charset : null);
    }
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1) : array
    {
        ...
        if ('html' === $format) {
            $dumper = new \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    private function doDump(\_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper, \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Cloner\Data $data, string $name, string $file, int $line)
    {
        if ($dumper instanceof \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \_PhpScoperb6361033cf41\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
