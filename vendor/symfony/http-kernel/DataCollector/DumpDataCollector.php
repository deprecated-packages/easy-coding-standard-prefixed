...
use _PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider;
use _PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\DataDumperInterface;
use _PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\HtmlDumper;
use _PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Server\Connection;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 *
 * @final since Symfony 4.3
 */
class DumpDataCollector extends \_PhpScoperfd70a7e8e84f\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
     ...
     */
    public function __construct(\_PhpScoperfd70a7e8e84f\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, string $charset = null, \_PhpScoperfd70a7e8e84f\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    public function __clone()
    {
        $this->clonesIndex = ++$this->clonesCount;
    }
    public function dump(\_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Cloner\Data $data)
    {
        if ($this->stopwatch) {
            $this->stopwatch->start('dump');
        }
        list('name' => $name, 'file' => $file, 'line' => $line, 'file_excerpt' => $fileExcerpt) = $this->sourceContextProvider->getContext();
        if ($this->dumper instanceof \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect(\_PhpScoperfd70a7e8e84f\Symfony\Component\HttpFoundation\Request $request, \_PhpScoperfd70a7e8e84f\Symfony\Component\HttpFoundation\Response $response)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $dumper = new \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1)
    {
        ...
        if ('html' === $format) {
            $dumper = new \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    private function doDump(\_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper, $data, string $name, string $file, int $line)
    {
        if ($dumper instanceof \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \_PhpScoperfd70a7e8e84f\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
