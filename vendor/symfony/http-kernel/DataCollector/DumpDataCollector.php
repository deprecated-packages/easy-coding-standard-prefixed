...
use _PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider;
 ...
 */
class DumpDataCollector extends \_PhpScoperc95ae4bf942a\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
     ...
     */
    public function __construct(\_PhpScoperc95ae4bf942a\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, string $charset = null, \_PhpScoperc95ae4bf942a\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    public function __clone()
    {
        $this->clonesIndex = ++$this->clonesCount;
    }
    public function dump(\_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Cloner\Data $data)
    {
        if ($this->stopwatch) {
            $this->stopwatch->start('dump');
        }
        list('name' => $name, 'file' => $file, 'line' => $line, 'file_excerpt' => $fileExcerpt) = $this->sourceContextProvider->getContext();
        if ($this->dumper instanceof \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect(\_PhpScoperc95ae4bf942a\Symfony\Component\HttpFoundation\Request $request, \_PhpScoperc95ae4bf942a\Symfony\Component\HttpFoundation\Response $response)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $dumper = new \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1)
    {
        ...
        if ('html' === $format) {
            $dumper = new \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    private function doDump(\_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper, $data, string $name, string $file, int $line)
    {
        if ($dumper instanceof \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \_PhpScoperc95ae4bf942a\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
