...
use _PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\DataDumperInterface;
use _PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\HtmlDumper;
use _PhpScopercae980ebf12d\Symfony\Component\VarDumper\Server\Connection;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 *
 * @final since Symfony 4.3
 */
class DumpDataCollector extends \_PhpScopercae980ebf12d\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
     ...
     */
    public function __construct(\_PhpScopercae980ebf12d\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, string $charset = null, \_PhpScopercae980ebf12d\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    public function __clone()
    {
        $this->clonesIndex = ++$this->clonesCount;
    }
    public function dump(\_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Cloner\Data $data)
    {
        if ($this->stopwatch) {
            $this->stopwatch->start('dump');
        }
        list('name' => $name, 'file' => $file, 'line' => $line, 'file_excerpt' => $fileExcerpt) = $this->sourceContextProvider->getContext();
        if ($this->dumper instanceof \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect(\_PhpScopercae980ebf12d\Symfony\Component\HttpFoundation\Request $request, \_PhpScopercae980ebf12d\Symfony\Component\HttpFoundation\Response $response)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $dumper = new \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1)
    {
        ...
        if ('html' === $format) {
            $dumper = new \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    private function doDump(\_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper, $data, string $name, string $file, int $line)
    {
        if ($dumper instanceof \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \_PhpScopercae980ebf12d\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
