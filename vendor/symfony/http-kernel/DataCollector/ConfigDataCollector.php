...
use _PhpScoperc7096eb2567d\Symfony\Component\VarDumper\Caster\ClassStub;
/**
 * @author Fabien Potencier <fabien@symfony.com>
 *
 * @final
 */
class ConfigDataCollector extends \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    /**
     * @var KernelInterface
     */
     ...
     */
    public function setKernel(\_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\KernelInterface $kernel = null)
    {
        $this->kernel = $kernel;
    }
    /**
    ...
    {
        $this->data = ['token' => $response->headers->get('X-Debug-Token'), 'symfony_version' => \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::VERSION, 'symfony_state' => 'unknown', 'env' => isset($this->kernel) ? $this->kernel->getEnvironment() : 'n/a', 'debug' => isset($this->kernel) ? $this->kernel->isDebug() : 'n/a', 'php_version' => \PHP_VERSION, 'php_architecture' => \PHP_INT_SIZE * 8, 'php_intl_locale' => \class_exists(\Locale::class, \false) && \Locale::getDefault() ? \Locale::getDefault() : 'n/a', 'php_timezone' => \date_default_timezone_get(), 'xdebug_enabled' => \extension_loaded('xdebug'), 'apcu_enabled' => \extension_loaded('apcu') && \filter_var(\ini_get('apc.enabled'), \FILTER_VALIDATE_BOOLEAN), 'zend_opcache_enabled' => \extension_loaded('Zend OPcache') && \filter_var(\ini_get('opcache.enable'), \FILTER_VALIDATE_BOOLEAN), 'bundles' => [], 'sapi_name' => \PHP_SAPI];
        if (isset($this->kernel)) {
            foreach ($this->kernel->getBundles() as $name => $bundle) {
                $this->data['bundles'][$name] = new \_PhpScoperc7096eb2567d\Symfony\Component\VarDumper\Caster\ClassStub(\get_class($bundle));
            }
            $this->data['symfony_state'] = $this->determineSymfonyState();
            $this->data['symfony_minor_version'] = \sprintf('%s.%s', \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::MAJOR_VERSION, \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::MINOR_VERSION);
            $this->data['symfony_lts'] = 4 === \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::MINOR_VERSION;
            $eom = \DateTime::createFromFormat('d/m/Y', '01/' . \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::END_OF_MAINTENANCE);
            $eol = \DateTime::createFromFormat('d/m/Y', '01/' . \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::END_OF_LIFE);
    ...
    {
        $now = new \DateTime();
        $eom = \DateTime::createFromFormat('d/m/Y', '01/' . \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::END_OF_MAINTENANCE)->modify('last day of this month');
        $eol = \DateTime::createFromFormat('d/m/Y', '01/' . \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::END_OF_LIFE)->modify('last day of this month');
        if ($now > $eol) {
            $versionState = 'eol';
        } elseif ($now > $eom) {
            $versionState = 'eom';
        } elseif ('' !== \_PhpScoperc7096eb2567d\Symfony\Component\HttpKernel\Kernel::EXTRA_VERSION) {
            $versionState = 'dev';
        } else {
            $versionState = 'stable';
        }
