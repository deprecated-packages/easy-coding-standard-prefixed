...
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Error\OutOfMemoryError;
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\ClassNotFoundErrorEnhancer;
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\ErrorEnhancerInterface;
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\UndefinedFunctionErrorEnhancer;
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\UndefinedMethodErrorEnhancer;
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorRenderer\CliErrorRenderer;
use _PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorRenderer\HtmlErrorRenderer;
...
class ErrorHandler
{
    ...
    private $loggers = [\E_DEPRECATED => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::INFO], \E_USER_DEPRECATED => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::INFO], \E_NOTICE => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_USER_NOTICE => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_STRICT => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_WARNING => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_USER_WARNING => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_COMPILE_WARNING => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_CORE_WARNING => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::WARNING], \E_USER_ERROR => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::CRITICAL], \E_RECOVERABLE_ERROR => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::CRITICAL], \E_COMPILE_ERROR => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::CRITICAL], \E_PARSE => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::CRITICAL], \E_ERROR => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::CRITICAL], \E_CORE_ERROR => [null, \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::CRITICAL]];
    ...
    public function __construct(\_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\BufferingLogger $bootstrappingLogger = null, bool $debug = \false)
    {
     ...
     */
    public function setDefaultLogger(\_PhpScoper5e5ebcdbebdf\Psr\Log\LoggerInterface $logger, $levels = \E_ALL, bool $replace = \false) : void
    {
        $loggers = [];
        if (\is_array($levels)) {
    ...
    public function setLoggers(array $loggers) : array
    {
        ...
        foreach ($loggers as $type => $log) {
            ...
            } elseif ($log[0] instanceof \_PhpScoper5e5ebcdbebdf\Psr\Log\LoggerInterface) {
                $this->loggedErrors |= $type;
            } else {
        ...
        if ($flush) {
            foreach ($this->bootstrappingLogger->cleanLogs() as $log) {
                $type = \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ThrowableUtils::getSeverity($log[2]['exception']);
                if (!isset($flush[$type])) {
    ...
    public function handleError(int $type, string $message, string $file, int $line) : bool
    {
        ...
        if (null !== self::$toStringException) {
            ...
            if (!isset(self::$silencedErrorCache[$id = $file . ':' . $line])) {
                ...
                $errorAsException = new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($type, $file, $line, isset($lightTrace[1]) ? [$lightTrace[0]] : $lightTrace);
        ...
        if ($this->isRecursive) {
            ...
            try {
                ...
                $level = $type & $level ? $this->loggers[$type][1] : \_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::DEBUG;
    ...
    public function handleException(\Throwable $exception)
    {
        ...
        if (!$exception instanceof \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Error\FatalError) {
            self::$exitCode = 255;
            $type = \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ThrowableUtils::getSeverity($exception);
        } else {
        ...
        if (!$exception instanceof \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Error\OutOfMemoryError) {
    ...
    public static function handleFatalError(array $error = null) : void
    {
        ...
        if ($error && ($error['type'] &= \E_PARSE | \E_ERROR | \E_CORE_ERROR | \E_COMPILE_ERROR)) {
            ...
            if (0 === \strpos($error['message'], 'Allowed memory') || 0 === \strpos($error['message'], 'Out of memory')) {
                $fatalError = new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Error\OutOfMemoryError($handler->levels[$error['type']] . ': ' . $error['message'], 0, $error, 2, \false, $trace);
            } else {
                $fatalError = new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Error\FatalError($handler->levels[$error['type']] . ': ' . $error['message'], 0, $error, 2, \true, $trace);
            }
        } else {
            $fatalError = null;
        }
        ...
        } catch (\_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\Error\FatalError $e) {
    ...
    {
        $renderer = \in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true) ? new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorRenderer\CliErrorRenderer() : new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorRenderer\HtmlErrorRenderer($this->debug);
    ...
    {
        return [new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\UndefinedFunctionErrorEnhancer(), new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\UndefinedMethodErrorEnhancer(), new \_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorEnhancer\ClassNotFoundErrorEnhancer()];
    }
    /**
    ...
    private function cleanTrace(array $backtrace, int $type, string &$file, int &$line, bool $throw) : array
    {
        ...
        if (\class_exists(\_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\DebugClassLoader::class, \false)) {
            for ($i = \count($lightTrace) - 2; 0 < $i; --$i) {
                if (\_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\DebugClassLoader::class === ($lightTrace[$i]['class'] ?? null)) {
