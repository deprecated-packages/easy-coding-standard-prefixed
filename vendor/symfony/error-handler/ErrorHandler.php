...
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\Error\OutOfMemoryError;
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorEnhancer\ClassNotFoundErrorEnhancer;
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorEnhancer\ErrorEnhancerInterface;
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorEnhancer\UndefinedFunctionErrorEnhancer;
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorEnhancer\UndefinedMethodErrorEnhancer;
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorRenderer\CliErrorRenderer;
use _PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorRenderer\HtmlErrorRenderer;
...
class ErrorHandler
{
    ...
    public function __construct(\_PhpScoperb36402634947\Symfony\Component\ErrorHandler\BufferingLogger $bootstrappingLogger = null)
    {
     ...
     */
    public function setDefaultLogger(\_PhpScoperb36402634947\Psr\Log\LoggerInterface $logger, $levels = \E_ALL, bool $replace = \false) : void
    {
        $loggers = [];
        if (\is_array($levels)) {
    ...
    public function setLoggers(array $loggers) : array
    {
        ...
        if ($flush) {
            foreach ($this->bootstrappingLogger->cleanLogs() as $log) {
                $type = \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\ThrowableUtils::getSeverity($log[2]['exception']);
                if (!isset($flush[$type])) {
    ...
    public function handleError(int $type, string $message, string $file, int $line) : bool
    {
        ...
        if (null !== self::$toStringException) {
            ...
            if (!isset(self::$silencedErrorCache[$id = $file . ':' . $line])) {
                ...
                $errorAsException = new \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($type, $file, $line, isset($lightTrace[1]) ? [$lightTrace[0]] : $lightTrace);
    ...
    public function handleException(\Throwable $exception)
    {
        ...
        if (!$exception instanceof \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\Error\FatalError) {
            self::$exitCode = 255;
            $type = \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\ThrowableUtils::getSeverity($exception);
        } else {
        ...
        if (!$exception instanceof \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\Error\OutOfMemoryError) {
    ...
    public static function handleFatalError(array $error = null) : void
    {
        ...
        if ($error && ($error['type'] &= \E_PARSE | \E_ERROR | \E_CORE_ERROR | \E_COMPILE_ERROR)) {
            ...
            if (0 === \strpos($error['message'], 'Allowed memory') || 0 === \strpos($error['message'], 'Out of memory')) {
                $fatalError = new \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\Error\OutOfMemoryError($handler->levels[$error['type']] . ': ' . $error['message'], 0, $error, 2, \false, $trace);
            } else {
                $fatalError = new \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\Error\FatalError($handler->levels[$error['type']] . ': ' . $error['message'], 0, $error, 2, \true, $trace);
            }
        } else {
            $fatalError = null;
        }
        ...
        } catch (\_PhpScoperb36402634947\Symfony\Component\ErrorHandler\Error\FatalError $e) {
    ...
    {
        $renderer = \in_array(\PHP_SAPI, ['cli', 'phpdbg'], \true) ? new \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorRenderer\CliErrorRenderer() : new \_PhpScoperb36402634947\Symfony\Component\ErrorHandler\ErrorRenderer\HtmlErrorRenderer(0 !== $this->scopedErrors);
    ...
    private function cleanTrace(array $backtrace, int $type, string $file, int $line, bool $throw) : array
    {
        ...
        if (\class_exists(\_PhpScoperb36402634947\Symfony\Component\ErrorHandler\DebugClassLoader::class, \false)) {
            for ($i = \count($lightTrace) - 2; 0 < $i; --$i) {
                if (\_PhpScoperb36402634947\Symfony\Component\ErrorHandler\DebugClassLoader::class === ($lightTrace[$i]['class'] ?? null)) {
