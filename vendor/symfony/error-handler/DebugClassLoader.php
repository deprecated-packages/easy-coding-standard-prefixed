...
use _PhpScoper0c0702cca4ac\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use _PhpScoper0c0702cca4ac\Doctrine\Persistence\Proxy;
use _PhpScoper0c0702cca4ac\Mockery\MockInterface;
use _PhpScoper0c0702cca4ac\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use _PhpScoper0c0702cca4ac\PHPUnit\Framework\MockObject\MockObject;
use _PhpScoper0c0702cca4ac\Prophecy\Prophecy\ProphecySubjectInterface;
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\_PhpScoper0c0702cca4ac\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\_PhpScoper0c0702cca4ac\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \_PhpScoper0c0702cca4ac\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \_PhpScoper0c0702cca4ac\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScoper0c0702cca4ac\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScoper0c0702cca4ac\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScoper0c0702cca4ac\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScoper0c0702cca4ac\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \_PhpScoper0c0702cca4ac\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
