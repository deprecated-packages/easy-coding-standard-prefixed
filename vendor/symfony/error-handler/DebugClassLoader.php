...
use _PhpScoper5e5ebcdbebdf\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use _PhpScoper5e5ebcdbebdf\Doctrine\Persistence\Proxy;
use _PhpScoper5e5ebcdbebdf\Mockery\MockInterface;
use _PhpScoper5e5ebcdbebdf\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use _PhpScoper5e5ebcdbebdf\PHPUnit\Framework\MockObject\MockObject;
use _PhpScoper5e5ebcdbebdf\Prophecy\Prophecy\ProphecySubjectInterface;
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\_PhpScoper5e5ebcdbebdf\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\_PhpScoper5e5ebcdbebdf\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \_PhpScoper5e5ebcdbebdf\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \_PhpScoper5e5ebcdbebdf\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScoper5e5ebcdbebdf\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScoper5e5ebcdbebdf\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScoper5e5ebcdbebdf\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScoper5e5ebcdbebdf\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \_PhpScoper5e5ebcdbebdf\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
