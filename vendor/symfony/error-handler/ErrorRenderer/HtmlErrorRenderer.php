 ...
 */
class HtmlErrorRenderer implements \_PhpScopere341acab57d4\Symfony\Component\ErrorHandler\ErrorRenderer\ErrorRendererInterface
{
    ...
    private $logger;
    public function __construct(bool $debug = \false, string $charset = null, $fileLinkFormat = null, string $projectDir = null, \_PhpScopere341acab57d4\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, \_PhpScopere341acab57d4\Psr\Log\LoggerInterface $logger = null)
    {
        $this->debug = $debug;
    ...
    {
        $exception = \_PhpScopere341acab57d4\Symfony\Component\ErrorHandler\Exception\FlattenException::createFromThrowable($exception, null, ['Content-Type' => 'text/html; charset=' . $this->charset]);
        return $exception->setAsString($this->renderException($exception));
    }
    /**
     * Gets the HTML content associated with the given exception.
     */
    public function getBody(\_PhpScopere341acab57d4\Symfony\Component\ErrorHandler\Exception\FlattenException $exception) : string
    {
    ...
    private function renderException(\_PhpScopere341acab57d4\Symfony\Component\ErrorHandler\Exception\FlattenException $exception, string $debugTemplate = 'views/exception_full.html.php') : string
    {
        ...
        return $this->include($debugTemplate, ['exception' => $exception, 'exceptionMessage' => $exceptionMessage, 'statusText' => $statusText, 'statusCode' => $statusCode, 'logger' => $this->logger instanceof \_PhpScopere341acab57d4\Symfony\Component\HttpKernel\Log\DebugLoggerInterface ? $this->logger : null, 'currentContent' => $request ? $this->getAndCleanOutputBuffering($request->headers->get('X-Php-Ob-Level', -1)) : '']);
    }
    private function getAndCleanOutputBuffering(int $startObLevel) : string
    {
        ...
        \_PhpScopere341acab57d4\Symfony\Component\HttpFoundation\Response::closeOutputBuffers($startObLevel + 1, \true);
        return \ob_get_clean();
    }
    /**
