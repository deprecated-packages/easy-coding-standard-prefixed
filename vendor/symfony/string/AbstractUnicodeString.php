 ...
 */
namespace _PhpScoperf6b7f9bf122d\Symfony\Component\String;

use _PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\ExceptionInterface;
use _PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException;
use _PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\RuntimeException;
/**
 ...
 */
abstract class AbstractUnicodeString extends \_PhpScoperf6b7f9bf122d\Symfony\Component\String\AbstractString
{
                ...
                if ('nfd' === ($rule = \strtolower($rule))) {
                    ...
                    if (null === ($transliterator = self::$transliterators[$rule] ?? (self::$transliterators[$rule] = \Transliterator::create($rule)))) {
                        ...
                        if (null === $transliterator) {
                            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Unknown transliteration rule "%s".', $rule));
                        }
    ...
    public function camel() : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
    ...
    public function folded(bool $compat = \true) : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
    ...
    public function join(array $strings, string $lastGlue = null) : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
        $tail = null !== $lastGlue && 1 < \count($strings) ? $lastGlue . \array_pop($strings) : '';
        $str->string = \implode($this->string, $strings) . $tail;
        if (!\preg_match('//u', $str->string)) {
            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
        return $str;
    }
    public function lower() : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
    ...
    public function match(string $regexp, int $flags = 0, int $offset = 0) : array
    {
        ...
        \set_error_handler(static function ($t, $m) {
            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException($m);
        });
        try {
            if (\false === $match($regexp . 'u', $this->string, $matches, $flags | \PREG_UNMATCHED_AS_NULL, $offset)) {
                ...
                foreach (\get_defined_constants(\true)['pcre'] as $k => $v) {
                    if ($lastError === $v && '_ERROR' === \substr($k, -6)) {
                        throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\RuntimeException('Matching failed with ' . $k . '.');
                    }
                }
                throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\RuntimeException('Matching failed with unknown error code.');
            }
        } finally {
    ...
    public function normalize(int $form = self::NFC) : self
    {
        if (!\in_array($form, [self::NFC, self::NFD, self::NFKC, self::NFKD])) {
            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Unsupported normalization form.');
        }
        $str = clone $this;
        \normalizer_is_normalized($str->string, $form) ?: ($str->string = \normalizer_normalize($str->string, $form));
        return $str;
    }
    public function padBoth(int $length, string $padStr = ' ') : \_PhpScoperf6b7f9bf122d\parent
    {
    ...
    public function padEnd(int $length, string $padStr = ' ') : \_PhpScoperf6b7f9bf122d\parent
    {
        if ('' === $padStr || !\preg_match('//u', $padStr)) {
            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
    ...
    public function padStart(int $length, string $padStr = ' ') : \_PhpScoperf6b7f9bf122d\parent
    {
        if ('' === $padStr || !\preg_match('//u', $padStr)) {
            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
    ...
    public function replaceMatches(string $fromRegexp, $to) : \_PhpScoperf6b7f9bf122d\parent
    {
        if ($this->ignoreCase) {
            $fromRegexp .= 'i';
        }
        if (\is_array($to) || $to instanceof \Closure) {
            ...
            $to = static function (array $m) use($to) : string {
                ...
                if ('' !== $to && (!\is_string($to) || !\preg_match('//u', $to))) {
                    throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Replace callback must return a valid UTF-8 string.');
                }
                return $to;
            };
    ...
    public function reverse() : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
        $str->string = \implode('', \array_reverse(\preg_split('/(\\X)/u', $str->string, -1, \PREG_SPLIT_DELIM_CAPTURE | \PREG_SPLIT_NO_EMPTY)));
        return $str;
    }
    public function snake() : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = $this->camel()->title();
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], '_PhpScoperf6b7f9bf122d\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
    public function title(bool $allWords = \false) : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
        $limit = $allWords ? -1 : 1;
    ...
    public function trim(string $chars = " \t\n\r\0\v\f ﻿") : \_PhpScoperf6b7f9bf122d\parent
    {
        if (" \t\n\r\0\v\f ﻿" !== $chars && !\preg_match('//u', $chars)) {
            throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 chars.');
        }
    ...
    public function upper() : \_PhpScoperf6b7f9bf122d\parent
    {
        $str = clone $this;
     ...
     * @return static
     */
    private function pad(int $len, self $pad, int $type) : \_PhpScoperf6b7f9bf122d\parent
    {
        $sLen = $this->length();
        if ($len <= $sLen) {
            return clone $this;
        ...
        switch ($type) {
            ...
            default:
                throw new \_PhpScoperf6b7f9bf122d\Symfony\Component\String\Exception\InvalidArgumentException('Invalid padding type.');
        }
    }
    /**
