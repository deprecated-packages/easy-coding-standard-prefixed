...
final class ProgressBar
{
    ...
    public function setMaxSteps(int $max)
    {
        $this->format = null;
        $this->max = \max(0, $max);
        $this->stepWidth = $this->max ? \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::strlen((string) $this->max) : 4;
    }
    /**
            ...
            if (null !== $this->previousMessage) {
                if ($this->output instanceof ConsoleSectionOutput) {
                    ...
                    foreach ($messageLines as $messageLine) {
                        $messageLineLength = \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::strlenWithoutDecoration($this->output->getFormatter(), $messageLine);
    ...
    private static function initPlaceholderFormatters() : array
    {
        return ['bar' => function (self $bar, OutputInterface $output) {
            ...
            if ($completeBars < $bar->getBarWidth()) {
                $emptyBars = $bar->getBarWidth() - $completeBars - \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::length(\_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::removeDecoration($output->getFormatter(), $bar->getProgressCharacter()));
                $display .= $bar->getProgressCharacter() . \str_repeat($bar->getEmptyBarCharacter(), $emptyBars);
            }
            return $display;
        }, 'elapsed' => function (self $bar) {
            return \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::formatTime(\time() - $bar->getStartTime());
        }, 'remaining' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new LogicException('Unable to display the remaining time if the maximum number of steps is not set.');
            }
            return \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::formatTime($bar->getRemaining());
        }, 'estimated' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new LogicException('Unable to display the estimated time if the maximum number of steps is not set.');
            }
            return \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::formatTime($bar->getEstimated());
        }, 'memory' => function (self $bar) {
            return \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::formatMemory(\memory_get_usage(\true));
    ...
    private function buildLine() : string
    {
        ...
        $linesLength = \array_map(function ($subLine) {
            return \_PhpScopercae9e6ab5cea\Symfony\Component\Console\Helper\Helper::strlenWithoutDecoration($this->output->getFormatter(), \rtrim($subLine, "\r"));
        }, \explode("\n", $line));
