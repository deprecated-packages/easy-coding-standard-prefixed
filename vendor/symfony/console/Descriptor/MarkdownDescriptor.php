 ...
 * @internal
 */
class MarkdownDescriptor extends \_PhpScopere050faf861e6\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     */
    public function describe(\_PhpScopere050faf861e6\Symfony\Component\Console\Output\OutputInterface $output, $object, array $options = [])
    {
    ...
    protected function describeCommand(\_PhpScopere050faf861e6\Symfony\Component\Console\Command\Command $command, array $options = [])
    {
        ...
        $this->write('`' . $command->getName() . "`\n" . \str_repeat('-', \_PhpScopere050faf861e6\Symfony\Component\Console\Helper\Helper::strlen($command->getName()) + 2) . "\n\n" . ($command->getDescription() ? $command->getDescription() . "\n\n" : '') . '### Usage' . "\n\n" . \array_reduce(\array_merge([$command->getSynopsis()], $command->getAliases(), $command->getUsages()), function ($carry, $usage) {
            return $carry . '* `' . $usage . '`' . "\n";
        }));
    ...
    protected function describeApplication(\_PhpScopere050faf861e6\Symfony\Component\Console\Application $application, array $options = [])
    {
        ...
        $this->write($title . "\n" . \str_repeat('=', \_PhpScopere050faf861e6\Symfony\Component\Console\Helper\Helper::strlen($title)));
        foreach ($description->getNamespaces() as $namespace) {
            if (\_PhpScopere050faf861e6\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
    ...
    private function getApplicationTitle(\_PhpScopere050faf861e6\Symfony\Component\Console\Application $application) : string
    {
