...
class PgSqlCaster
{
    ...
    public static function castLink($link, array $a, Stub $stub, bool $isNested)
    {
        ...
        $a['status'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ConstStub(\PGSQL_CONNECTION_OK === $a['status'] ? 'PGSQL_CONNECTION_OK' : 'PGSQL_CONNECTION_BAD', $a['status']);
        $a['busy'] = \pg_connection_busy($link);
        $a['transaction'] = \pg_transaction_status($link);
        if (isset(self::TRANSACTION_STATUS[$a['transaction']])) {
            $a['transaction'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ConstStub(self::TRANSACTION_STATUS[$a['transaction']], $a['transaction']);
        }
        ...
        $a['param'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\EnumStub($a['param']);
        return $a;
    }
    public static function castResult($result, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (isset(self::RESULT_STATUS[$a['status']])) {
            $a['status'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ConstStub(self::RESULT_STATUS[$a['status']], $a['status']);
        }
        ...
        for ($i = 0; $i < $fields; ++$i) {
            ...
            $a['fields'][] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\EnumStub($field);
        }
        return $a;
    }
}
