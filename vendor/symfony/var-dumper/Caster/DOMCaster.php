...
class DOMCaster
{
    ...
    {
        $k = \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\Caster::PREFIX_PROTECTED . 'code';
        if (isset($a[$k], self::ERROR_CODES[$a[$k]])) {
            $a[$k] = new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\ConstStub(self::ERROR_CODES[$a[$k]], $a[$k]);
        }
        return $a;
    }
    ...
    {
        $a += [\_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'Core' => '1.0', \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'XML' => '2.0'];
        return $a;
    }
    public static function castNode(\DOMNode $dom, array $a, Stub $stub, bool $isNested)
    {
        $a += ['nodeName' => $dom->nodeName, 'nodeValue' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->nodeValue), 'nodeType' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\ConstStub(self::NODE_TYPES[$dom->nodeType], $dom->nodeType), 'parentNode' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->parentNode), 'childNodes' => $dom->childNodes, 'firstChild' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->firstChild), 'lastChild' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->lastChild), 'previousSibling' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->previousSibling), 'nextSibling' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->nextSibling), 'attributes' => $dom->attributes, 'ownerDocument' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->ownerDocument), 'namespaceURI' => $dom->namespaceURI, 'prefix' => $dom->prefix, 'localName' => $dom->localName, 'baseURI' => $dom->baseURI ? new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\LinkStub($dom->baseURI) : $dom->baseURI, 'textContent' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->textContent)];
        return $a;
    }
    public static function castNameSpaceNode(\DOMNameSpaceNode $dom, array $a, Stub $stub, bool $isNested)
    {
        $a += ['nodeName' => $dom->nodeName, 'nodeValue' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->nodeValue), 'nodeType' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\ConstStub(self::NODE_TYPES[$dom->nodeType], $dom->nodeType), 'prefix' => $dom->prefix, 'localName' => $dom->localName, 'namespaceURI' => $dom->namespaceURI, 'ownerDocument' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->ownerDocument), 'parentNode' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->parentNode)];
        return $a;
    }
    public static function castDocument(\DOMDocument $dom, array $a, Stub $stub, bool $isNested, int $filter = 0)
    {
        $a += ['doctype' => $dom->doctype, 'implementation' => $dom->implementation, 'documentElement' => new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\CutStub($dom->documentElement), 'actualEncoding' => $dom->actualEncoding, 'encoding' => $dom->encoding, 'xmlEncoding' => $dom->xmlEncoding, 'standalone' => $dom->standalone, 'xmlStandalone' => $dom->xmlStandalone, 'version' => $dom->version, 'xmlVersion' => $dom->xmlVersion, 'strictErrorChecking' => $dom->strictErrorChecking, 'documentURI' => $dom->documentURI ? new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\LinkStub($dom->documentURI) : $dom->documentURI, 'config' => $dom->config, 'formatOutput' => $dom->formatOutput, 'validateOnParse' => $dom->validateOnParse, 'resolveExternals' => $dom->resolveExternals, 'preserveWhiteSpace' => $dom->preserveWhiteSpace, 'recover' => $dom->recover, 'substituteEntities' => $dom->substituteEntities];
        if (!($filter & \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_VERBOSE)) {
            $formatOutput = $dom->formatOutput;
            $dom->formatOutput = \true;
            $a += [\_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'xml' => $dom->saveXML()];
    ...
    {
        $a += ['lineNumber' => $dom->lineNumber, 'columnNumber' => $dom->columnNumber, 'offset' => $dom->offset, 'relatedNode' => $dom->relatedNode, 'uri' => $dom->uri ? new \_PhpScoperc2b2a9bb0e13\Symfony\Component\VarDumper\Caster\LinkStub($dom->uri, $dom->lineNumber) : $dom->uri];
        return $a;
    }
