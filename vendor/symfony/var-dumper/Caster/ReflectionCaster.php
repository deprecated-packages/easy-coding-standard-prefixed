...
class ReflectionCaster
{
    ...
    public static function castClosure(\Closure $c, array $a, Stub $stub, bool $isNested, int $filter = 0)
    {
        ...
        if ($f) {
            $a[$prefix . 'file'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\LinkStub($f, $c->getStartLine());
    ...
    {
        unset($a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'file'], $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'line']);
        return $a;
    }
    public static function castGenerator(\Generator $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        try {
            ...
            $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'closed'] = \true;
            return $a;
        }
    ...
    public static function castReflectionGenerator(\ReflectionGenerator $c, array $a, Stub $stub, bool $isNested)
    {
        $prefix = \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL;
        if ($c->getThis()) {
            $a[$prefix . 'this'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\CutStub($c->getThis());
        }
        ...
        if ($trace = $c->getTrace(\DEBUG_BACKTRACE_IGNORE_ARGS)) {
            ...
            $trace[] = $frame;
            $a[$prefix . 'trace'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\TraceStub($trace, \false, 0, -1, -1);
        } else {
            $function = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\FrameStub($frame, \false, \true);
            $function = \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ExceptionCaster::castFrameStub($function, [], $function, \true);
            $a[$prefix . 'executing'] = $function[$prefix . 'src'];
        }
        $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'closed'] = \false;
        return $a;
    }
    public static function castClass(\ReflectionClass $c, array $a, Stub $stub, bool $isNested, int $filter = 0)
    {
        ...
        if (!($filter & \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_VERBOSE) && !$isNested) {
            self::addExtra($a, $c);
        }
        return $a;
    }
    public static function castFunctionAbstract(\ReflectionFunctionAbstract $c, array $a, Stub $stub, bool $isNested, int $filter = 0)
    {
        ...
        if (isset($a[$prefix . 'returnType'])) {
            ...
            $a[$prefix . 'returnType'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ClassStub($a[$prefix . 'returnType'] instanceof \ReflectionNamedType && $a[$prefix . 'returnType']->allowsNull() && 'mixed' !== $v ? '?' . $v : $v, [\class_exists($v, \false) || \interface_exists($v, \false) || \trait_exists($v, \false) ? $v : '', '']);
        }
        if (isset($a[$prefix . 'class'])) {
            $a[$prefix . 'class'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ClassStub($a[$prefix . 'class']);
        }
        if (isset($a[$prefix . 'this'])) {
            $a[$prefix . 'this'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\CutStub($a[$prefix . 'this']);
        }
        ...
        if (isset($a[$prefix . 'parameters'])) {
            $a[$prefix . 'parameters'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\EnumStub($a[$prefix . 'parameters']);
        }
        self::addAttributes($a, $c, $prefix);
        if (!($filter & \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_VERBOSE) && ($v = $c->getStaticVariables())) {
            foreach ($v as $k => &$v) {
                if (\is_object($v)) {
                    $a[$prefix . 'use']['$' . $k] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\CutStub($v);
                } else {
                    $a[$prefix . 'use']['$' . $k] =& $v;
                }
            }
            unset($v);
            $a[$prefix . 'use'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\EnumStub($a[$prefix . 'use']);
        }
        if (!($filter & \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_VERBOSE) && !$isNested) {
            self::addExtra($a, $c);
        }
    ...
    {
        $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'modifiers'] = \implode(' ', \Reflection::getModifierNames($c->getModifiers()));
        $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'value'] = $c->getValue();
    ...
    public static function castParameter(\ReflectionParameter $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (isset($a[$prefix . 'typeHint'])) {
            $v = $a[$prefix . 'typeHint'];
            $a[$prefix . 'typeHint'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ClassStub($v, [\class_exists($v, \false) || \interface_exists($v, \false) || \trait_exists($v, \false) ? $v : '', '']);
        } else {
        ...
        try {
            ...
            if ($c->isDefaultValueConstant()) {
                $a[$prefix . 'default'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\ConstStub($c->getDefaultValueConstantName(), $v);
            }
            if (null === $v) {
                unset($a[$prefix . 'allowsNull']);
            }
    ...
    {
        $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'id'] = $c->getId();
        return $a;
    }
    ...
    {
        $x = isset($a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'extra']) ? $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'extra']->value : [];
        if (\method_exists($c, 'getFileName') && ($m = $c->getFileName())) {
            $x['file'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\LinkStub($m, $c->getStartLine());
            $x['line'] = $c->getStartLine() . ' to ' . $c->getEndLine();
        }
        self::addMap($x, $c, self::EXTRA_MAP, '');
        if ($x) {
            $a[\_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'extra'] = new \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\EnumStub($x);
        }
    }
    private static function addMap(array &$a, object $c, array $map, string $prefix = \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL)
    {
        foreach ($map as $k => $m) {
    ...
    private static function addAttributes(array &$a, \Reflector $c, string $prefix = \_PhpScoper32abeec2fe5a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL) : void
    {
