...
class XmlReaderCaster
{
    ...
    {
        $props = \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'parserProperties';
        $info = ['localName' => $reader->localName, 'prefix' => $reader->prefix, 'nodeType' => new \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\ConstStub(self::NODE_TYPES[$reader->nodeType], $reader->nodeType), 'depth' => $reader->depth, 'isDefault' => $reader->isDefault, 'isEmptyElement' => \XMLReader::NONE === $reader->nodeType ? null : $reader->isEmptyElement, 'xmlLang' => $reader->xmlLang, 'attributeCount' => $reader->attributeCount, 'value' => $reader->value, 'namespaceURI' => $reader->namespaceURI, 'baseURI' => $reader->baseURI ? new \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\LinkStub($reader->baseURI) : $reader->baseURI, $props => ['LOADDTD' => $reader->getParserProperty(\XMLReader::LOADDTD), 'DEFAULTATTRS' => $reader->getParserProperty(\XMLReader::DEFAULTATTRS), 'VALIDATE' => $reader->getParserProperty(\XMLReader::VALIDATE), 'SUBST_ENTITIES' => $reader->getParserProperty(\XMLReader::SUBST_ENTITIES)]];
        if ($info[$props] = \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\Caster::filter($info[$props], \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_EMPTY, [], $count)) {
            $info[$props] = new \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\EnumStub($info[$props]);
            $info[$props]->cut = $count;
        }
        $info = \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\Caster::filter($info, \_PhpScoper653866602a9e\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_EMPTY, [], $count);
