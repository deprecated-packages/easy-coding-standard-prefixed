 ...
 */
class ArgsStub extends \_PhpScoper45e1bde0f567\Symfony\Component\VarDumper\Caster\EnumStub
{
    ...
    public function __construct(array $args, string $function, ?string $class)
    {
        ...
        $values = [];
        foreach ($args as $k => $v) {
            $values[$k] = !\is_scalar($v) && !$v instanceof Stub ? new \_PhpScoper45e1bde0f567\Symfony\Component\VarDumper\Caster\CutStub($v) : $v;
        }
        if (null === $params) {
        ...
        if (\count($values) < \count($params)) {
            ...
            $values[] = new \_PhpScoper45e1bde0f567\Symfony\Component\VarDumper\Caster\EnumStub(\array_splice($values, \count($params)), \false);
            $params[] = $variadic;
        }
        if (['...'] === $params) {
            $this->dumpKeys = \false;
