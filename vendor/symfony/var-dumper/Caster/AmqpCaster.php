...
class AmqpCaster
{
    ...
    public static function castExchange(\AMQPExchange $c, array $a, Stub $stub, bool $isNested)
    {
        $prefix = \_PhpScoper7e6a1331d94a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL;
        $a += [$prefix . 'flags' => self::extractFlags($c->getFlags())];
        $type = isset(self::EXCHANGE_TYPES[$c->getType()]) ? new \_PhpScoper7e6a1331d94a\Symfony\Component\VarDumper\Caster\ConstStub(self::EXCHANGE_TYPES[$c->getType()], $c->getType()) : $c->getType();
        // Recent version of the extension already expose private properties
    ...
    {
        $prefix = \_PhpScoper7e6a1331d94a\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL;
        $deliveryMode = new \_PhpScoper7e6a1331d94a\Symfony\Component\VarDumper\Caster\ConstStub($c->getDeliveryMode() . (2 === $c->getDeliveryMode() ? ' (persistent)' : ' (non-persistent)'), $c->getDeliveryMode());
        // Recent version of the extension already expose private properties
    ...
    private static function extractFlags(int $flags) : \_PhpScoper7e6a1331d94a\Symfony\Component\VarDumper\Caster\ConstStub
    {
        $flagsArray = [];
        ...
        return new \_PhpScoper7e6a1331d94a\Symfony\Component\VarDumper\Caster\ConstStub(\implode('|', $flagsArray), $flags);
    }
}
