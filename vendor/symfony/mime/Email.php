 ...
 * @experimental in 4.3
 */
class Email extends \_PhpScoper78af57a363a0\Symfony\Component\Mime\Message
{
    ...
    {
        return $this->setHeaderBody('Path', 'Return-Path', \_PhpScoper78af57a363a0\Symfony\Component\Mime\Address::create($address));
    }
    public function getReturnPath() : ?\_PhpScoper78af57a363a0\Symfony\Component\Mime\Address
    {
    ...
    {
        return $this->setHeaderBody('Mailbox', 'Sender', \_PhpScoper78af57a363a0\Symfony\Component\Mime\Address::create($address));
    }
    public function getSender() : ?\_PhpScoper78af57a363a0\Symfony\Component\Mime\Address
    {
     ...
     * @return $this
     */
    public function attachPart(\_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\DataPart $part)
    {
     ...
     */
    private function generateBody() : \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\AbstractPart
    {
        if (null === $this->text && null === $this->html) {
            throw new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Exception\LogicException('A message must have a text and/or an HTML part.');
        }
        $part = null === $this->text ? null : new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\TextPart($this->text, $this->textCharset);
        [$htmlPart, $attachmentParts, $inlineParts] = $this->prepareParts();
        if (null !== $htmlPart) {
            if (null !== $part) {
                $part = new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\Multipart\AlternativePart($part, $htmlPart);
            } else {
                $part = $htmlPart;
            }
        }
        if ($inlineParts) {
            $part = new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\Multipart\RelatedPart($part, ...$inlineParts);
        }
        if ($attachmentParts) {
            $part = new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\Multipart\MixedPart($part, ...$attachmentParts);
        }
        return $part;
    }
    private function prepareParts() : ?array
    {
        ...
        if (null !== $this->html) {
            ...
            $htmlPart = new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\TextPart($html, $this->htmlCharset, 'html');
    ...
    private function createDataPart(array $attachment) : \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\DataPart
    {
        if (isset($attachment['part'])) {
            return $attachment['part'];
        }
        if (isset($attachment['body'])) {
            $part = new \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\DataPart($attachment['body'], $attachment['name'] ?? null, $attachment['content-type'] ?? null);
        } else {
            $part = \_PhpScoper78af57a363a0\Symfony\Component\Mime\Part\DataPart::fromPath($attachment['path'] ?? '', $attachment['name'] ?? null, $attachment['content-type'] ?? null);
        }
    ...
    private function addListAddressHeaderBody($name, array $addresses)
    {
        ...
        $to->addAddresses(\_PhpScoper78af57a363a0\Symfony\Component\Mime\Address::createArray($addresses));
        return $this;
    }
    private function setListAddressHeaderBody($name, array $addresses)
    {
        $addresses = \_PhpScoper78af57a363a0\Symfony\Component\Mime\Address::createArray($addresses);
