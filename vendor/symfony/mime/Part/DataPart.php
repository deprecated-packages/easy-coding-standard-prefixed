 ...
 * @experimental in 4.3
 */
class DataPart extends \_PhpScoper7cef7256eba6\Symfony\Component\Mime\Part\TextPart
{
    ...
    public static function fromPath(string $path, string $name = null, string $contentType = null) : self
    {
        ...
        if (null === $contentType) {
            ...
            if (null === self::$mimeTypes) {
                self::$mimeTypes = new \_PhpScoper7cef7256eba6\Symfony\Component\Mime\MimeTypes();
            }
            $contentType = self::$mimeTypes->getMimeTypes($ext)[0] ?? 'application/octet-stream';
        }
        if (\false === ($handle = @\fopen($path, 'r', \false))) {
            throw new \_PhpScoper7cef7256eba6\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Unable to open path "%s"', $path));
        }
    ...
    public function __sleep()
    {
        ...
        foreach (['body', 'charset', 'subtype', 'disposition', 'name', 'encoding'] as $name) {
            $r = new \ReflectionProperty(\_PhpScoper7cef7256eba6\Symfony\Component\Mime\Part\TextPart::class, $name);
            $r->setAccessible(\true);
