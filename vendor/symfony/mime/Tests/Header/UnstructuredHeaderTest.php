...
class UnstructuredHeaderTest extends \_PhpScoperedc2e0c967db\PHPUnit\Framework\TestCase
{
    private $charset = 'utf-8';
    public function testGetNameReturnsNameVerbatim()
    {
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('Subject', '');
        $this->assertEquals('Subject', $header->getName());
    }
    public function testGetValueReturnsValueVerbatim()
    {
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('Subject', 'Test');
    ...
    public function testLongHeadersAreFoldedAtWordBoundary()
    {
        ...
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('X-Custom-Header', $value);
        /*
    ...
    public function testPrintableAsciiOnlyAppearsInHeaders()
    {
        ...
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', $nonAsciiChar);
    ...
    public function testEncodedWordsAreUsedToEncodedNonPrintableAscii()
    {
        ...
        foreach ($nonPrintableBytes as $byte) {
            ...
            $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('X-A', $char);
            $header->setCharset('iso-8859-1');
    ...
    public function testFWSPIsUsedWhenEncoderReturnsMultipleLines()
    {
        ...
        // Note also that =?utf-8?q??= is 12 chars which makes 75 - 12 = 63
        //* X-Test: is 8 chars
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', \pack('C', 0x8f) . 'line_one_here' . "\r\n" . 'line_two_here');
        $header->setCharset('iso-8859-1');
    ...
    public function testAdjacentWordsAreEncodedTogether()
    {
        ...
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', $text);
        $header->setCharset('iso-8859-1');
    ...
    public function testLanguageInformationAppearsInEncodedWords()
    {
        ...
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('Subject', $value);
    ...
    {
        $header = new \_PhpScoperedc2e0c967db\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', '');
        $header->setBody('test');
        $this->assertEquals('test', $header->getValue());
    }
