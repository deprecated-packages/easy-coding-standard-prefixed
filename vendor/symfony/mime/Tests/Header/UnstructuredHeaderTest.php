...
class UnstructuredHeaderTest extends \_PhpScoper614deab2c612\PHPUnit\Framework\TestCase
{
    private $charset = 'utf-8';
    public function testGetNameReturnsNameVerbatim()
    {
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('Subject', '');
        $this->assertEquals('Subject', $header->getName());
    }
    public function testGetValueReturnsValueVerbatim()
    {
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('Subject', 'Test');
    ...
    public function testPrintableAsciiOnlyAppearsInHeaders()
    {
        ...
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', $nonAsciiChar);
    ...
    public function testEncodedWordsAreUsedToEncodedNonPrintableAscii()
    {
        ...
        $nonPrintableBytes = \array_merge(\range(0x0, 0x8), \range(0x10, 0x19), [0x7f]);
        foreach ($nonPrintableBytes as $byte) {
            $char = \pack('C', $byte);
            $encodedChar = \sprintf('=%02X', $byte);
            $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('X-A', $char);
            $header->setCharset('iso-8859-1');
    ...
    public function testFWSPIsUsedWhenEncoderReturnsMultipleLines()
    {
        ...
        // Note also that =?utf-8?q??= is 12 chars which makes 75 - 12 = 63
        //* X-Test: is 8 chars
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', \pack('C', 0x8f) . 'line_one_here' . "\r\n" . 'line_two_here');
        $header->setCharset('iso-8859-1');
    ...
    public function testAdjacentWordsAreEncodedTogether()
    {
        ...
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', $text);
        $header->setCharset('iso-8859-1');
    ...
    public function testLanguageInformationAppearsInEncodedWords()
    {
        ...
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('Subject', $value);
    ...
    {
        $header = new \_PhpScoper614deab2c612\Symfony\Component\Mime\Header\UnstructuredHeader('X-Test', '');
        $header->setBody('test');
        $this->assertEquals('test', $header->getValue());
    }
