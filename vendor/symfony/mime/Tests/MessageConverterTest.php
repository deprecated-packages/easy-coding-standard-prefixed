...
use _PhpScoperfb2c402b972b\Symfony\Component\Mime\MessageConverter;
class MessageConverterTest extends \_PhpScoperfb2c402b972b\PHPUnit\Framework\TestCase
{
    public function testToEmail()
    {
        $file = \file_get_contents(__DIR__ . '/Fixtures/mimetypes/test.gif');
        $email = (new \_PhpScoperfb2c402b972b\Symfony\Component\Mime\Email())->from('fabien@symfony.com');
        $this->assertSame($email, \_PhpScoperfb2c402b972b\Symfony\Component\Mime\MessageConverter::toEmail($email));
    ...
    private function assertConversion(\_PhpScoperfb2c402b972b\Symfony\Component\Mime\Email $expected)
    {
        $r = new \ReflectionMethod($expected, 'generateBody');
        $r->setAccessible(\true);
        $message = new \_PhpScoperfb2c402b972b\Symfony\Component\Mime\Message($expected->getHeaders(), $r->invoke($expected));
        $converted = \_PhpScoperfb2c402b972b\Symfony\Component\Mime\MessageConverter::toEmail($message);
