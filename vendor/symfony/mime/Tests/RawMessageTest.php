...
class RawMessageTest extends \_PhpScopera37d6fb0b1ab\PHPUnit\Framework\TestCase
{
    public function testToString()
    {
        $message = new \_PhpScopera37d6fb0b1ab\Symfony\Component\Mime\RawMessage('string');
        $this->assertEquals('string', $message->toString());
        $this->assertEquals('string', \implode('', \iterator_to_array($message->toIterable())));
        // calling methods more than once work
        $this->assertEquals('string', $message->toString());
        $this->assertEquals('string', \implode('', \iterator_to_array($message->toIterable())));
        $message = new \_PhpScopera37d6fb0b1ab\Symfony\Component\Mime\RawMessage(new \ArrayObject(['some', ' ', 'string']));
