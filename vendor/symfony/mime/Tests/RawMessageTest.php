...
class RawMessageTest extends \_PhpScopere341acab57d4\PHPUnit\Framework\TestCase
{
    public function testToString()
    {
        $message = new \_PhpScopere341acab57d4\Symfony\Component\Mime\RawMessage('string');
        $this->assertEquals('string', $message->toString());
        $this->assertEquals('string', \implode('', \iterator_to_array($message->toIterable())));
        // calling methods more than once work
        $this->assertEquals('string', $message->toString());
        $this->assertEquals('string', \implode('', \iterator_to_array($message->toIterable())));
        $message = new \_PhpScopere341acab57d4\Symfony\Component\Mime\RawMessage(new \ArrayObject(['some', ' ', 'string']));
