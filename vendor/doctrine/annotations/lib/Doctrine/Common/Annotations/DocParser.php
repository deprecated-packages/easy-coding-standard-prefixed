...
use ECSPrefix20210507\Doctrine\Common\Annotations\Annotation\Attribute;
use ECSPrefix20210507\Doctrine\Common\Annotations\Annotation\Attributes;
use ECSPrefix20210507\Doctrine\Common\Annotations\Annotation\Enum;
use ECSPrefix20210507\Doctrine\Common\Annotations\Annotation\NamedArgumentConstructor;
...
final class DocParser
{
     ...
     */
    private static $classIdentifiers = [\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER, \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_TRUE, \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_FALSE, \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_NULL];
    /**
     * The lexer.
     *
     * @var DocLexer
    ...
    {
        $this->lexer = new \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer();
    }
    /**
    ...
    private function syntaxError(string $expected, ?array $token = null) : \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException
    {
        ...
        $message .= '.';
        return \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::syntaxError($message);
    }
    /**
    ...
    private function classExists(string $fqcn) : bool
    {
        ...
        return $this->classExists[$fqcn] = \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationRegistry::loadAnnotationClass($fqcn);
    }
    /**
    ...
    private function collectAnnotationMetadata(string $name) : void
    {
        ...
        $metadata['has_named_argument_constructor'] = $metadata['has_constructor'] && $class->implementsInterface(\ECSPrefix20210507\Doctrine\Common\Annotations\NamedArgumentConstructorAnnotation::class);
    ...
    private function Annotations() : array
    {
        ...
        while ($this->lexer->lookahead !== null) {
            if ($this->lexer->lookahead['type'] !== \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_AT) {
                $this->lexer->moveNext();
                continue;
            }
            ...
            if ($peek === null || $peek['type'] !== \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_NAMESPACE_SEPARATOR && !in_array($peek['type'], self::$classIdentifiers, \true) || $peek['position'] !== $this->lexer->lookahead['position'] + 1) {
                $this->lexer->moveNext();
                continue;
            }
    ...
    {
        $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_AT);
        // check if we have an annotation
        $name = $this->Identifier();
        if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_MINUS) && $this->lexer->nextTokenIsAdjacent()) {
        ...
        if ($name[0] !== '\\') {
            ...
            if (!$found) {
                ...
                throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::semanticalError(sprintf(<<<'EXCEPTION'
...
, $name, $this->context));
        ...
        if (!$this->classExists($name)) {
            throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::semanticalError(sprintf('The annotation "@%s" in %s does not exist, or could not be auto-loaded.', $name, $this->context));
        }
...
, $originalName, $this->context, self::$annotationMetadata[$name]['targets_literal']));
        ...
        if (isset(self::$annotationMetadata[$name]['enum'])) {
            ...
            foreach (self::$annotationMetadata[$name]['enum'] as $property => $enum) {
                ...
                if (isset($values[$property]) && !in_array($values[$property], $enum['value'])) {
                    throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::enumeratorError($property, $name, $this->context, $enum['literal'], $values[$property]);
                }
            }
        }
        // checks all declared attributes
        foreach (self::$annotationMetadata[$name]['attribute_types'] as $property => $type) {
            ...
            if (!isset($values[$property])) {
                if ($type['required']) {
                    throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::requiredError($property, $originalName, $this->context, 'a(n) ' . $type['value']);
                }
                continue;
            }
            if ($type['type'] === 'array') {
                ...
                if (isset($type['array_type'])) {
                    foreach ($values[$property] as $item) {
                        if (gettype($item) !== $type['array_type'] && !$item instanceof $type['array_type']) {
                            throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::attributeTypeError($property, $originalName, $this->context, 'either a(n) ' . $type['array_type'] . ', or an array of ' . $type['array_type'] . 's', $item);
                        }
                    }
                }
            } elseif (gettype($values[$property]) !== $type['type'] && !$values[$property] instanceof $type['type']) {
                throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::attributeTypeError($property, $originalName, $this->context, 'a(n) ' . $type['value'], $values[$property]);
            }
        }
        if (self::$annotationMetadata[$name]['has_named_argument_constructor']) {
            ...
            foreach ($values as $property => $value) {
                if (!isset(self::$annotationMetadata[$name]['constructor_args'][$property])) {
                    throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::creationError(sprintf(<<<'EXCEPTION'
...
, $originalName, $this->context, $property, implode(', ', self::$annotationMetadata[$name]['properties'])));
                ...
                if (!$property) {
                    throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::creationError(sprintf('The annotation @%s declared on %s does not accept any values, but got %s.', $originalName, $this->context, json_encode($values)));
                }
            }
    ...
    private function MethodCall() : array
    {
        ...
        $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS);
        if (!$this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS)) {
            $values = $this->Values();
        }
        $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS);
        return $values;
    }
    /**
    ...
    private function Values() : array
    {
        ...
        while ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_COMMA)) {
            $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_COMMA);
            if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS)) {
                break;
            }
            $token = $this->lexer->lookahead;
    ...
    private function Constant()
    {
        ...
        if (!defined($identifier)) {
            throw \ECSPrefix20210507\Doctrine\Common\Annotations\AnnotationException::semanticalErrorConstants($identifier, $this->context);
        }
    ...
    private function Identifier() : string
    {
        ...
        while ($this->lexer->lookahead !== null && $this->lexer->lookahead['position'] === $this->lexer->token['position'] + strlen($this->lexer->token['value']) && $this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_NAMESPACE_SEPARATOR)) {
            $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_NAMESPACE_SEPARATOR);
    ...
    {
        $peek = $this->lexer->glimpse();
        if ($peek['type'] === \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_EQUALS) {
    ...
    {
        if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES)) {
            return $this->Arrayx();
        }
        if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_AT)) {
            return $this->Annotation();
        }
        if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER)) {
            return $this->Constant();
        }
        switch ($this->lexer->lookahead['type']) {
            case \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_STRING:
                $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_STRING);
                return $this->lexer->token['value'];
            case \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_INTEGER:
                $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_INTEGER);
                return (int) $this->lexer->token['value'];
            case \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_FLOAT:
                $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_FLOAT);
                return (float) $this->lexer->token['value'];
            case \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_TRUE:
                $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_TRUE);
                return \true;
            case \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_FALSE:
                $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_FALSE);
                return \false;
            case \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_NULL:
                $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_NULL);
                return null;
            default:
    ...
    private function FieldAssignment() : stdClass
    {
        ...
        $this->match(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_EQUALS);
        $item = new stdClass();
        $item->name = $fieldName;
    ...
    private function Arrayx() : array
    {
        ...
        if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES)) {
    ...
    private function ArrayEntry() : array
    {
        ...
        if ($peek['type'] === \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_EQUALS || $peek['type'] === \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_COLON) {
            if ($this->lexer->isNextToken(\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER)) {
                ...
                $this->matchAny([\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_INTEGER, \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_STRING]);
                $key = $this->lexer->token['value'];
            }
            $this->matchAny([\ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_EQUALS, \ECSPrefix20210507\Doctrine\Common\Annotations\DocLexer::T_COLON]);
