...
use _PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\IgnoreAnnotation;
use _PhpScoper18bd934c069f\Doctrine\Common\Annotations\DocParser;
use _PhpScoper18bd934c069f\Doctrine\Common\Annotations\AnnotationRegistry;
use _PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\Target;
use _PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants;
use _PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants;
use _PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\IntefaceWithConstants;
class DocParserTest extends \_PhpScoper18bd934c069f\PHPUnit_Framework_TestCase
{
    public function testNestedArraysWithNestedAnnotation()
    {
        ...
        $annot = $result[0];
        $this->assertTrue($annot instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Name);
        ...
        $this->assertTrue($nestedArray['key'] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Name);
    }
    public function testBasicAnnotations()
    {
        ...
        $this->assertTrue($annot instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Name);
        $this->assertTrue(\is_array($annot->value));
        $this->assertTrue($annot->value[0] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Name);
        $this->assertTrue($annot->value[1] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Name);
...
DOCBLOCK;
        ...
        $this->assertEquals("bar", $annot->foo);
        $marker = $result[1];
        $this->assertTrue($marker instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Marker);
    }
...
DOCBLOCK;
        ...
        $this->assertTrue($annot instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\SomeAnnotationClassNameWithoutConstructor);
...
DOCBLOCK;
        $result = $parser->parse($docblock);
        $this->assertEquals(\count($result), 1);
        $this->assertTrue($result[0] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\SomeAnnotationClassNameWithoutConstructorAndProperties);
    }
    public function testAnnotationTarget()
    {
        $parser = new \_PhpScoper18bd934c069f\Doctrine\Common\Annotations\DocParser();
        $parser->setImports(array('__NAMESPACE__' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures'));
        $class = new \ReflectionClass('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\ClassWithValidAnnotationTarget');
        $context = 'class ' . $class->getName();
        $docComment = $class->getDocComment();
        $parser->setTarget(\_PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\Target::TARGET_CLASS);
        $this->assertNotNull($parser->parse($docComment, $context));
        $property = $class->getProperty('foo');
        $docComment = $property->getDocComment();
        $context = 'property ' . $class->getName() . "::\$" . $property->getName();
        $parser->setTarget(\_PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\Target::TARGET_PROPERTY);
        $this->assertNotNull($parser->parse($docComment, $context));
        $method = $class->getMethod('someFunction');
        $docComment = $property->getDocComment();
        $context = 'method ' . $class->getName() . '::' . $method->getName() . '()';
        $parser->setTarget(\_PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\Target::TARGET_METHOD);
        $this->assertNotNull($parser->parse($docComment, $context));
        try {
            $class = new \ReflectionClass('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\ClassWithInvalidAnnotationTargetAtClass');
            $context = 'class ' . $class->getName();
            $docComment = $class->getDocComment();
            $parser->setTarget(\_PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\Target::TARGET_CLASS);
            $parser->parse($docComment, $context);
            $this->fail();
        } catch (\_PhpScoper18bd934c069f\Doctrine\Common\Annotations\AnnotationException $exc) {
            $this->assertNotNull($exc->getMessage());
        }
        try {
            $class = new \ReflectionClass('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\ClassWithInvalidAnnotationTargetAtMethod');
        ...
        try {
            $class = new \ReflectionClass('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\ClassWithInvalidAnnotationTargetAtProperty');
            $property = $class->getProperty('foo');
            $docComment = $property->getDocComment();
            $context = 'property ' . $class->getName() . "::\$" . $property->getName();
            $parser->setTarget(\_PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation\Target::TARGET_PROPERTY);
    ...
    public function getAnnotationVarTypeProviderValid()
    {
        ...
        return array(
            ...
            array('mixed', '1'),
            array('mixed', '1.2'),
            array('mixed', '_PhpScoper18bd934c069f\\@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll'),
            // boolean type
            array('boolean', 'true'),
            array('boolean', 'false'),
            ...
            // annotation instance
            array('annotation', '_PhpScoper18bd934c069f\\@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll'),
    ...
    public function getAnnotationVarTypeProviderInvalid()
    {
        ...
        return array(
            ...
            array('boolean', 'boolean', '@Name', '_PhpScoper18bd934c069f\\an instance of Doctrine\\Tests\\Common\\Annotations\\Name'),
            // alias for internal type boolean
            ...
            // annotation instance
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', 'true', 'boolean'),
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', 'false', 'boolean'),
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '12', 'integer'),
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '1.2', 'double'),
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '{"str"}', 'array'),
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '{1,2,3,4}', 'array'),
            array('annotation', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '@Name', '_PhpScoper18bd934c069f\\an instance of Doctrine\\Tests\\Common\\Annotations\\Name'),
        );
    }
    public function getAnnotationVarTypeArrayProviderInvalid()
    {
        //({attribute name}, {type declared type}, {attribute value} , {given type or class})
        return array(array('arrayOfIntegers', 'integer', 'true', 'boolean'), array('arrayOfIntegers', 'integer', 'false', 'boolean'), array('arrayOfIntegers', 'integer', '{true,true}', 'boolean'), array('arrayOfIntegers', 'integer', '{1,true}', 'boolean'), array('arrayOfIntegers', 'integer', '{1,2,1.2}', 'double'), array('arrayOfIntegers', 'integer', '{1,2,"str"}', 'string'), array('arrayOfStrings', 'string', 'true', 'boolean'), array('arrayOfStrings', 'string', 'false', 'boolean'), array('arrayOfStrings', 'string', '{true,true}', 'boolean'), array('arrayOfStrings', 'string', '{"foo",true}', 'boolean'), array('arrayOfStrings', 'string', '{"foo","bar",1.2}', 'double'), array('arrayOfStrings', 'string', '1', 'integer'), array('arrayOfAnnotations', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', 'true', 'boolean'), array('arrayOfAnnotations', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', 'false', 'boolean'), array('arrayOfAnnotations', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '{@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll,true}', 'boolean'), array('arrayOfAnnotations', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '{@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll,true}', 'boolean'), array('arrayOfAnnotations', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '{@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll,1.2}', 'double'), array('arrayOfAnnotations', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll', '{@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAll,@AnnotationExtendsAnnotationTargetAll,"str"}', 'string'));
    }
    /**
    ...
    public function testAnnotationWithVarType($attribute, $value)
    {
        ...
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithVarType', $result[0]);
    ...
    public function testAnnotationWithAttributes($attribute, $value)
    {
        ...
        $result = $parser->parse($docblock, $context);
        $this->assertTrue(\sizeof($result) === 1);
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithAttributes', $result[0]);
    ...
    public function testAnnotationWithRequiredAttributes()
    {
        ...
        $result = $parser->parse($docblock);
        $this->assertTrue(\sizeof($result) === 1);
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithRequiredAttributes', $result[0]);
        $this->assertEquals("Some Value", $result[0]->getValue());
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAnnotation', $result[0]->getAnnot());
    ...
    public function testAnnotationWithRequiredAttributesWithoutContructor()
    {
        ...
        $result = $parser->parse($docblock);
        $this->assertTrue(\sizeof($result) === 1);
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithRequiredAttributesWithoutContructor', $result[0]);
        $this->assertEquals("Some Value", $result[0]->value);
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationTargetAnnotation', $result[0]->annot);
    ...
    public function getConstantsProvider()
    {
        ...
        $provider[] = array('@AnnotationWithConstants(AnnotationWithConstants::INTEGER)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::INTEGER);
        $provider[] = array('@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants(AnnotationWithConstants::STRING)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::STRING);
        $provider[] = array('@AnnotationWithConstants(Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants::FLOAT)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::FLOAT);
        $provider[] = array('@AnnotationWithConstants(ClassWithConstants::SOME_VALUE)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::SOME_VALUE);
        $provider[] = array('@AnnotationWithConstants(ClassWithConstants::OTHER_KEY_)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::OTHER_KEY_);
        $provider[] = array('@AnnotationWithConstants(ClassWithConstants::OTHER_KEY_2)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::OTHER_KEY_2);
        $provider[] = array('@AnnotationWithConstants(Doctrine\\Tests\\Common\\Annotations\\Fixtures\\ClassWithConstants::SOME_VALUE)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::SOME_VALUE);
        $provider[] = array('@AnnotationWithConstants(IntefaceWithConstants::SOME_VALUE)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\IntefaceWithConstants::SOME_VALUE);
        $provider[] = array('@AnnotationWithConstants(\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\IntefaceWithConstants::SOME_VALUE)', \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\IntefaceWithConstants::SOME_VALUE);
        $provider[] = array('@AnnotationWithConstants({AnnotationWithConstants::STRING, AnnotationWithConstants::INTEGER, AnnotationWithConstants::FLOAT})', array(\_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::STRING, \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::INTEGER, \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::FLOAT));
        $provider[] = array('@AnnotationWithConstants({
                AnnotationWithConstants::STRING = AnnotationWithConstants::INTEGER
             })', array(\_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::STRING => \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::INTEGER));
        $provider[] = array('@AnnotationWithConstants({
                Doctrine\\Tests\\Common\\Annotations\\Fixtures\\IntefaceWithConstants::SOME_KEY = AnnotationWithConstants::INTEGER
             })', array(\_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\IntefaceWithConstants::SOME_KEY => \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::INTEGER));
        $provider[] = array('@AnnotationWithConstants({
                \\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\IntefaceWithConstants::SOME_KEY = AnnotationWithConstants::INTEGER
             })', array(\_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\IntefaceWithConstants::SOME_KEY => \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::INTEGER));
        $provider[] = array('@AnnotationWithConstants({
             ...
             })', array(\_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::STRING => \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationWithConstants::INTEGER, \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::SOME_KEY => \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::SOME_VALUE, \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\ClassWithConstants::SOME_KEY => \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\IntefaceWithConstants::SOME_VALUE));
        $provider[] = array('@AnnotationWithConstants(AnnotationWithConstants::class)', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants');
        $provider[] = array('@AnnotationWithConstants({AnnotationWithConstants::class = AnnotationWithConstants::class})', array('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants'));
        $provider[] = array('@AnnotationWithConstants(Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants::class)', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants');
        $provider[] = array('@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants(Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants::class)', '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants');
        return $provider;
    }
    /**
    ...
    {
        $parser = $this->createTestParser();
        $parser->setImports(array('classwithconstants' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\ClassWithConstants', 'intefacewithconstants' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\IntefaceWithConstants', 'annotationwithconstants' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants'));
        $result = $parser->parse($docblock);
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\AnnotationWithConstants', $annotation = $result[0]);
...
DOCBLOCK;
        $result = $parser->parse($docblock);
        $this->assertInstanceOf("_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Name", $result[0]);
        $docblock = <<<DOCBLOCK
/**
 * @Name
 * @Marker
...
DOCBLOCK;
    ...
    {
        $this->assertFalse(\class_exists('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixture\\Annotation\\Autoload', \false), 'Pre-condition: Doctrine\\Tests\\Common\\Annotations\\Fixture\\Annotation\\Autoload not allowed to be loaded.');
        $parser = new \_PhpScoper18bd934c069f\Doctrine\Common\Annotations\DocParser();
        \_PhpScoper18bd934c069f\Doctrine\Common\Annotations\AnnotationRegistry::registerAutoloadNamespace('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\Annotation', __DIR__ . '/../../../../');
        $parser->setImports(array('autoload' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\Annotation\\Autoload'));
        $annotations = $parser->parse('@Autoload');
        $this->assertEquals(1, \count($annotations));
        $this->assertInstanceOf('_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Fixtures\\Annotation\\Autoload', $annotations[0]);
    }
    public function createTestParser()
    {
        $parser = new \_PhpScoper18bd934c069f\Doctrine\Common\Annotations\DocParser();
        $parser->setIgnoreNotImportedAnnotations(\true);
        $parser->setImports(array('name' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations\\Name', '__NAMESPACE__' => '_PhpScoper18bd934c069f\\Doctrine\\Tests\\Common\\Annotations'));
        return $parser;
    }
    /**
     * @group DDC-78
    ...
    {
        $parser = $this->createTestParser();
        $parser->parse("@Name(foo='bar')", "_PhpScoper18bd934c069f\\class \\Doctrine\\Tests\\Common\\Annotations\\Name");
    }
    /**
     * @group DDC-183
     */
...
DOCBLOCK;
    ...
    {
        $parser = $this->createTestParser();
        $result = $parser->parse('_PhpScoper18bd934c069f\\@Doctrine\\Tests\\Common\\Annotations\\True');
        $this->assertTrue($result[0] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\True);
        $result = $parser->parse('_PhpScoper18bd934c069f\\@Doctrine\\Tests\\Common\\Annotations\\False');
        $this->assertTrue($result[0] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\False);
        $result = $parser->parse('_PhpScoper18bd934c069f\\@Doctrine\\Tests\\Common\\Annotations\\Null');
        $this->assertTrue($result[0] instanceof \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Null);
...
DOCBLOCK;
    ...
    {
        $parser = $this->createTestParser();
        $annots = $parser->parse('_PhpScoper18bd934c069f\\@Doctrine\\Tests\\Common\\Annotations\\Fixtures\\Annotation\\AnnotWithDefaultValue');
        $this->assertEquals(1, \count($annots));
...
/** @Annotation */
class AnnotationExtendsAnnotationTargetAll extends \_PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\Fixtures\AnnotationTargetAll
{
}
/** @Annotation */
class Name extends \_PhpScoper18bd934c069f\Doctrine\Common\Annotations\Annotation
{
    public $foo;
}
/** @Annotation */
...
namespace _PhpScoper18bd934c069f\Doctrine\Tests\Common\Annotations\FooBar;
