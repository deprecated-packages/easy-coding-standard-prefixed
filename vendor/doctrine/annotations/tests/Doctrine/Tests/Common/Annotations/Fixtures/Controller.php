...
class Controller
{
    ...
    {
        $form = \_PhpScoper592524ba34f0\Doctrine\Tests\Common\Annotations\Fixtures\ContactForm::create($this->get('form.context'), 'contact');
        $form->bind($this->container->get('request'), $form);
        if ($form->isValid()) {
            $form->send($this->get('mailer'));
            $this->get('session')->setFlash('notice', 'Message sent!');
            return new \_PhpScoper592524ba34f0\Doctrine\Tests\Common\Annotations\Fixtures\RedirectResponse($this->generateUrl('_demo'));
        }
     ...
     * @return void
     */
    private function createObjectIdentity(\_PhpScoper592524ba34f0\Doctrine\Tests\Common\Annotations\Fixtures\ObjectIdentityInterface $oid)
    {
     ...
     * @return integer
     */
    private function createOrRetrieveSecurityIdentityId(\_PhpScoper592524ba34f0\Doctrine\Tests\Common\Annotations\Fixtures\SecurityIdentityInterface $sid)
    {
     ...
     * @return void
     */
    private function regenerateAncestorRelations(\_PhpScoper592524ba34f0\Doctrine\Tests\Common\Annotations\Fixtures\AclInterface $acl)
    {
        $pk = $acl->getId();
        $this->connection->executeQuery($this->getDeleteObjectIdentityRelationsSql($pk));
        ...
        foreach ($changes[1] as $field => $new) {
            for ($i = 0, $c = \count($new); $i < $c; $i++) {
                ...
                if (null === $ace->getId()) {
                    ...
                    $aceIdProperty = new \ReflectionProperty('_PhpScoper592524ba34f0\\Symfony\\Component\\Security\\Acl\\Domain\\Entry', 'id');
