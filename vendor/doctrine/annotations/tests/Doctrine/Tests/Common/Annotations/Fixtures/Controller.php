...
class Controller
{
    ...
    {
        $form = \_PhpScoper8a05d21c15c9\Doctrine\Tests\Common\Annotations\Fixtures\ContactForm::create($this->get('form.context'), 'contact');
        $form->bind($this->container->get('request'), $form);
        if ($form->isValid()) {
            $form->send($this->get('mailer'));
            $this->get('session')->setFlash('notice', 'Message sent!');
            return new \_PhpScoper8a05d21c15c9\Doctrine\Tests\Common\Annotations\Fixtures\RedirectResponse($this->generateUrl('_demo'));
        }
     ...
     * @return void
     */
    private function createObjectIdentity(\_PhpScoper8a05d21c15c9\Doctrine\Tests\Common\Annotations\Fixtures\ObjectIdentityInterface $oid)
    {
     ...
     * @return integer
     */
    private function createOrRetrieveSecurityIdentityId(\_PhpScoper8a05d21c15c9\Doctrine\Tests\Common\Annotations\Fixtures\SecurityIdentityInterface $sid)
    {
     ...
     * @return void
     */
    private function regenerateAncestorRelations(\_PhpScoper8a05d21c15c9\Doctrine\Tests\Common\Annotations\Fixtures\AclInterface $acl)
    {
        $pk = $acl->getId();
        $this->connection->executeQuery($this->getDeleteObjectIdentityRelationsSql($pk));
        ...
        foreach ($changes[1] as $field => $new) {
            for ($i = 0, $c = \count($new); $i < $c; $i++) {
                ...
                if (null === $ace->getId()) {
                    ...
                    $aceIdProperty = new \ReflectionProperty('_PhpScoper8a05d21c15c9\\Symfony\\Component\\Security\\Acl\\Domain\\Entry', 'id');
