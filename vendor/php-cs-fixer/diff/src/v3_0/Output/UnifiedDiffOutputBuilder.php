 ...
 */
final class UnifiedDiffOutputBuilder extends \PhpCsFixer\Diff\v3_0\Output\AbstractChunkOutputBuilder
{
    /**
     * @var bool
     */
    ...
    private function writeDiffHunks($output, array $diff)
    {
        ...
        if (0 === $diff[$upperLimit - 1][1]) {
            ...
            if ("\n" !== $lc) {
                \array_splice($diff, $upperLimit, 0, [["\n\\ No newline at end of file\n", \PhpCsFixer\Diff\v3_0\Differ::NO_LINE_END_EOF_WARNING]]);
            }
        } else {
        ...
        foreach ($diff as $i => $entry) {
            ...
            $sameCount = 0;
            if ($entry[1] === \PhpCsFixer\Diff\v3_0\Differ::NO_LINE_END_EOF_WARNING) {
                continue;
            }
            if (\false === $hunkCapture) {
                $hunkCapture = $i;
            }
            if (\PhpCsFixer\Diff\v3_0\Differ::ADDED === $entry[1]) {
                ++$toRange;
            }
            if (\PhpCsFixer\Diff\v3_0\Differ::REMOVED === $entry[1]) {
                ++$fromRange;
            }
        }
        if (\false === $hunkCapture) {
    ...
    private function writeHunk(array $diff, $diffStartIndex, $diffEndIndex, $fromStart, $fromRange, $toStart, $toRange, $output)
    {
        ...
        for ($i = $diffStartIndex; $i < $diffEndIndex; ++$i) {
            if ($diff[$i][1] === \PhpCsFixer\Diff\v3_0\Differ::ADDED) {
                \fwrite($output, '+' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \PhpCsFixer\Diff\v3_0\Differ::REMOVED) {
                \fwrite($output, '-' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \PhpCsFixer\Diff\v3_0\Differ::OLD) {
                \fwrite($output, ' ' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \PhpCsFixer\Diff\v3_0\Differ::NO_LINE_END_EOF_WARNING) {
                \fwrite($output, "\n");
                // $diff[$i][0]
            } else {
