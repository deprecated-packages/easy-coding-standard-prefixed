...
final class Differ
{
    ...
    public function diffToArray($from, $to, $lcs = null)
    {
        if (\is_string($from)) {
            ...
            throw new \PhpCsFixer\Diff\InvalidArgumentException('"from" must be an array or string.');
        }
        if (\is_string($to)) {
            $to = $this->splitStringByLines($to);
        } elseif (!\is_array($to)) {
            throw new \PhpCsFixer\Diff\InvalidArgumentException('"to" must be an array or string.');
        }
    ...
    private function selectLcsImplementation(array $from, array $to)
    {
        ...
        if ($this->calculateEstimatedFootprint($from, $to) > $memoryLimit) {
            return new \PhpCsFixer\Diff\MemoryEfficientLongestCommonSubsequenceCalculator();
        }
        return new \PhpCsFixer\Diff\TimeEfficientLongestCommonSubsequenceCalculator();
    }
    /**
