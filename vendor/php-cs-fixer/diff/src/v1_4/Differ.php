...
class Differ
{
    ...
    private function getDiffBufferElement(array $diff, $buffer, $diffIndex)
    {
        if ($diff[$diffIndex][1] === 1) {
            ...
            $buffer .= '-' . $diff[$diffIndex][0] . "\n";
        } elseif ($this->showNonDiffLines === \true) {
            $buffer .= ' ' . $diff[$diffIndex][0] . "\n";
        }
        return $buffer;
    }
    ...
    public function diffToArray($from, $to, LongestCommonSubsequence $lcs = null)
    {
        ...
        if ($this->detectUnmatchedLineEndings($fromMatches, $toMatches)) {
            $diff[] = array('#Warnings contain different line endings!', 0);
        }
    ...
    {
        return isset($fromMatches[0], $toMatches[0]) && \count($fromMatches[0]) === \count($toMatches[0]) && $fromMatches[0] !== $toMatches[0];
    }
    /**
     * @param array $from
     * @param array $to
