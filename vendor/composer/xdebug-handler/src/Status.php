...
class Status
{
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \_PhpScoper3d04c8135695\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \_PhpScoper3d04c8135695\Psr\Log\LogLevel::WARNING);
    }
    ...
    private function reportRestarted()
    {
        ...
        $level = $this->loaded ? \_PhpScoper3d04c8135695\Psr\Log\LogLevel::WARNING : null;
