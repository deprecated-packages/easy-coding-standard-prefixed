...
class Status
{
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \_PhpScoper2b44cb0c30af\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \_PhpScoper2b44cb0c30af\Psr\Log\LogLevel::WARNING);
    }
    ...
    private function reportRestarted()
    {
        ...
        $level = $this->loaded ? \_PhpScoper2b44cb0c30af\Psr\Log\LogLevel::WARNING : null;
