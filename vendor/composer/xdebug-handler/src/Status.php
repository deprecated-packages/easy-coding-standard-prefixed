...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \_PhpScoper14718bfcc75e\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \_PhpScoper14718bfcc75e\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \_PhpScoper14718bfcc75e\Psr\Log\LogLevel::WARNING);
    }
    ...
    {
        $this->output($this->getLoadedMessage());
        \_PhpScoper14718bfcc75e\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    private function reportRestarted()
    {
        $loaded = $this->getLoadedMessage();
        $text = \sprintf('Restarted (%d ms). %s', $this->time, $loaded);
        $level = $this->loaded ? \_PhpScoper14718bfcc75e\Psr\Log\LogLevel::WARNING : null;
