...
class Status
{
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \_PhpScoperaad82bb90a86\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \_PhpScoperaad82bb90a86\Psr\Log\LogLevel::WARNING);
    }
    ...
    private function reportRestarted()
    {
        ...
        $level = $this->loaded ? \_PhpScoperaad82bb90a86\Psr\Log\LogLevel::WARNING : null;
