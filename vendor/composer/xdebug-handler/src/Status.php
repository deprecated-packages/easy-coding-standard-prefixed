...
class Status
{
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \_PhpScoper589e8c04a9ef\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \_PhpScoper589e8c04a9ef\Psr\Log\LogLevel::WARNING);
    }
    ...
    private function reportRestarted()
    {
        ...
        $level = $this->loaded ? \_PhpScoper589e8c04a9ef\Psr\Log\LogLevel::WARNING : null;
