...
class Status
{
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \_PhpScoper83a475a0590e\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \_PhpScoper83a475a0590e\Psr\Log\LogLevel::WARNING);
    }
    ...
    private function reportRestarted()
    {
        ...
        $level = $this->loaded ? \_PhpScoper83a475a0590e\Psr\Log\LogLevel::WARNING : null;
