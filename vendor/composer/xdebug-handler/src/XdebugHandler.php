...
class XdebugHandler
{
    ...
    {
        $this->notify(\Composer\XdebugHandler\Status::CHECK, $this->loaded);
        ...
        if (empty($envArgs[0]) && $this->requiresRestart((bool) $this->loaded)) {
            ...
            if ($this->prepareRestart()) {
                ...
                $this->notify(\Composer\XdebugHandler\Status::RESTARTING, $command);
    ...
    private function prepareRestart()
    {
        ...
        if ($error) {
            $this->notify(\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return empty($error);
    }
    /**
    ...
    private function getCommand()
    {
        ...
        if (\defined('STDOUT') && \Composer\XdebugHandler\Process::supportsColor(\STDOUT)) {
            $args = \Composer\XdebugHandler\Process::addColorOption($args, $this->colorOption);
        }
        $args = \array_merge($php, array($this->script), $args);
        $cmd = \Composer\XdebugHandler\Process::escape(\array_shift($args), \true, \true);
        foreach ($args as $arg) {
            $cmd .= ' ' . \Composer\XdebugHandler\Process::escape($arg);
        }
        return $cmd;
    }
    /**
