...
use ECSPrefix20210507\Composer\Semver\Constraint\MatchNoneConstraint;
...
class Intervals
{
    ...
    public static function compactConstraint($constraint)
    {
        ...
        if (\count($intervals['numeric']) === 1 && (string) $intervals['numeric'][0]->getStart() === (string) \ECSPrefix20210507\Composer\Semver\Interval::fromZero() && (string) $intervals['numeric'][0]->getEnd() === (string) \ECSPrefix20210507\Composer\Semver\Interval::untilPositiveInfinity()) {
            ...
            for ($i = 0, $count = \count($intervals['numeric']); $i < $count; $i++) {
                ...
                if ($interval->getEnd()->getOperator() === '<' && $i + 1 < $count) {
                    ...
                    if ($interval->getEnd()->getVersion() === $nextInterval->getStart()->getVersion() && $nextInterval->getStart()->getOperator() === '>') {
                        ...
                        if (\count($unEqualConstraints) === 0 && (string) $interval->getStart() !== (string) \ECSPrefix20210507\Composer\Semver\Interval::fromZero()) {
                ...
                if (\count($unEqualConstraints) > 0) {
                    ...
                    if ((string) $interval->getEnd() !== (string) \ECSPrefix20210507\Composer\Semver\Interval::untilPositiveInfinity()) {
                ...
                if ((string) $interval->getStart() === (string) \ECSPrefix20210507\Composer\Semver\Interval::fromZero()) {
                    $constraints[] = $interval->getEnd();
                } elseif ((string) $interval->getEnd() === (string) \ECSPrefix20210507\Composer\Semver\Interval::untilPositiveInfinity()) {
        ...
        if (0 === \count($intervals['branches']['names'])) {
            ...
            if ($intervals['branches']['exclude']) {
                ...
                if (\count($constraints) === 1 && (string) $constraints[0] === (string) \ECSPrefix20210507\Composer\Semver\Interval::fromZero()) {
    ...
    private static function generateIntervals($constraint, $stopOnFirstValidInterval = \false)
    {
        ...
        if ($constraint->isDisjunctive()) {
            $branches = \ECSPrefix20210507\Composer\Semver\Interval::noDev();
            ...
            $branches = \ECSPrefix20210507\Composer\Semver\Interval::anyDev();
            foreach ($constraintBranches as $b) {
                if ($b['exclude']) {
                    if ($branches['exclude']) {
        ...
        foreach ($borders as $border) {
            ...
            if ($active && $activeIntervals < $activationThreshold) {
                ...
                if (\version_compare($start->getVersion(), $border['version'], '=') && ($start->getOperator() === '>' && $border['operator'] === '<=' || $start->getOperator() === '>=' && $border['operator'] === '<')) {
                    ...
                    $intervals[$index] = new \ECSPrefix20210507\Composer\Semver\Interval($start, new Constraint($border['operator'], $border['version']));
                    $index++;
    ...
    private static function generateSingleConstraintIntervals($constraint)
    {
        ...
        if (\strpos($constraint->getVersion(), 'dev-') === 0) {
            ...
            if ($op === '!=') {
                $intervals[] = new \ECSPrefix20210507\Composer\Semver\Interval(\ECSPrefix20210507\Composer\Semver\Interval::fromZero(), \ECSPrefix20210507\Composer\Semver\Interval::untilPositiveInfinity());
        ...
        if ($op[0] === '>') {
            // > & >=
            return array('numeric' => array(new \ECSPrefix20210507\Composer\Semver\Interval($constraint, \ECSPrefix20210507\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \ECSPrefix20210507\Composer\Semver\Interval::noDev());
        }
        if ($op[0] === '<') {
            // < & <=
            return array('numeric' => array(new \ECSPrefix20210507\Composer\Semver\Interval(\ECSPrefix20210507\Composer\Semver\Interval::fromZero(), $constraint)), 'branches' => \ECSPrefix20210507\Composer\Semver\Interval::noDev());
        }
        if ($op === '!=') {
            // convert !=x to intervals of 0 - <x && >x - +inf + dev*
            return array('numeric' => array(new \ECSPrefix20210507\Composer\Semver\Interval(\ECSPrefix20210507\Composer\Semver\Interval::fromZero(), new Constraint('<', $constraint->getVersion())), new \ECSPrefix20210507\Composer\Semver\Interval(new Constraint('>', $constraint->getVersion()), \ECSPrefix20210507\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \ECSPrefix20210507\Composer\Semver\Interval::anyDev());
        }
        // convert ==x to an interval of >=x - <=x
        return array('numeric' => array(new \ECSPrefix20210507\Composer\Semver\Interval(new Constraint('>=', $constraint->getVersion()), new Constraint('<=', $constraint->getVersion()))), 'branches' => \ECSPrefix20210507\Composer\Semver\Interval::noDev());
    }
}
