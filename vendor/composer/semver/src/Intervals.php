...
use _PhpScoper917c99b6aa4c\Composer\Semver\Constraint\MatchNoneConstraint;
...
class Intervals
{
    ...
    public static function compactConstraint(ConstraintInterface $constraint)
    {
        ...
        if (\count($intervals['numeric']) === 1 && (string) $intervals['numeric'][0]->getStart() === (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero() && (string) $intervals['numeric'][0]->getEnd() === (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity()) {
            ...
            for ($i = 0, $count = \count($intervals['numeric']); $i < $count; $i++) {
                ...
                if ($interval->getEnd()->getOperator() === '<' && $i + 1 < $count) {
                    ...
                    if ($interval->getEnd()->getVersion() === $nextInterval->getStart()->getVersion() && $nextInterval->getStart()->getOperator() === '>') {
                        ...
                        if (\count($unEqualConstraints) === 0 && (string) $interval->getStart() !== (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero()) {
                ...
                if (\count($unEqualConstraints) > 0) {
                    ...
                    if ((string) $interval->getEnd() !== (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity()) {
                ...
                if ((string) $interval->getStart() === (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero()) {
                    $constraints[] = $interval->getEnd();
                } elseif ((string) $interval->getEnd() === (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity()) {
        ...
        if (0 === \count($intervals['branches']['names'])) {
            ...
            if ($intervals['branches']['exclude']) {
                ...
                if (\count($constraints) === 1 && (string) $constraints[0] === (string) \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero()) {
    ...
    {
        if ($constraint instanceof MatchAllConstraint) {
            return array('numeric' => array(new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval(\_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero(), \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::anyDev());
        }
        ...
        if ($constraint->isDisjunctive()) {
            $branches = \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::noDev();
            ...
            $branches = \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::anyDev();
            foreach ($constraintBranches as $b) {
                if ($b['exclude']) {
                    if ($branches['exclude']) {
        ...
        foreach ($borders as $border) {
            ...
            if ($active && $activeIntervals < $activationThreshold) {
                ...
                if (\version_compare($start->getVersion(), $border['version'], '=') && ($start->getOperator() === '>' && $border['operator'] === '<=' || $start->getOperator() === '>=' && $border['operator'] === '<')) {
                    ...
                    $intervals[$index] = new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval($start, new Constraint($border['operator'], $border['version']));
                    $index++;
    ...
    private static function generateSingleConstraintIntervals(Constraint $constraint)
    {
        ...
        if (\strpos($constraint->getVersion(), 'dev-') === 0) {
            ...
            if ($op === '!=') {
                $intervals[] = new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval(\_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero(), \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity());
        ...
        if ($op[0] === '>') {
            // > & >=
            return array('numeric' => array(new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval($constraint, \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::noDev());
        }
        if ($op[0] === '<') {
            // < & <=
            return array('numeric' => array(new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval(\_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero(), $constraint)), 'branches' => \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::noDev());
        }
        if ($op === '!=') {
            // convert !=x to intervals of 0 - <x && >x - +inf + dev*
            return array('numeric' => array(new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval(\_PhpScoper917c99b6aa4c\Composer\Semver\Interval::fromZero(), new Constraint('<', $constraint->getVersion())), new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval(new Constraint('>', $constraint->getVersion()), \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::anyDev());
        }
        // convert ==x to an interval of >=x - <=x
        return array('numeric' => array(new \_PhpScoper917c99b6aa4c\Composer\Semver\Interval(new Constraint('>=', $constraint->getVersion()), new Constraint('<=', $constraint->getVersion()))), 'branches' => \_PhpScoper917c99b6aa4c\Composer\Semver\Interval::noDev());
    }
}
