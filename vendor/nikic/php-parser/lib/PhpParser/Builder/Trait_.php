...
class Trait_ extends \_PhpScoper04022cd986ec\PhpParser\Builder\Declaration
{
    protected $name;
    protected $uses = [];
    ...
    public function addStmt($stmt)
    {
        ...
        if ($stmt instanceof \_PhpScoper04022cd986ec\PhpParser\Node\Stmt\Property) {
            ...
            $this->methods[] = $stmt;
        } elseif ($stmt instanceof \_PhpScoper04022cd986ec\PhpParser\Node\Stmt\TraitUse) {
            $this->uses[] = $stmt;
        } else {
    ...
    {
        return new \_PhpScoper04022cd986ec\PhpParser\Node\Stmt\Trait_($this->name, ['stmts' => \array_merge($this->uses, $this->properties, $this->methods)], $this->attributes);
    }
}
