...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \_PhpScoperad4605bb9267\resolveNodes($grammarCode);
    $grammarCode = \_PhpScoperad4605bb9267\resolveMacros($grammarCode);
    $grammarCode = \_PhpScoperad4605bb9267\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \_PhpScoperad4605bb9267\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \_PhpScoperad4605bb9267\removeTrailingWhitespace($resultCode);
    \_PhpScoperad4605bb9267\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \_PhpScoperad4605bb9267\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \_PhpScoperad4605bb9267\resolveNodes($matches['params']);
        $params = \_PhpScoperad4605bb9267\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \_PhpScoperad4605bb9267\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \_PhpScoperad4605bb9267\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' == $name) {
            \_PhpScoperad4605bb9267\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' == $name) {
            \_PhpScoperad4605bb9267\assertArgs(1, $args, $name);
        ...
        if ('push' == $name) {
            \_PhpScoperad4605bb9267\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' == $name) {
            \_PhpScoperad4605bb9267\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\_PhpScoperad4605bb9267\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
