...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \_PhpScopera04bf8e97c06\resolveNodes($grammarCode);
    $grammarCode = \_PhpScopera04bf8e97c06\resolveMacros($grammarCode);
    $grammarCode = \_PhpScopera04bf8e97c06\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \_PhpScopera04bf8e97c06\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \_PhpScopera04bf8e97c06\removeTrailingWhitespace($resultCode);
    \_PhpScopera04bf8e97c06\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \_PhpScopera04bf8e97c06\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \_PhpScopera04bf8e97c06\resolveNodes($matches['params']);
        $params = \_PhpScopera04bf8e97c06\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \_PhpScopera04bf8e97c06\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \_PhpScopera04bf8e97c06\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \_PhpScopera04bf8e97c06\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \_PhpScopera04bf8e97c06\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \_PhpScopera04bf8e97c06\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \_PhpScopera04bf8e97c06\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\_PhpScopera04bf8e97c06\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
