...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \_PhpScopera37d6fb0b1ab\resolveNodes($grammarCode);
    $grammarCode = \_PhpScopera37d6fb0b1ab\resolveMacros($grammarCode);
    $grammarCode = \_PhpScopera37d6fb0b1ab\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \_PhpScopera37d6fb0b1ab\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \_PhpScopera37d6fb0b1ab\removeTrailingWhitespace($resultCode);
    \_PhpScopera37d6fb0b1ab\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \_PhpScopera37d6fb0b1ab\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \_PhpScopera37d6fb0b1ab\resolveNodes($matches['params']);
        $params = \_PhpScopera37d6fb0b1ab\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \_PhpScopera37d6fb0b1ab\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \_PhpScopera37d6fb0b1ab\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \_PhpScopera37d6fb0b1ab\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \_PhpScopera37d6fb0b1ab\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \_PhpScopera37d6fb0b1ab\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \_PhpScopera37d6fb0b1ab\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\_PhpScopera37d6fb0b1ab\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
