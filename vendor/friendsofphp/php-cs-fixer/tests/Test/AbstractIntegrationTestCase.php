...
use _PhpScoper5465fda93cc7\Prophecy\Argument;
...
abstract class AbstractIntegrationTestCase extends \PhpCsFixer\Tests\TestCase
{
    ...
    public function provideIntegrationCases()
    {
        ...
        foreach (\_PhpScoper5465fda93cc7\Symfony\Component\Finder\Finder::create()->files()->in($fixturesDir) as $file) {
    ...
    protected function doTest(\PhpCsFixer\Tests\Test\IntegrationCase $case)
    {
        ...
        if (\false === @\file_put_contents($tmpFile, $input)) {
            throw new \_PhpScoper5465fda93cc7\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Failed to write to tmp. file "%s".', $tmpFile));
        }
    ...
    private function getLinter()
    {
        ...
        if (null === $linter) {
            if (\getenv('SKIP_LINT_TEST_CASES')) {
                ...
                $linterProphecy->lintSource(\_PhpScoper5465fda93cc7\Prophecy\Argument::type('string'))->willReturn($this->prophesize(\PhpCsFixer\Linter\LintingResultInterface::class)->reveal());
                $linterProphecy->lintFile(\_PhpScoper5465fda93cc7\Prophecy\Argument::type('string'))->willReturn($this->prophesize(\PhpCsFixer\Linter\LintingResultInterface::class)->reveal());
