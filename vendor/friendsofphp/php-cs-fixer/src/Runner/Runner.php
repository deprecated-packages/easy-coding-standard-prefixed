        ...
        if ($oldHash !== $newHash) {
            ...
            if (!$this->isDryRun) {
                if (\false === @\file_put_contents($file->getRealPath(), $new)) {
                    ...
                    throw new \_PhpScoperbcc0395698f8\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Failed to write file "%s", "%s".', $file->getPathname(), $error ? $error['message'] : 'no reason available'), 0, null, $file->getRealPath());
                }
            }
        }
