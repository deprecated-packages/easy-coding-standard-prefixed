        ...
        if ($oldHash !== $newHash) {
            ...
            if (!$this->isDryRun) {
                if (\false === @\file_put_contents($file->getRealPath(), $new)) {
                    ...
                    throw new \_PhpScoper6c2f17c43d2d\Symfony\Component\Filesystem\Exception\IOException(\sprintf('Failed to write file "%s", "%s".', $file->getPathname(), $error ? $error['message'] : 'no reason available'), 0, null, $file->getRealPath());
                }
            }
        }
