 ...
 * @internal
 */
final class DescribeCommand extends \_PhpScoper470d6df94ac0\Symfony\Component\Console\Command\Command
{
    ...
    {
        $this->setDefinition([new \_PhpScoper470d6df94ac0\Symfony\Component\Console\Input\InputArgument('name', \_PhpScoper470d6df94ac0\Symfony\Component\Console\Input\InputArgument::REQUIRED, 'Name of rule / set.')])->setDescription('Describe rule / ruleset.');
    }
    /**
     * {@inheritdoc}
     */
     ...
     * @param string $name
     */
    private function describeRule(\_PhpScoper470d6df94ac0\Symfony\Component\Console\Output\OutputInterface $output, $name)
    {
        ...
        if ($output->getVerbosity() >= \_PhpScoper470d6df94ac0\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE) {
        ...
        if ($fixer instanceof \PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface) {
            ...
            foreach ($options as $option) {
                $line = '* <info>' . \_PhpScoper470d6df94ac0\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getName()) . '</info>';
                ...
                $description = \PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \_PhpScoper470d6df94ac0\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getDescription()));
                ...
                if ($option instanceof \PhpCsFixer\FixerConfiguration\DeprecatedFixerOption) {
                    $line .= '. <error>DEPRECATED</error>: ' . \PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \_PhpScoper470d6df94ac0\Symfony\Component\Console\Formatter\OutputFormatter::escape(\lcfirst($option->getDeprecationMessage())));
                }
     ...
     * @param string $name
     */
    private function describeSet(\_PhpScoper470d6df94ac0\Symfony\Component\Console\Output\OutputInterface $output, $name)
    {
     ...
     */
    private function describeList(\_PhpScoper470d6df94ac0\Symfony\Component\Console\Output\OutputInterface $output, $type)
    {
        if ($output->getVerbosity() >= \_PhpScoper470d6df94ac0\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE) {
            $describe = ['set' => $this->getSetNames(), 'rules' => $this->getFixers()];
        } elseif ($output->getVerbosity() >= \_PhpScoper470d6df94ac0\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE) {
