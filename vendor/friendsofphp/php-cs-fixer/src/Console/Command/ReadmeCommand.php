 ...
 * @internal
 */
final class ReadmeCommand extends \_PhpScoper61bdb485188f\Symfony\Component\Console\Command\Command
{
...
EOF;
        ...
        $help = \PhpCsFixer\Preg::replace('#</?(comment|info)>#', '``', $help);
        $help = \PhpCsFixer\Preg::replace('#`(``.+?``)`#', '$1', $help);
        $help = \PhpCsFixer\Preg::replace('#^(\\s+)``(.+)``$#m', '$1$2', $help);
        $help = \PhpCsFixer\Preg::replace('#^ \\* ``(.+)``(.*?\\n)#m', "* **\$1**\$2\n", $help);
        $help = \PhpCsFixer\Preg::replace('#^   \\| #m', '  ', $help);
        $help = \PhpCsFixer\Preg::replace('#^   \\|#m', '', $help);
        $help = \PhpCsFixer\Preg::replace('#^(?=  \\*Risky rule: )#m', "\n", $help);
        $help = \PhpCsFixer\Preg::replace("#^(  Configuration options:\n)(  - )#m", "\$1\n\$2", $help);
        $help = \PhpCsFixer\Preg::replace("#^\n( +\\\$ )#m", "\n.. code-block:: bash\n\n\$1", $help);
        $help = \PhpCsFixer\Preg::replace("#^\n( +<\\?php)#m", "\n.. code-block:: php\n\n\$1", $help);
        $help = \PhpCsFixer\Preg::replaceCallback('#^\\s*<\\?(\\w+).*?\\?>#ms', static function ($matches) {
            $result = \PhpCsFixer\Preg::replace("#^\\.\\. code-block:: bash\n\n#m", '', $matches[0]);
            if ('php' !== $matches[1]) {
                $result = \PhpCsFixer\Preg::replace("#<\\?{$matches[1]}\\s*#", '', $result);
            }
        ...
        //      `description <http://...>`_
        $help = \PhpCsFixer\Preg::replaceCallback('#`(.+)`\\s?\\(<url>(.+)<\\/url>\\)#', static function (array $matches) {
            return \sprintf('`%s <%s>`_', \str_replace('\\', '\\\\', $matches[1]), $matches[2]);
        }, $help);
        $help = \PhpCsFixer\Preg::replace('#^                        #m', '  ', $help);
        $help = \PhpCsFixer\Preg::replace('#\\*\\* +\\[#', '** [', $help);
        $downloadLatestUrl = \sprintf('https://github.com/FriendsOfPHP/PHP-CS-Fixer/releases/download/v%s/php-cs-fixer.phar', \PhpCsFixer\Console\Command\HelpCommand::getLatestReleaseVersionFromChangeLog());
