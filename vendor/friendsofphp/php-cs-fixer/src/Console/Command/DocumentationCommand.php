...
use _PhpScopere050faf861e6\Symfony\Component\Filesystem\Filesystem;
use _PhpScopere050faf861e6\Symfony\Component\Finder\Finder;
use _PhpScopere050faf861e6\Symfony\Component\Finder\SplFileInfo;
/**
 * @internal
 */
final class DocumentationCommand extends \_PhpScopere050faf861e6\Symfony\Component\Console\Command\Command
{
    ...
    {
        $filesystem = new \_PhpScopere050faf861e6\Symfony\Component\Filesystem\Filesystem();
        ...
        foreach ((new \_PhpScopere050faf861e6\Symfony\Component\Finder\Finder())->files()->in($this->generator->getFixersDocumentationDirectoryPath())->notPath($docForFixerRelativePaths) as $file) {
    ...
    private function generateRuleSetsDocs(array $fixers)
    {
        $filesystem = new \_PhpScopere050faf861e6\Symfony\Component\Filesystem\Filesystem();
        /** @var SplFileInfo $file */
        foreach ((new \_PhpScopere050faf861e6\Symfony\Component\Finder\Finder())->files()->in($this->generator->getRuleSetsDocumentationDirectoryPath()) as $file) {
            $filesystem->remove($file->getPathname());
        }
