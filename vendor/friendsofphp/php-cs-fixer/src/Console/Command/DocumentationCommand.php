 ...
 * @internal
 */
final class DocumentationCommand extends \_PhpScoper069ebd53a518\Symfony\Component\Console\Command\Command
{
    ...
    {
        $filesystem = new \_PhpScoper069ebd53a518\Symfony\Component\Filesystem\Filesystem();
        ...
        foreach ((new \_PhpScoper069ebd53a518\Symfony\Component\Finder\Finder())->files()->in($this->generator->getFixersDocumentationDirectoryPath())->notPath($docForFixerRelativePaths) as $file) {
    ...
    private function generateRuleSetsDocs(array $fixers)
    {
        $filesystem = new \_PhpScoper069ebd53a518\Symfony\Component\Filesystem\Filesystem();
        /** @var SplFileInfo $file */
        foreach ((new \_PhpScoper069ebd53a518\Symfony\Component\Finder\Finder())->files()->in($this->generator->getRuleSetsDocumentationDirectoryPath()) as $file) {
            $filesystem->remove($file->getPathname());
        }
