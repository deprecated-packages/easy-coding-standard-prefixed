...
use _PhpScoper3fa05b4669af\Symfony\Component\Console\Command\HelpCommand as BaseHelpCommand;
 ...
 * @internal
 */
final class HelpCommand extends \_PhpScoper3fa05b4669af\Symfony\Component\Console\Command\HelpCommand
{
...
EOF;
        return \strtr($template, ['%%%CONFIG_INTERFACE_URL%%%' => \sprintf('https://github.com/FriendsOfPHP/PHP-CS-Fixer/blob/v%s/src/ConfigInterface.php', self::getLatestReleaseVersionFromChangeLog()), '%%%CI_INTEGRATION%%%' => \implode("\n", \array_map(static function ($line) {
            return '    $ ' . $line;
        }, \array_slice(\file(__DIR__ . '/../../../dev-tools/ci-integration.sh', \FILE_IGNORE_NEW_LINES), 3))), '%%%FIXERS_DETAILS%%%' => self::getFixersHelp()]);
    }
    /**
     * @param mixed $value
     *
    ...
    public static function toString($value)
    {
        if (\is_array($value)) {
            ...
            do {
                $strNew = \PhpCsFixer\Preg::replace($replaces[0], $replaces[1], $str);
                if ($strNew === $str) {
                    break;
                }
                $str = $strNew;
    ...
    public static function getLatestReleaseVersionFromChangeLog()
    {
        ...
        for ($i = (int) \PhpCsFixer\Console\Application::VERSION; $i > 0; --$i) {
    ...
    {
        $output->getFormatter()->setStyle('url', new \_PhpScoper3fa05b4669af\Symfony\Component\Console\Formatter\OutputFormatterStyle('blue'));
    }
    /**
     * @return null|string
     */
    ...
    private static function getFixersHelp()
    {
        ...
        $ruleSets = [];
        foreach (\PhpCsFixer\RuleSet::create()->getSetDefinitionNames() as $setName) {
            $ruleSets[$setName] = new \PhpCsFixer\RuleSet([$setName => \true]);
        }
            ...
            if ($fixer instanceof \PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface) {
                ...
                if (\count($configurationDefinitionOptions)) {
                    ...
                    foreach ($configurationDefinitionOptions as $option) {
                        ...
                        $line .= ': ' . \PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \lcfirst(\PhpCsFixer\Preg::replace('/\\.$/', '', \_PhpScoper3fa05b4669af\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getDescription())))) . '; ';
                        ...
                        if ($option instanceof \PhpCsFixer\FixerConfiguration\DeprecatedFixerOption) {
                            $line .= '. DEPRECATED: ' . \PhpCsFixer\Preg::replace('/(`.+?`)/', '<info>$1</info>', \lcfirst(\PhpCsFixer\Preg::replace('/\\.$/', '', \_PhpScoper3fa05b4669af\Symfony\Component\Console\Formatter\OutputFormatter::escape($option->getDeprecationMessage()))));
                        }
