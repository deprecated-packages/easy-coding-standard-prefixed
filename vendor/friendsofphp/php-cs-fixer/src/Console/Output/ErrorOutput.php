...
final class ErrorOutput
{
    ...
    public function listErrors($process, array $errors)
    {
        ...
        $showDetails = $this->output->getVerbosity() >= \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE;
        $showTrace = $this->output->getVerbosity() >= \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_DEBUG;
        foreach ($errors as $i => $error) {
            ...
            if ($showTrace && !$e instanceof \PhpCsFixer\Linter\LintingException) {
                ...
                foreach ($stackTrace as $trace) {
                    if (isset($trace['class'], $trace['function']) && \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Command\Command::class === $trace['class'] && 'run' === $trace['function']) {
                        $this->output->writeln('      [ ... ]');
                        break;
                    }
    ...
    {
        return $this->isDecorated ? \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Formatter\OutputFormatter::escape($string) : $string;
    }
}
