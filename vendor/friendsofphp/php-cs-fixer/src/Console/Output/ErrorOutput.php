...
final class ErrorOutput
{
    ...
    public function listErrors($process, array $errors)
    {
        ...
        $showDetails = $this->output->getVerbosity() >= \_PhpScoper8e2d8a2760d1\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE;
        $showTrace = $this->output->getVerbosity() >= \_PhpScoper8e2d8a2760d1\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_DEBUG;
        foreach ($errors as $i => $error) {
            ...
            if ($showTrace && !$e instanceof \PhpCsFixer\Linter\LintingException) {
                ...
                foreach ($stackTrace as $trace) {
                    if (isset($trace['class'], $trace['function']) && \_PhpScoper8e2d8a2760d1\Symfony\Component\Console\Command\Command::class === $trace['class'] && 'run' === $trace['function']) {
                        $this->output->writeln('      [ ... ]');
                        break;
                    }
    ...
    {
        return $this->isDecorated ? \_PhpScoper8e2d8a2760d1\Symfony\Component\Console\Formatter\OutputFormatter::escape($string) : $string;
    }
}
