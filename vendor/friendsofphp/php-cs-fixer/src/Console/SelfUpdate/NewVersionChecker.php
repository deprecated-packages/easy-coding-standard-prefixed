...
use _PhpScoperb6d4bd368bd9\Composer\Semver\Comparator;
use _PhpScoperb6d4bd368bd9\Composer\Semver\Semver;
...
final class NewVersionChecker implements \PhpCsFixer\Console\SelfUpdate\NewVersionCheckerInterface
{
    ...
    public function __construct(\PhpCsFixer\Console\SelfUpdate\GithubClientInterface $githubClient)
    {
        ...
        $this->versionParser = new \_PhpScoperb6d4bd368bd9\Composer\Semver\VersionParser();
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function getLatestVersionOfMajor($majorVersion)
    {
        ...
        foreach ($this->availableVersions as $availableVersion) {
            if (\_PhpScoperb6d4bd368bd9\Composer\Semver\Semver::satisfies($availableVersion, $semverConstraint)) {
    ...
    public function compareVersions($versionA, $versionB)
    {
        ...
        if (\_PhpScoperb6d4bd368bd9\Composer\Semver\Comparator::lessThan($versionA, $versionB)) {
            return -1;
        }
        if (\_PhpScoperb6d4bd368bd9\Composer\Semver\Comparator::greaterThan($versionA, $versionB)) {
            return 1;
        }
        return 0;
    }
    private function retrieveAvailableVersions()
    {
        ...
        foreach ($this->githubClient->getTags() as $tag) {
            ...
            try {
                ...
                if ('stable' === \_PhpScoperb6d4bd368bd9\Composer\Semver\VersionParser::parseStability($version)) {
        ...
        $this->availableVersions = \_PhpScoperb6d4bd368bd9\Composer\Semver\Semver::rsort($this->availableVersions);
    }
}
