...
use _PhpScoper5e5ebcdbebdf\Composer\Semver\Comparator;
use _PhpScoper5e5ebcdbebdf\Composer\Semver\Semver;
...
final class NewVersionChecker implements \PhpCsFixer\Console\SelfUpdate\NewVersionCheckerInterface
{
    ...
    public function __construct(\PhpCsFixer\Console\SelfUpdate\GithubClientInterface $githubClient)
    {
        ...
        $this->versionParser = new \_PhpScoper5e5ebcdbebdf\Composer\Semver\VersionParser();
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function getLatestVersionOfMajor($majorVersion)
    {
        ...
        foreach ($this->availableVersions as $availableVersion) {
            if (\_PhpScoper5e5ebcdbebdf\Composer\Semver\Semver::satisfies($availableVersion, $semverConstraint)) {
    ...
    public function compareVersions($versionA, $versionB)
    {
        ...
        if (\_PhpScoper5e5ebcdbebdf\Composer\Semver\Comparator::lessThan($versionA, $versionB)) {
            return -1;
        }
        if (\_PhpScoper5e5ebcdbebdf\Composer\Semver\Comparator::greaterThan($versionA, $versionB)) {
            return 1;
        }
        return 0;
    }
    private function retrieveAvailableVersions()
    {
        ...
        foreach ($this->githubClient->getTags() as $tag) {
            ...
            try {
                ...
                if ('stable' === \_PhpScoper5e5ebcdbebdf\Composer\Semver\VersionParser::parseStability($version)) {
        ...
        $this->availableVersions = \_PhpScoper5e5ebcdbebdf\Composer\Semver\Semver::rsort($this->availableVersions);
    }
}
