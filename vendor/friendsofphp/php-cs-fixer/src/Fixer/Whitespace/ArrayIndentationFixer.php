...
final class ArrayIndentationFixer extends \PhpCsFixer\AbstractFixer implements \PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
        ...
        foreach ($this->findArrays($tokens) as $array) {
            ...
            foreach ($array['braces'] as $index => $braces) {
                ...
                if ($this->newlineIsInArrayScope($tokens, $index, $array)) {
                    $content = \PhpCsFixer\Preg::replace('/(\\R+)[\\t ]*$/', '$1' . $arrayIndent . \str_repeat($this->whitespacesConfig->getIndent(), $currentIndentLevel), $token->getContent());
                    $previousLineInitialIndent = $this->extractIndent($token->getContent());
                    $previousLineNewIndent = $this->extractIndent($content);
                } else {
                    $content = \PhpCsFixer\Preg::replace('/(\\R)' . \preg_quote($previousLineInitialIndent, '/') . '([\\t ]*)$/', '$1' . $previousLineNewIndent . '$2', $token->getContent());
                }
    ...
    private function findArrays(\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $arrays = [];
    ...
    private function findArrayTokenRanges(\PhpCsFixer\Tokenizer\Tokens $tokens, $from, $to)
    {
        $arrayTokenRanges = [];
        $currentArray = null;
        for ($index = $from; $index <= $to; ++$index) {
            ...
            if (null === $currentArray && $token->isGivenKind([\T_ARRAY, \PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_OPEN])) {
                if ($token->isGivenKind(\T_ARRAY)) {
                    $index = $tokens->getNextTokenOfKind($index, ['(']);
                }
                $currentArray = ['start' => $index, 'end' => $tokens->findBlockEnd($tokens[$index]->equals('(') ? \PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE : \PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_ARRAY_SQUARE_BRACE, $index), 'ignored_tokens_ranges' => []];
                continue;
            }
            if (null !== $currentArray && ($token->equals('(') && !$tokens[$tokens->getPrevMeaningfulToken($index)]->isGivenKind(\T_ARRAY) || $token->equals('{'))) {
                $endIndex = $tokens->findBlockEnd($token->equals('{') ? \PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE : \PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $index);
    ...
    private function computeArrayLineSignificantBraces(\PhpCsFixer\Tokenizer\Tokens $tokens, array $array)
    {
        $braces = [];
    ...
    private function getLineSignificantBraces(\PhpCsFixer\Tokenizer\Tokens $tokens, $index, array $array)
    {
        $deltas = [];
        for (++$index; $index <= $array['end']; ++$index) {
            ...
            if ($token->equals(')')) {
                $openBraceIndex = $tokens->findBlockStart(\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $index);
                if (!$tokens[$tokens->getPrevMeaningfulToken($openBraceIndex)]->isGivenKind(\T_ARRAY)) {
                    continue;
                }
            }
            if ($token->equalsAny(['(', [\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_OPEN]])) {
                $deltas[] = 1;
                continue;
            }
    ...
    private function isClosingLineWithMeaningfulContent(\PhpCsFixer\Tokenizer\Tokens $tokens, $newLineIndex)
    {
        $nextMeaningfulIndex = $tokens->getNextMeaningfulToken($newLineIndex);
        return !$tokens[$nextMeaningfulIndex]->equalsAny([')', [\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_CLOSE]]);
    }
    ...
    private function newlineIsInArrayScope(\PhpCsFixer\Tokenizer\Tokens $tokens, $index, array $array)
    {
