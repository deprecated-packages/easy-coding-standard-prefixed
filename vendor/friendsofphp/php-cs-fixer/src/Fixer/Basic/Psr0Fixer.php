 ...
 */
final class Psr0Fixer extends \PhpCsFixer\AbstractPsrAutoloadingFixer implements \PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function applyFix(\SplFileInfo $file, \PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        if (\false !== $namespace) {
            ...
            if ($classyName !== $filename) {
                $tokens[$classyIndex] = new \PhpCsFixer\Tokenizer\Token([\T_STRING, $filename]);
            }
            ...
            if ($normClass !== $filename && \strtolower($normClass) === \strtolower($filename)) {
                $tokens[$classyIndex] = new \PhpCsFixer\Tokenizer\Token([\T_STRING, \str_replace('/', '_', $filename)]);
            }
        }
    }
    /**
