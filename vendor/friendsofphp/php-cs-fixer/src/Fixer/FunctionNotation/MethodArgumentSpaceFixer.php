 ...
 */
final class MethodArgumentSpaceFixer extends \PhpCsFixer\AbstractFixer implements \PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface, \PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    /**
     ...
     * @param int $index
     */
    public function fixSpace(\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
...
\_PhpScoper3e8786a75afe\sample(<<<EOD
foo
EOD
, 'bar');
...
, new \PhpCsFixer\FixerDefinition\VersionSpecification(70300), ['after_heredoc' => \true])]);
    ...
    {
        return new \PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('keep_multiple_spaces_after_comma', 'Whether keep multiple spaces after comma.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption(), (new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('ensure_fully_multiline', 'ensure every argument of a multiline argument list is on its own line'))->setAllowedTypes(['bool'])->setDefault(\false)->setDeprecationMessage('Use option `on_multiline` instead.')->getOption(), (new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('on_multiline', 'Defines how to handle function arguments lists that contain newlines.'))->setAllowedValues(['ignore', 'ensure_single_line', 'ensure_fully_multiline'])->setDefault('ignore')->getOption(), (new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('after_heredoc', 'Whether the whitespace between heredoc end and comma should be removed.'))->setAllowedTypes(['bool'])->setDefault(\false)->setNormalizer(static function (\_PhpScoper3e8786a75afe\Symfony\Component\OptionsResolver\Options $options, $value) {
    ...
    private function ensureSingleLine(\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        $content = \PhpCsFixer\Preg::replace('/\\R[ \\t]*/', '', $tokens[$index]->getContent());
        if ('' !== $content) {
     ...
     * @param int $index
     */
    private function fixSpace2(\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     * @return bool
     */
    private function isNewline(\PhpCsFixer\Tokenizer\Token $token)
    {
