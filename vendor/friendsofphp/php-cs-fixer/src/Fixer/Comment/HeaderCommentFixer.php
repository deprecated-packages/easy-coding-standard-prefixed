...
', ['header' => 'Made with love.', 'comment_type' => 'comment', 'location' => 'after_declare_strict'])]);
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('header', 'Proper header content.'))->setAllowedTypes(['string'])->setNormalizer(static function (\_PhpScoper5465fda93cc7\Symfony\Component\OptionsResolver\Options $options, $value) use($fixerName) {
            if ('' === \trim($value)) {
                return '';
            }
     ...
     * @return int
     */
    private function findHeaderCommentInsertionIndex(\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
    ...
    private function fixWhiteSpaceAroundHeader(\PhpCsFixer\Tokenizer\Tokens $tokens, $headerIndex)
    {
        ...
        if ($headerIndex === \count($tokens) - 1) {
            ...
            if ($lineBreakCount < $expectedLineCount) {
                ...
                if ($tokens[$headerIndex + 1]->isWhitespace()) {
                    $tokens[$headerIndex + 1] = new \PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $lineEnding . $lineEnding]);
                }
            }
        }
     ...
     * @return int
     */
    private function getLineBreakCount(\PhpCsFixer\Tokenizer\Tokens $tokens, $indexStart, $indexEnd)
    {
        $lineCount = 0;
