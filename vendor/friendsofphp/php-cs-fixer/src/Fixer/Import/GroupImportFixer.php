...
final class GroupImportFixer extends AbstractFixer
{
    ...
    private function getSameNamespaces($tokens)
    {
        ...
        $allNamespaceAndType = \array_map(function (NamespaceUseAnalysis $useDeclaration) {
    ...
    private function addGroupUseStatements(array $statements, $tokens)
    {
        ...
        foreach ($statements as $index => $useDeclaration) {
            if ($this->areDeclarationsDifferent($currentUseDeclaration, $useDeclaration)) {
                ...
                $newTokens = [new Token(','), new Token([\T_WHITESPACE, ' '])];
    ...
    private function createNewGroup($tokens, $insertIndex, $useDeclaration, $currentNamespace)
    {
        ...
        $newTokens = [new Token([\T_USE, 'use']), new Token([\T_WHITESPACE, ' '])];
        if ($useDeclaration->isFunction() || $useDeclaration->isConstant()) {
            $importStatementParams = $useDeclaration->isFunction() ? [CT::T_FUNCTION_IMPORT, 'function'] : [CT::T_CONST_IMPORT, 'const'];
