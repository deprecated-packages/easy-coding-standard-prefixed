...
final class ClassAttributesSeparationFixer extends \PhpCsFixer\AbstractFixer implements \PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface, \PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    ...
    {
        return new \PhpCsFixer\FixerDefinition\FixerDefinition('Class, trait and interface elements must be separated with one blank line.', [new \PhpCsFixer\FixerDefinition\CodeSample('<?php
final class Sample
{
...
', ['elements' => ['property']]), new \PhpCsFixer\FixerDefinition\CodeSample('<?php
class Sample
{
    const A = 1;
...
', ['elements' => ['const']])]);
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \PhpCsFixer\FixerConfiguration\FixerOptionBuilder('elements', \sprintf('List of classy elements; \'%s\'.', \implode("', '", $types))))->setAllowedTypes(['array'])->setAllowedValues([new \PhpCsFixer\FixerConfiguration\AllowedValueSubset($types)])->setDefault(['const', 'method', 'property'])->getOption()]);
    }
    /**
     ...
     */
    private function fixSpaceBelowClassElement(\PhpCsFixer\Tokenizer\Tokens $tokens, $classEndIndex, $elementEndIndex)
    {
     ...
     * @param int $elementEndIndex
     */
    private function fixSpaceBelowClassMethod(\PhpCsFixer\Tokenizer\Tokens $tokens, $classEndIndex, $elementEndIndex)
    {
     ...
     */
    private function fixSpaceAboveClassElement(\PhpCsFixer\Tokenizer\Tokens $tokens, $classStartIndex, $elementIndex)
    {
        static $methodAttr = [\T_PRIVATE, \T_PROTECTED, \T_PUBLIC, \T_ABSTRACT, \T_FINAL, \T_STATIC, \T_STRING, \T_NS_SEPARATOR, \T_VAR, \PhpCsFixer\Tokenizer\CT::T_NULLABLE_TYPE];
        $nonWhiteAbove = null;
