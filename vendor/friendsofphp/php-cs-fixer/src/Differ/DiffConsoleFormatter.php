        ...
        return \sprintf($template, \implode(\PHP_EOL, \array_map(static function ($line) use($isDecorated, $lineTemplate) {
            if ($isDecorated) {
                ...
                $line = \PhpCsFixer\Preg::replaceCallback(['/^(\\+.*)/', '/^(\\-.*)/', '/^(@.*)/'], function ($matches) {
                    ...
                    return \sprintf('<fg=%s>%s</fg=%s>', $colour, \_PhpScoper589e8c04a9ef\Symfony\Component\Console\Formatter\OutputFormatter::escape($matches[0]), $colour);
                }, $line, 1, $count);
                if (0 === $count) {
                    $line = \_PhpScoper589e8c04a9ef\Symfony\Component\Console\Formatter\OutputFormatter::escape($line);
                }
            }
