...
final class AccessibleObject
{
    ...
    {
        @\trigger_error(\sprintf('The "%s" class is deprecated and will be removed in 3.0 version. Use "php-cs-fixer/accessible-object" package instead.', __CLASS__), \E_USER_DEPRECATED);
        $this->object = $object;
    ...
    {
        if (!\property_exists($this->object, $name)) {
            throw new \LogicException(\sprintf('Cannot set non existing property %s->%s = %s.', \get_class($this->object), $name, \var_export($value, \true)));
        }
        $property = $this->reflection->getProperty($name);
        $property->setAccessible(\true);
