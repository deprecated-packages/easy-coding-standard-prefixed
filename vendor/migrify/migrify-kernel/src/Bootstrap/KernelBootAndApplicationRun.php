...
namespace _PhpScoperb09c3ec8e01a\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoperb09c3ec8e01a\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoperb09c3ec8e01a\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoperb09c3ec8e01a\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoperb09c3ec8e01a\Symfony\Component\Console\Application::class);
            throw new \_PhpScoperb09c3ec8e01a\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoperb09c3ec8e01a\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoperb09c3ec8e01a\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoperb09c3ec8e01a\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoperb09c3ec8e01a\Migrify\MigrifyKernel\Exception\BootException($message);
        }
