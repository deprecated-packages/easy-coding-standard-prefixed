...
namespace _PhpScoperbcc0395698f8\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoperbcc0395698f8\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoperbcc0395698f8\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoperbcc0395698f8\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoperbcc0395698f8\Symfony\Component\Console\Application::class);
            throw new \_PhpScoperbcc0395698f8\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoperbcc0395698f8\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoperbcc0395698f8\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoperbcc0395698f8\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoperbcc0395698f8\Migrify\MigrifyKernel\Exception\BootException($message);
        }
