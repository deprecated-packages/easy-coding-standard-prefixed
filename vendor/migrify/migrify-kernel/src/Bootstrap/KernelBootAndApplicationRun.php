...
namespace _PhpScoperc753ccca5a0c\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoperc753ccca5a0c\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoperc753ccca5a0c\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoperc753ccca5a0c\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoperc753ccca5a0c\Symfony\Component\Console\Application::class);
            throw new \_PhpScoperc753ccca5a0c\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoperc753ccca5a0c\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoperc753ccca5a0c\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoperc753ccca5a0c\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoperc753ccca5a0c\Migrify\MigrifyKernel\Exception\BootException($message);
        }
