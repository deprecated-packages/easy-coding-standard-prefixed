...
namespace _PhpScoper8e2d8a2760d1\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoper8e2d8a2760d1\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoper8e2d8a2760d1\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoper8e2d8a2760d1\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoper8e2d8a2760d1\Symfony\Component\Console\Application::class);
            throw new \_PhpScoper8e2d8a2760d1\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoper8e2d8a2760d1\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoper8e2d8a2760d1\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoper8e2d8a2760d1\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoper8e2d8a2760d1\Migrify\MigrifyKernel\Exception\BootException($message);
        }
