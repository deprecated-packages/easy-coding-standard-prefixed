...
namespace _PhpScoper6207116d4311\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoper6207116d4311\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoper6207116d4311\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoper6207116d4311\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoper6207116d4311\Symfony\Component\Console\Application::class);
            throw new \_PhpScoper6207116d4311\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoper6207116d4311\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoper6207116d4311\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoper6207116d4311\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoper6207116d4311\Migrify\MigrifyKernel\Exception\BootException($message);
        }
