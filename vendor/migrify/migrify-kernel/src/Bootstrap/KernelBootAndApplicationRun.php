...
namespace _PhpScoperedc2e0c967db\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoperedc2e0c967db\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoperedc2e0c967db\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoperedc2e0c967db\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoperedc2e0c967db\Symfony\Component\Console\Application::class);
            throw new \_PhpScoperedc2e0c967db\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoperedc2e0c967db\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoperedc2e0c967db\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoperedc2e0c967db\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoperedc2e0c967db\Migrify\MigrifyKernel\Exception\BootException($message);
        }
