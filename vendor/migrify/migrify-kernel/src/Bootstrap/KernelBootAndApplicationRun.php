...
namespace _PhpScopera4be459e5e3d\Migrify\MigrifyKernel\Bootstrap;

use _PhpScopera4be459e5e3d\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScopera4be459e5e3d\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScopera4be459e5e3d\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScopera4be459e5e3d\Symfony\Component\Console\Application::class);
            throw new \_PhpScopera4be459e5e3d\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScopera4be459e5e3d\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScopera4be459e5e3d\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScopera4be459e5e3d\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScopera4be459e5e3d\Migrify\MigrifyKernel\Exception\BootException($message);
        }
