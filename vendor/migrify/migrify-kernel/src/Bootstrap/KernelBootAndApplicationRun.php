...
namespace _PhpScoperecb978830f1e\Migrify\MigrifyKernel\Bootstrap;

use _PhpScoperecb978830f1e\Migrify\MigrifyKernel\Exception\BootException;
...
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \_PhpScoperecb978830f1e\Symfony\Component\HttpKernel\KernelInterface
    {
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\_PhpScoperecb978830f1e\Symfony\Component\Console\Application::class);
        if ($application === null) {
            $message = \sprintf('Application class %s was not found. Make it public in the config.', \_PhpScoperecb978830f1e\Symfony\Component\Console\Application::class);
            throw new \_PhpScoperecb978830f1e\Migrify\MigrifyKernel\Exception\BootException($message);
        }
        exit($application->run());
    }
    private function setExtraConfigs(\_PhpScoperecb978830f1e\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
    ...
    {
        if (!\is_a($kernelClass, \_PhpScoperecb978830f1e\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \_PhpScoperecb978830f1e\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \_PhpScoperecb978830f1e\Migrify\MigrifyKernel\Exception\BootException($message);
        }
