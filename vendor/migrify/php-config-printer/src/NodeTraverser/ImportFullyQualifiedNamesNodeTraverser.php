...
namespace _PhpScoper3639953bb9e5\Migrify\PhpConfigPrinter\NodeTraverser;

use _PhpScoper3639953bb9e5\Migrify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor;
use _PhpScoper3639953bb9e5\Nette\Utils\Strings;
use _PhpScoper3639953bb9e5\PhpParser\BuilderFactory;
use _PhpScoper3639953bb9e5\PhpParser\Node;
use _PhpScoper3639953bb9e5\PhpParser\Node\Name;
use _PhpScoper3639953bb9e5\PhpParser\Node\Stmt\Nop;
use _PhpScoper3639953bb9e5\PhpParser\Node\Stmt\Use_;
use _PhpScoper3639953bb9e5\PhpParser\NodeTraverser;
final class ImportFullyQualifiedNamesNodeTraverser
{
    ...
    public function __construct(\_PhpScoper3639953bb9e5\Migrify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor $importFullyQualifiedNamesNodeVisitor, \_PhpScoper3639953bb9e5\PhpParser\BuilderFactory $builderFactory)
    {
    ...
    private function addUseImportsToNamespace(array $nodes, array $nameImports) : array
    {
        ...
        $useImports[] = new \_PhpScoper3639953bb9e5\PhpParser\Node\Stmt\Nop();
    ...
    {
        $nodeTraverser = new \_PhpScoper3639953bb9e5\PhpParser\NodeTraverser();
    ...
    private function createUses(array $nameImports) : array
    {
        ...
        foreach ($nameImports as $nameImport) {
            $shortNameImport = \_PhpScoper3639953bb9e5\Nette\Utils\Strings::after($nameImport, '\\', -1);
            if (\function_exists($nameImport) || $shortNameImport === 'ref') {
                $useBuilder = $this->builderFactory->useFunction(new \_PhpScoper3639953bb9e5\PhpParser\Node\Name($nameImport));
                $useImports[] = $useBuilder->getNode();
            } else {
                $useBuilder = $this->builderFactory->use(new \_PhpScoper3639953bb9e5\PhpParser\Node\Name($nameImport));
                $useImports[] = $useBuilder->getNode();
            }
        }
        return $useImports;
