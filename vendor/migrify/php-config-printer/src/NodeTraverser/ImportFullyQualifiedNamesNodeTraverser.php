...
namespace _PhpScoper4f985154d5a0\Migrify\PhpConfigPrinter\NodeTraverser;

use _PhpScoper4f985154d5a0\Migrify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor;
use _PhpScoper4f985154d5a0\Nette\Utils\Strings;
use _PhpScoper4f985154d5a0\PhpParser\BuilderFactory;
...
use _PhpScoper4f985154d5a0\PhpParser\Node\Stmt\Use_;
use _PhpScoper4f985154d5a0\PhpParser\NodeTraverser;
final class ImportFullyQualifiedNamesNodeTraverser
{
    ...
    public function __construct(\_PhpScoper4f985154d5a0\Migrify\PhpConfigPrinter\NodeVisitor\ImportFullyQualifiedNamesNodeVisitor $importFullyQualifiedNamesNodeVisitor, \_PhpScoper4f985154d5a0\PhpParser\BuilderFactory $builderFactory)
    {
    ...
    private function addUseImportsToNamespace(array $nodes, array $nameImports) : array
    {
        ...
        $useImports[] = new \_PhpScoper4f985154d5a0\PhpParser\Node\Stmt\Nop();
    ...
    {
        $nodeTraverser = new \_PhpScoper4f985154d5a0\PhpParser\NodeTraverser();
    ...
    private function createUses(array $nameImports) : array
    {
        ...
        foreach ($nameImports as $nameImport) {
            $shortNameImport = \_PhpScoper4f985154d5a0\Nette\Utils\Strings::after($nameImport, '\\', -1);
            if (\function_exists($nameImport) || $shortNameImport === 'ref') {
                $useBuilder = $this->builderFactory->useFunction(new \_PhpScoper4f985154d5a0\PhpParser\Node\Name($nameImport));
                $useImports[] = $useBuilder->getNode();
            } else {
                $useBuilder = $this->builderFactory->use(new \_PhpScoper4f985154d5a0\PhpParser\Node\Name($nameImport));
                $useImports[] = $useBuilder->getNode();
            }
        }
        return $useImports;
