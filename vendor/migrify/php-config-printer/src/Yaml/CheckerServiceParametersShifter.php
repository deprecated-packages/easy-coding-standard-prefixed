...
namespace _PhpScoperb6d4bd368bd9\Migrify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    public function __construct()
    {
        ...
        $serviceKeywordsProperty = (new \ReflectionClass(\_PhpScoperb6d4bd368bd9\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class))->getStaticProperties()['serviceKeywords'];
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoperb6d4bd368bd9\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoperb6d4bd368bd9\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoperb6d4bd368bd9\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoperb6d4bd368bd9\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoperb6d4bd368bd9\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
