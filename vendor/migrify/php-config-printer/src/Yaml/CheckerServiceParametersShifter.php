...
namespace _PhpScoperddde3ba4aebc\Migrify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    public function __construct()
    {
        ...
        $serviceKeywordsProperty = (new \ReflectionClass(\_PhpScoperddde3ba4aebc\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class))->getStaticProperties()['serviceKeywords'];
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoperddde3ba4aebc\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoperddde3ba4aebc\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoperddde3ba4aebc\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoperddde3ba4aebc\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoperddde3ba4aebc\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
