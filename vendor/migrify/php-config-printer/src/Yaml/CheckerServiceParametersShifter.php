...
namespace _PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    public function __construct()
    {
        ...
        $serviceKeywordsProperty = (new \ReflectionClass(\_PhpScoperedc2e0c967db\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class))->getStaticProperties()['serviceKeywords'];
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoperedc2e0c967db\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoperedc2e0c967db\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoperedc2e0c967db\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoperedc2e0c967db\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function correctHeader(string $checker, array $serviceDefinition) : array
    {
        ...
        if ($checker !== \PhpCsFixer\Fixer\Comment\HeaderCommentFixer::class) {
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoperedc2e0c967db\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
