...
namespace _PhpScopera749ac204cd2\Migrify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    public function __construct()
    {
        ...
        $serviceKeywordsProperty = (new \ReflectionClass(\_PhpScopera749ac204cd2\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class))->getStaticProperties()['serviceKeywords'];
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScopera749ac204cd2\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScopera749ac204cd2\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScopera749ac204cd2\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScopera749ac204cd2\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScopera749ac204cd2\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
