...
final class ArgumentsServiceOptionKeyYamlToPhpFactory implements \_PhpScoper21763e6c7ac4\Migrify\PhpConfigPrinter\Contract\Converter\ServiceOptionsKeyYamlToPhpFactoryInterface
{
    /**
     * @var ArgsNodeFactory
     */
    ...
    public function decorateServiceMethodCall($key, $yaml, $values, \_PhpScoper21763e6c7ac4\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoper21763e6c7ac4\PhpParser\Node\Expr\MethodCall
    {
        if (!$this->hasNamedArguments($yaml)) {
            ...
            return new \_PhpScoper21763e6c7ac4\PhpParser\Node\Expr\MethodCall($methodCall, 'args', $args);
        }
        foreach ($yaml as $key => $value) {
            $args = $this->argsNodeFactory->createFromValues([$key, $value], \false, \true);
            $methodCall = new \_PhpScoper21763e6c7ac4\PhpParser\Node\Expr\MethodCall($methodCall, 'arg', $args);
        }
        return $methodCall;
    }
    public function isMatch($key, $values) : bool
    {
        return $key === \_PhpScoper21763e6c7ac4\Migrify\PhpConfigPrinter\ValueObject\YamlServiceKey::ARGUMENTS;
    }
    private function hasNamedArguments(array $data) : bool
    {
        ...
        foreach (\array_keys($data) as $key) {
            if (!\_PhpScoper21763e6c7ac4\Nette\Utils\Strings::startsWith((string) $key, '$')) {
                return \false;
            }
        }
        return \true;
