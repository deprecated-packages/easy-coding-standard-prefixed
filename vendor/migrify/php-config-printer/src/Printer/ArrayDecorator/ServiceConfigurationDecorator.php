...
namespace _PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\Printer\ArrayDecorator;

use _PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory;
use _PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver;
use _PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\ValueObject\Option;
...
final class ServiceConfigurationDecorator
{
    ...
    public function __construct(\_PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\Reflection\ConstantNameFromValueResolver $constantNameFromValueResolver, \_PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\NodeFactory\NewValueObjectFactory $newValueObjectFactory, \Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider)
    {
    ...
    private function decorateValueObject(object $value) : \_PhpScoperf77bffce0320\PhpParser\Node\Expr\FuncCall
    {
        $new = $this->newValueObjectFactory->create($value);
        $args = [new \_PhpScoperf77bffce0320\PhpParser\Node\Arg($new)];
        $functionName = $this->parameterProvider->provideStringParameter(\_PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\ValueObject\Option::INLINE_VALUE_OBJECT_FUNC_CALL_NAME);
    ...
    private function decorateValueObjects(array $values) : \_PhpScoperf77bffce0320\PhpParser\Node\Expr\FuncCall
    {
        $arrayItems = [];
        foreach ($values as $value) {
            $new = $this->newValueObjectFactory->create($value);
            $arrayItems[] = new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\ArrayItem($new);
        }
        $array = new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\Array_($arrayItems);
        $args = [new \_PhpScoperf77bffce0320\PhpParser\Node\Arg($array)];
        $functionName = $this->parameterProvider->provideStringParameter(\_PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\ValueObject\Option::INLINE_VALUE_OBJECTS_FUNC_CALL_NAME);
        return new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\FuncCall(new \_PhpScoperf77bffce0320\PhpParser\Node\Name\FullyQualified($functionName), $args);
    }
