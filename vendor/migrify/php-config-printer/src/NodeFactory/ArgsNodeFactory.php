...
use _PhpScoperedc2e0c967db\Migrify\ConfigTransformer\ValueObject\FunctionName;
...
final class ArgsNodeFactory
{
    ...
    public function __construct(\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\Contract\SymfonyVersionFeatureGuardInterface $symfonyVersionFeatureGuard)
    {
    ...
    public function createFromValuesAndWrapInArray($values) : array
    {
        if (\is_array($values)) {
            ...
            $items = [new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\ArrayItem($expr)];
            $array = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Array_($items);
        }
        return [new \_PhpScoperedc2e0c967db\PhpParser\Node\Arg($array)];
    }
    /**
     * @return Arg[]
     */
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : array
    {
        ...
        if ($values instanceof \_PhpScoperedc2e0c967db\PhpParser\Node) {
            if ($values instanceof \_PhpScoperedc2e0c967db\PhpParser\Node\Arg) {
                return [$values];
            }
            if ($values instanceof \_PhpScoperedc2e0c967db\PhpParser\Node\Expr) {
                return [new \_PhpScoperedc2e0c967db\PhpParser\Node\Arg($values)];
            }
        }
        if (\is_string($values)) {
            $expr = $this->resolveExpr($values);
            return [new \_PhpScoperedc2e0c967db\PhpParser\Node\Arg($expr)];
        }
        throw new \_PhpScoperedc2e0c967db\Migrify\MigrifyKernel\Exception\NotImplementedYetException();
    }
    public function resolveExpr($value, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : \_PhpScoperedc2e0c967db\PhpParser\Node\Expr
    {
        if (\is_string($value)) {
            return $this->resolveStringExpr($value, $skipServiceReference, $skipClassesToConstantReference);
        }
        if ($value instanceof \_PhpScoperedc2e0c967db\PhpParser\Node\Expr) {
            return $value;
        }
        ...
        if (\is_array($value)) {
            ...
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Array_($arrayItems);
        }
        return \_PhpScoperedc2e0c967db\PhpParser\BuilderHelpers::normalizeValue($value);
    }
    private function resolveServiceReferenceExpr(string $value, bool $skipServiceReference, string $functionName) : \_PhpScoperedc2e0c967db\PhpParser\Node\Expr
    {
        $value = \ltrim($value, '@');
    ...
    private function resolveExprFromArray(array $values) : \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Array_
    {
        $arrayItems = [];
        foreach ($values as $key => $value) {
            ...
            if (!\is_int($key)) {
                ...
                $arrayItem = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\ArrayItem($expr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\ArrayItem($expr);
            }
            $arrayItems[] = $arrayItem;
        }
        return new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Array_($arrayItems);
    }
    private function createServiceReferenceFromTaggedValue(\_PhpScoperedc2e0c967db\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \_PhpScoperedc2e0c967db\PhpParser\Node\Expr
    {
        ...
        if ($taggedValue->getTag() === self::TAG_RETURNS_CLONE) {
            ...
            $functionName = \_PhpScoperedc2e0c967db\Migrify\ConfigTransformer\ValueObject\FunctionName::INLINE_SERVICE;
        } else {
            ...
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\FuncCall(new \_PhpScoperedc2e0c967db\PhpParser\Node\Name($taggedValue->getTag()), $args);
        }
        $funcCall = $this->resolveServiceReferenceExpr($serviceName, \false, $functionName);
        if ($shouldWrapInArray) {
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Array_([new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\ArrayItem($funcCall)]);
        }
        return $funcCall;
    }
    private function resolveStringExpr(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \_PhpScoperedc2e0c967db\PhpParser\Node\Expr
    {
        if ($value === '') {
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Scalar\String_($value);
        }
        ...
        if (\in_array($value, ["\r", "\n", "\r\n"], \true)) {
            $string = new \_PhpScoperedc2e0c967db\PhpParser\Node\Scalar\String_($value);
            $string->setAttribute('kind', \_PhpScoperedc2e0c967db\PhpParser\Node\Scalar\String_::KIND_DOUBLE_QUOTED);
            return $string;
        }
        $value = \ltrim($value, '\\');
        ...
        if (\_PhpScoperedc2e0c967db\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            $args = $this->createFromValues($value);
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\FuncCall(new \_PhpScoperedc2e0c967db\PhpParser\Node\Name\FullyQualified(\_PhpScoperedc2e0c967db\Migrify\ConfigTransformer\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\_PhpScoperedc2e0c967db\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    private function resolveArrayItems(array $value, bool $skipClassesToConstantReference) : array
    {
        ...
        foreach ($value as $nestedKey => $nestedValue) {
            ...
            if (!\is_int($nestedKey) || $nestedKey !== $naturalKey) {
                ...
                $arrayItem = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\ArrayItem($valueExpr, $keyExpr);
            } else {
                $arrayItem = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\ArrayItem($valueExpr);
            }
            $arrayItems[] = $arrayItem;
            ++$naturalKey;
        }
    ...
    {
        if ($this->symfonyVersionFeatureGuard->isAtLeastSymfonyVersion(\_PhpScoperedc2e0c967db\Migrify\ConfigTransformer\ValueObject\SymfonyVersionFeature::REF_OVER_SERVICE)) {
            return \_PhpScoperedc2e0c967db\Migrify\ConfigTransformer\ValueObject\FunctionName::SERVICE;
        }
        return \_PhpScoperedc2e0c967db\Migrify\ConfigTransformer\ValueObject\FunctionName::REF;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \_PhpScoperedc2e0c967db\Nette\Utils\Strings::match($value, '#\\.(twig|html|xml)$#');
    }
