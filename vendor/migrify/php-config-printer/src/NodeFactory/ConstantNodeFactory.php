...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoper967c4b7e296e\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper967c4b7e296e\PhpParser\Node\Expr
    {
        if (\_PhpScoper967c4b7e296e\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper967c4b7e296e\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper967c4b7e296e\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper967c4b7e296e\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper967c4b7e296e\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper967c4b7e296e\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
