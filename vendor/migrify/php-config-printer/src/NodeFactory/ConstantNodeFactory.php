...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScopercb576ca159b5\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopercb576ca159b5\PhpParser\Node\Expr
    {
        if (\_PhpScopercb576ca159b5\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopercb576ca159b5\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopercb576ca159b5\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopercb576ca159b5\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopercb576ca159b5\PhpParser\Node\Expr\ConstFetch(new \_PhpScopercb576ca159b5\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
