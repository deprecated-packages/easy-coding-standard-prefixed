...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoperbcc0395698f8\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperbcc0395698f8\PhpParser\Node\Expr
    {
        if (\_PhpScoperbcc0395698f8\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperbcc0395698f8\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperbcc0395698f8\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperbcc0395698f8\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperbcc0395698f8\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperbcc0395698f8\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
