...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoper279cf54b77ad\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper279cf54b77ad\PhpParser\Node\Expr
    {
        if (\_PhpScoper279cf54b77ad\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper279cf54b77ad\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper279cf54b77ad\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper279cf54b77ad\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper279cf54b77ad\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper279cf54b77ad\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
