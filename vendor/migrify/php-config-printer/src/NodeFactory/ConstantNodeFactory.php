...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoper2a8ad010dfbd\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper2a8ad010dfbd\PhpParser\Node\Expr
    {
        if (\_PhpScoper2a8ad010dfbd\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper2a8ad010dfbd\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper2a8ad010dfbd\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper2a8ad010dfbd\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper2a8ad010dfbd\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper2a8ad010dfbd\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
