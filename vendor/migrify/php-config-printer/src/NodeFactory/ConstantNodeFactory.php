...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScopercda2b863d098\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScopercda2b863d098\PhpParser\Node\Expr
    {
        if (\_PhpScopercda2b863d098\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScopercda2b863d098\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScopercda2b863d098\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScopercda2b863d098\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScopercda2b863d098\PhpParser\Node\Expr\ConstFetch(new \_PhpScopercda2b863d098\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
