...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoperfacc742d2745\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoperfacc742d2745\PhpParser\Node\Expr
    {
        if (\_PhpScoperfacc742d2745\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoperfacc742d2745\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoperfacc742d2745\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoperfacc742d2745\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoperfacc742d2745\PhpParser\Node\Expr\ConstFetch(new \_PhpScoperfacc742d2745\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
