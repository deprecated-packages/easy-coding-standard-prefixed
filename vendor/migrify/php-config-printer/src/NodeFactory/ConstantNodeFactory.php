...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoper0f5cd390c37a\Migrify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper0f5cd390c37a\PhpParser\Node\Expr
    {
        if (\_PhpScoper0f5cd390c37a\Nette\Utils\Strings::contains($value, '::')) {
            [$class, $constant] = \explode('::', $value);
            // not uppercase → probably not a constant
            if (\strtoupper($constant) !== $constant) {
                return null;
            }
            return new \_PhpScoper0f5cd390c37a\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper0f5cd390c37a\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper0f5cd390c37a\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper0f5cd390c37a\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper0f5cd390c37a\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
