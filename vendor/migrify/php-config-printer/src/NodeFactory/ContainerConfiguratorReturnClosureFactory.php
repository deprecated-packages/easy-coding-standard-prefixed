...
final class ContainerConfiguratorReturnClosureFactory
{
     ...
     */
    public function __construct(\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\PhpParser\NodeFactory\ConfiguratorClosureNodeFactory $configuratorClosureNodeFactory, array $caseConverters, array $nestedCaseConverters)
    {
    ...
    public function createFromYamlArray(array $arrayData) : \_PhpScoperedc2e0c967db\PhpParser\Node\Stmt\Return_
    {
    ...
    private function createInitializeAssign(string $variableName, string $methodName) : \_PhpScoperedc2e0c967db\PhpParser\Node\Stmt\Expression
    {
        $servicesVariable = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Variable($variableName);
        ...
        $assign = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Assign($servicesVariable, new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\MethodCall($containerConfiguratorVariable, $methodName));
        return new \_PhpScoperedc2e0c967db\PhpParser\Node\Stmt\Expression($assign);
    }
    private function createInitializeNode(string $key, array $nodes) : array
    {
        if ($key === \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            $nodes[] = $this->createInitializeAssign(\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\VariableName::SERVICES, \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::SERVICES);
        }
        if ($key === \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\YamlKey::PARAMETERS) {
            $nodes[] = $this->createInitializeAssign(\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\VariableName::PARAMETERS, \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::PARAMETERS);
        }
        return $nodes;
    }
}
