 ...
 */
final class ConfiguredServiceCaseConverter implements \_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
     * @var ArgsNodeFactory
     */
    ...
    {
        $valuesForArgs = [$key];
        if (isset($values[\_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
        $args = $this->argsNodeFactory->createFromValues($valuesForArgs);
        $methodCall = new \_PhpScoper239b374a39c8\PhpParser\Node\Expr\MethodCall(new \_PhpScoper239b374a39c8\PhpParser\Node\Expr\Variable(\_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
        $methodCall = $this->serviceOptionNodeFactory->convertServiceOptionsToNodes($values, $methodCall);
        $expression = new \_PhpScoper239b374a39c8\PhpParser\Node\Stmt\Expression($methodCall);
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if ($key === \_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
    ...
    {
        if (isset($values[\_PhpScoper239b374a39c8\Migrify\PhpConfigPrinter\ValueObject\YamlKey::ALIAS])) {
            return \true;
        }
        return \is_string($values) && \_PhpScoper239b374a39c8\Nette\Utils\Strings::startsWith($values, '@');
    }
}
