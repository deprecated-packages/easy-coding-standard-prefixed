 ...
 */
final class ConfiguredServiceCaseConverter implements \_PhpScoper666af036e800\Migrify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
     * @var ArgsNodeFactory
     */
    ...
    {
        $valuesForArgs = [$key];
        if (isset($values[\_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
        $args = $this->argsNodeFactory->createFromValues($valuesForArgs);
        $methodCall = new \_PhpScoper666af036e800\PhpParser\Node\Expr\MethodCall(new \_PhpScoper666af036e800\PhpParser\Node\Expr\Variable(\_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
        $methodCall = $this->serviceOptionNodeFactory->convertServiceOptionsToNodes($values, $methodCall);
        $expression = new \_PhpScoper666af036e800\PhpParser\Node\Stmt\Expression($methodCall);
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if ($key === \_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
    ...
    {
        if (isset($values[\_PhpScoper666af036e800\Migrify\PhpConfigPrinter\ValueObject\YamlKey::ALIAS])) {
            return \true;
        }
        return \is_string($values) && \_PhpScoper666af036e800\Nette\Utils\Strings::startsWith($values, '@');
    }
}
