 ...
 *     Some: Other <---
 */
final class AliasCaseConverter implements \_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function __construct(\_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory, \_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory $serviceOptionNodeFactory)
    {
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoper229e8121cf9f\PhpParser\Node\Stmt\Expression
    {
        ...
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $argValues[] = $values[\_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS] ?? $values;
            $args = $this->argsNodeFactory->createFromValues($argValues, \true);
            $methodCall = new \_PhpScoper229e8121cf9f\PhpParser\Node\Expr\MethodCall($servicesVariable, \_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoper229e8121cf9f\PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \_PhpScoper229e8121cf9f\Nette\Utils\Strings::before($key, ' $');
        if ($fullClassName !== null) {
            $methodCall = $this->createAliasNode($key, $fullClassName, $values);
            return new \_PhpScoper229e8121cf9f\PhpParser\Node\Stmt\Expression($methodCall);
        }
        if (isset($values[\_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS])) {
            $className = $values[\_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS];
            ...
            $methodCall = new \_PhpScoper229e8121cf9f\PhpParser\Node\Expr\MethodCall($servicesVariable, \_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            unset($values[\_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS]);
        }
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoper229e8121cf9f\Nette\Utils\Strings::match($key, '#\\w+\\s+\\$\\w+#')) {
            return \true;
        }
        return \is_string($values) && $values[0] === '@';
    }
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoper229e8121cf9f\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoper229e8121cf9f\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = \strstr($key, '$');
        $concat = new \_PhpScoper229e8121cf9f\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoper229e8121cf9f\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoper229e8121cf9f\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoper229e8121cf9f\PhpParser\Node\Arg(new \_PhpScoper229e8121cf9f\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoper229e8121cf9f\PhpParser\Node\Expr\MethodCall(new \_PhpScoper229e8121cf9f\PhpParser\Node\Expr\Variable(\_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \_PhpScoper229e8121cf9f\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
