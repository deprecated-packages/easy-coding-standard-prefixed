 ...
 *     Some: Other <---
 */
final class AliasCaseConverter implements \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \_PhpScoperedc2e0c967db\PhpParser\Node\Stmt\Expression
    {
        ...
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $argValues[] = $values[\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS] ?? $values;
            $args = $this->argsNodeFactory->createFromValues($argValues, \true);
            $methodCall = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\MethodCall($servicesVariable, \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \_PhpScoperedc2e0c967db\Nette\Utils\Strings::before($key, ' $');
        if ($fullClassName !== null) {
            $methodCall = $this->createAliasNode($key, $fullClassName, $values);
            return new \_PhpScoperedc2e0c967db\PhpParser\Node\Stmt\Expression($methodCall);
        }
        if (isset($values[\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS])) {
            $className = $values[\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS];
            ...
            $methodCall = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\MethodCall($servicesVariable, \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            unset($values[\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS]);
        }
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\_PhpScoperedc2e0c967db\Nette\Utils\Strings::match($key, '#\\w+\\s+\\$\\w+#')) {
            return \true;
        }
        return \is_string($values) && $values[0] === '@';
    }
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\MethodCall
    {
        $args = [];
        $classConstFetch = $this->commonNodeFactory->createClassReference($fullClassName);
        \_PhpScoperedc2e0c967db\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = \strstr($key, '$');
        $concat = new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\BinaryOp\Concat($classConstFetch, new \_PhpScoperedc2e0c967db\PhpParser\Node\Scalar\String_(' ' . $argumentName));
        $args[] = new \_PhpScoperedc2e0c967db\PhpParser\Node\Arg($concat);
        $serviceName = \ltrim($serviceValues, '@');
        $args[] = new \_PhpScoperedc2e0c967db\PhpParser\Node\Arg(new \_PhpScoperedc2e0c967db\PhpParser\Node\Scalar\String_($serviceName));
        return new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\MethodCall(new \_PhpScoperedc2e0c967db\PhpParser\Node\Expr\Variable(\_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \_PhpScoperedc2e0c967db\Migrify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
