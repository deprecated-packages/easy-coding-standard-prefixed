...
namespace _PhpScoper207eb8f99af3\Migrify\PhpConfigPrinter\RoutingCaseConverter;

use _PhpScoper207eb8f99af3\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface;
...
final class ImportRoutingCaseConverter implements \_PhpScoper207eb8f99af3\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
    ...
    {
        $variable = new \_PhpScoper207eb8f99af3\PhpParser\Node\Expr\Variable(\_PhpScoper207eb8f99af3\Migrify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        $args = $this->createAddArgs(self::IMPORT_ARGS, $values);
        $methodCall = new \_PhpScoper207eb8f99af3\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \_PhpScoper207eb8f99af3\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \_PhpScoper207eb8f99af3\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
