...
namespace _PhpScoper64a921a5401b\Migrify\PhpConfigPrinter\RoutingCaseConverter;

use _PhpScoper64a921a5401b\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface;
...
final class ImportRoutingCaseConverter implements \_PhpScoper64a921a5401b\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
    ...
    {
        $variable = new \_PhpScoper64a921a5401b\PhpParser\Node\Expr\Variable(\_PhpScoper64a921a5401b\Migrify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        $args = $this->createAddArgs(self::IMPORT_ARGS, $values);
        $methodCall = new \_PhpScoper64a921a5401b\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \_PhpScoper64a921a5401b\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \_PhpScoper64a921a5401b\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
