...
namespace _PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\RoutingCaseConverter;

use _PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface;
...
final class ImportRoutingCaseConverter implements \_PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
    ...
    {
        $variable = new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\Variable(\_PhpScoperf77bffce0320\Migrify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        $args = $this->createAddArgs(self::IMPORT_ARGS, $values);
        $methodCall = new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \_PhpScoperf77bffce0320\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
