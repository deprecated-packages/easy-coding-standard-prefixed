...
namespace _PhpScoper4936962185e7\Migrify\PhpConfigPrinter\RoutingCaseConverter;

use _PhpScoper4936962185e7\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface;
...
final class ImportRoutingCaseConverter implements \_PhpScoper4936962185e7\Migrify\PhpConfigPrinter\Contract\RoutingCaseConverterInterface
{
    /**
     * @var string[]
     */
    ...
    public function convertToMethodCall(string $key, $values) : \_PhpScoper4936962185e7\PhpParser\Node\Stmt\Expression
    {
        $variable = new \_PhpScoper4936962185e7\PhpParser\Node\Expr\Variable(\_PhpScoper4936962185e7\Migrify\PhpConfigPrinter\ValueObject\VariableName::ROUTING_CONFIGURATOR);
        $args = $this->createAddArgs(self::IMPORT_ARGS, $values);
        $methodCall = new \_PhpScoper4936962185e7\PhpParser\Node\Expr\MethodCall($variable, 'import', $args);
        ...
        if (isset($values[self::PREFIX])) {
            ...
            $methodCall = new \_PhpScoper4936962185e7\PhpParser\Node\Expr\MethodCall($methodCall, self::PREFIX, $args);
        }
        foreach (self::NESTED_KEYS as $nestedKey) {
            ...
            $methodCall = new \_PhpScoper4936962185e7\PhpParser\Node\Expr\MethodCall($methodCall, $name, $args);
        }
