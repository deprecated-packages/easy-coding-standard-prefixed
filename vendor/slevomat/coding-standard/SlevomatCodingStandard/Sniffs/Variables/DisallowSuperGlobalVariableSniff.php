...
class DisallowSuperGlobalVariableSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    public const CODE_DISALLOWED_SUPER_GLOBAL_VARIABLE = 'DisallowedSuperGlobalVariable';
    private const SUPER_GLOBALS = ['$GLOBALS', '$_SERVER', '$_GET', '$_POST', '$_FILES', '$_COOKIE', '$_SESSION', '$_REQUEST', '$_ENV'];
    /**
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $variablePointer) : void
    {
        ...
        if (!\in_array($tokens[$variablePointer]['content'], self::SUPER_GLOBALS, \true)) {
            return;
        }
