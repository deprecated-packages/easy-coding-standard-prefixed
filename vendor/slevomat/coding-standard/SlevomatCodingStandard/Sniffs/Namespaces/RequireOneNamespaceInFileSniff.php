...
class RequireOneNamespaceInFileSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $namespacePointer) : void
    {
        ...
        $pointerAfterNamespace = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $namespacePointer + 1);
        if ($tokens[$pointerAfterNamespace]['code'] === \T_NS_SEPARATOR) {
            return;
        }
        $previousNamespacePointer = $namespacePointer;
        do {
            $previousNamespacePointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, \T_NAMESPACE, $previousNamespacePointer - 1);
            if ($previousNamespacePointer === null) {
                return;
            }
            $pointerAfterPreviousNamespace = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $previousNamespacePointer + 1);
