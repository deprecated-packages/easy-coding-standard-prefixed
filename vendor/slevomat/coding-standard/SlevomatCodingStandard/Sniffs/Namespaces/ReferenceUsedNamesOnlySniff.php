...
class ReferenceUsedNamesOnlySniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $openTagPointer) : void
    {
                    ...
                    if ($isFullyQualified && !\SlevomatCodingStandard\Helpers\NamespaceHelper::hasNamespace($name) && $namespacePointers === []) {
                        ...
                        if ($fix) {
                            ...
                            if ($reference->source === self::SOURCE_ANNOTATION) {
                                $fixedAnnotationContent = \SlevomatCodingStandard\Helpers\AnnotationHelper::fixAnnotationType($phpcsFile, $reference->annotation, $reference->nameNode, new \_PhpScoper229e8121cf9f\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode(\substr($reference->name, 1)));
                                ...
                                $fixedAnnotationContent = \SlevomatCodingStandard\Helpers\AnnotationHelper::fixAnnotationConstantFetchNode($phpcsFile, $reference->annotation, $reference->constantFetchNode, new \_PhpScoper229e8121cf9f\PHPStan\PhpDocParser\Ast\ConstExpr\ConstFetchNode(\substr($reference->name, 1), $reference->constantFetchNode->name));
                                $phpcsFile->fixer->replaceToken($startPointer, $fixedAnnotationContent);
                                for ($i = $startPointer + 1; $i <= $reference->endPointer; $i++) {
                                    $phpcsFile->fixer->replaceToken($i, '');
                                }
        ...
        foreach ($referenceErrors as $referenceData) {
            ...
            if ($reference->source === self::SOURCE_ANNOTATION) {
                $fixedAnnotationContent = \SlevomatCodingStandard\Helpers\AnnotationHelper::fixAnnotationType($phpcsFile, $reference->annotation, $reference->nameNode, new \_PhpScoper229e8121cf9f\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode($nameToReference));
                $phpcsFile->fixer->replaceToken($startPointer, $fixedAnnotationContent);
            } elseif ($reference->source === self::SOURCE_ANNOTATION_CONSTANT_FETCH) {
                $fixedAnnotationContent = \SlevomatCodingStandard\Helpers\AnnotationHelper::fixAnnotationConstantFetchNode($phpcsFile, $reference->annotation, $reference->constantFetchNode, new \_PhpScoper229e8121cf9f\PHPStan\PhpDocParser\Ast\ConstExpr\ConstFetchNode($nameToReference, $reference->constantFetchNode->name));
                $phpcsFile->fixer->replaceToken($startPointer, $fixedAnnotationContent);
            } else {
