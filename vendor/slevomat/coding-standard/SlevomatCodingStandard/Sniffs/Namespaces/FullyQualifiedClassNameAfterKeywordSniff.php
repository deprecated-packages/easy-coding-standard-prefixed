...
class FullyQualifiedClassNameAfterKeywordSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $keywordPointer) : void
    {
        ...
        $nameStartPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $keywordPointer + 1);
        if (!\in_array($tokens[$nameStartPointer]['code'], $nameTokenCodes, \true)) {
            return;
        }
        $possibleCommaPointer = $this->checkReferencedName($phpcsFile, $keywordPointer, $nameStartPointer);
        if (!\in_array($tokens[$keywordPointer]['code'], [\T_IMPLEMENTS, \T_USE], \true)) {
            return;
        }
        while (\true) {
            $possibleCommaPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, \array_merge($nameTokenCodes, [\T_WHITESPACE]), $possibleCommaPointer);
            if ($possibleCommaPointer !== null) {
                $possibleCommaToken = $tokens[$possibleCommaPointer];
                if ($possibleCommaToken['code'] === \T_COMMA) {
                    /** @var int $nameStartPointer */
                    $nameStartPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $possibleCommaPointer + 1);
                    $possibleCommaPointer = $this->checkReferencedName($phpcsFile, $keywordPointer, $nameStartPointer);
                    continue;
                }
            }
    ...
    private function getKeywordsToCheck() : array
    {
        if ($this->normalizedKeywordsToCheck === null) {
            $this->normalizedKeywordsToCheck = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeArray($this->keywordsToCheck);
        }
    ...
    private function checkReferencedName(\PHP_CodeSniffer\Files\File $phpcsFile, int $keywordPointer, int $nameStartPointer) : int
    {
        ...
        $endPointer = \SlevomatCodingStandard\Helpers\ReferencedNameHelper::getReferencedNameEndPointer($phpcsFile, $nameStartPointer);
        if (!\SlevomatCodingStandard\Helpers\NamespaceHelper::isFullyQualifiedPointer($phpcsFile, $nameStartPointer)) {
            $name = \SlevomatCodingStandard\Helpers\ReferencedNameHelper::getReferenceName($phpcsFile, $nameStartPointer, $endPointer);
            $keyword = $tokens[$keywordPointer]['content'];
            $fix = $phpcsFile->addFixableError(\sprintf('Type %s in %s statement should be referenced via a fully qualified name.', $name, $keyword), $keywordPointer, self::getErrorCode($keyword));
            if ($fix) {
                $fullyQualifiedName = \SlevomatCodingStandard\Helpers\NamespaceHelper::resolveClassName($phpcsFile, $name, $nameStartPointer);
