...
class NamespaceSpacingSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    private function checkLinesBeforeNamespace(\PHP_CodeSniffer\Files\File $phpcsFile, int $namespacePointer) : void
    {
        ...
        $pointerBeforeNamespace = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousExcluding($phpcsFile, \T_WHITESPACE, $namespacePointer - 1);
        ...
        if ($pointerBeforeNamespace + 1 !== $namespacePointer) {
            $whitespaceBeforeNamespace .= \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $pointerBeforeNamespace + 1, $namespacePointer - 1);
        }
        $requiredLinesCountBeforeNamespace = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBeforeNamespace);
        ...
        $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines before namespace statement, found %d.', $requiredLinesCountBeforeNamespace, $actualLinesCountBeforeNamespace), $namespacePointer, self::CODE_INCORRECT_LINES_COUNT_BEFORE_NAMESPACE);
        if (!$fix) {
            return;
        }
    ...
    private function checkLinesAfterNamespace(\PHP_CodeSniffer\Files\File $phpcsFile, int $namespacePointer) : void
    {
        ...
        $namespaceSemicolonPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextLocal($phpcsFile, \T_SEMICOLON, $namespacePointer + 1);
        $pointerAfterWhitespaceEnd = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, \T_WHITESPACE, $namespaceSemicolonPointer + 1);
        if ($pointerAfterWhitespaceEnd === null) {
            return;
        }
        $whitespaceAfterNamespace = \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $namespaceSemicolonPointer + 1, $pointerAfterWhitespaceEnd - 1);
        $requiredLinesCountAfterNamespace = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountAfterNamespace);
        ...
        $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines after namespace statement, found %d.', $requiredLinesCountAfterNamespace, $actualLinesCountAfterNamespace), $namespacePointer, self::CODE_INCORRECT_LINES_COUNT_AFTER_NAMESPACE);
        if (!$fix) {
            return;
        }
        $phpcsFile->fixer->beginChangeset();
