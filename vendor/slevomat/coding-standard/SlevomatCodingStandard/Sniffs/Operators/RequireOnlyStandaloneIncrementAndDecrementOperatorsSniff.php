...
class RequireOnlyStandaloneIncrementAndDecrementOperatorsSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $operatorPointer) : void
    {
        ...
        $afterVariableEndPointer = \SlevomatCodingStandard\Helpers\IdentificatorHelper::findEndPointer($phpcsFile, $nextPointer);
        $isPostOperator = $afterVariableEndPointer === null;
        if ($isPostOperator) {
            /** @var int $beforeVariableEndPointer */
            $beforeVariableEndPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $operatorPointer - 1);
            /** @var int $instructionStartPointer */
            $instructionStartPointer = \SlevomatCodingStandard\Helpers\IdentificatorHelper::findStartPointer($phpcsFile, $beforeVariableEndPointer);
    ...
    private function isStandalone(\PHP_CodeSniffer\Files\File $phpcsFile, int $instructionStartPointer, int $instructionEndPointer) : bool
    {
        $tokens = $phpcsFile->getTokens();
        $pointerBeforeInstructionStart = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $instructionStartPointer - 1);
        if (!\in_array($tokens[$pointerBeforeInstructionStart]['code'], [\T_SEMICOLON, \T_COLON, \T_OPEN_CURLY_BRACKET, \T_CLOSE_CURLY_BRACKET, \T_OPEN_TAG], \true)) {
            return \false;
        }
        $pointerAfterInstructionEnd = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $instructionEndPointer + 1);
        if ($tokens[$pointerAfterInstructionEnd]['code'] === \T_SEMICOLON) {
            return \true;
        }
        if ($tokens[$pointerAfterInstructionEnd]['code'] === \T_CLOSE_PARENTHESIS) {
            return \array_key_exists('parenthesis_owner', $tokens[$pointerAfterInstructionEnd]) && \in_array($tokens[$tokens[$pointerAfterInstructionEnd]['parenthesis_owner']]['code'], [\T_FOR, \T_WHILE], \true);
        }
        return \false;
    }
}
