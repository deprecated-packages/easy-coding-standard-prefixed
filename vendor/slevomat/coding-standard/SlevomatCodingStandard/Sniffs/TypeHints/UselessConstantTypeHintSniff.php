...
class UselessConstantTypeHintSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $constantPointer) : void
    {
        ...
        $docCommentOpenPointer = \SlevomatCodingStandard\Helpers\DocCommentHelper::findDocCommentOpenToken($phpcsFile, $constantPointer);
        if ($docCommentOpenPointer === null) {
            return;
        }
        $annotations = \SlevomatCodingStandard\Helpers\AnnotationHelper::getAnnotations($phpcsFile, $constantPointer);
        $uselessAnnotation = \array_key_exists('@var', $annotations);
        if (!$uselessAnnotation) {
            return;
        }
        $uselessDocComment = !\SlevomatCodingStandard\Helpers\DocCommentHelper::hasDocCommentDescription($phpcsFile, $constantPointer) && \count($annotations) === 1;
        if ($uselessDocComment) {
            ...
            /** @var int $fixerStart */
            $fixerStart = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousContent($phpcsFile, \T_DOC_COMMENT_WHITESPACE, $phpcsFile->eolChar, $annotation->getStartPointer() - 1);
