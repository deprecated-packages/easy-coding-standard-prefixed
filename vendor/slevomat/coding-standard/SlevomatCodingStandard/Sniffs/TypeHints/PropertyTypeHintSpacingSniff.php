...
class PropertyTypeHintSpacingSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $visibilityPointer) : void
    {
        ...
        $typeHintEndPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, $typeHintTokenCodes, $propertyPointer - 1, $propertyStartPointer);
        if ($typeHintEndPointer === null) {
            return;
        }
        $typeHintStartPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousExcluding($phpcsFile, $typeHintTokenCodes, $typeHintEndPointer, $propertyStartPointer) + 1;
        ...
        if ($tokens[$typeHintEndPointer + 1]['code'] !== \T_WHITESPACE) {
            $fix = $phpcsFile->addFixableError('There must be exactly one space between type hint and property.', $typeHintEndPointer, self::CODE_NO_SPACE_BETWEEN_TYPE_HINT_AND_PROPERTY);
            if ($fix) {
                $phpcsFile->fixer->beginChangeset();
            ...
            $fix = $phpcsFile->addFixableError('There must be exactly one space between type hint and property.', $typeHintEndPointer, self::CODE_MULTIPLE_SPACES_BETWEEN_TYPE_HINT_AND_PROPERTY);
            if ($fix) {
                $phpcsFile->fixer->beginChangeset();
        ...
        $fix = $phpcsFile->addFixableError('There must be no whitespace between type hint nullability symbol and type hint.', $typeHintStartPointer, self::CODE_WHITESPACE_AFTER_NULLABILITY_SYMBOL);
        if (!$fix) {
            return;
        }
        $phpcsFile->fixer->beginChangeset();
