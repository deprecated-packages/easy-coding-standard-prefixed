...
class ParameterTypeHintSpacingSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $functionPointer) : void
    {
        ...
        $typeHintTokenCodes = \SlevomatCodingStandard\Helpers\TokenHelper::getNameTokenCodes();
        for ($i = $parametersStartPointer; $i <= $parametersEndPointer; $i++) {
            ...
            $parameterStartPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, \T_COMMA, $parameterPointer - 1, $parametersStartPointer);
            if ($parameterStartPointer === null) {
                $parameterStartPointer = $parametersStartPointer;
            }
            $parameterEndPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, \T_COMMA, $parameterPointer + 1, $parametersEndPointer + 1);
            if ($parameterEndPointer === null) {
                $parameterEndPointer = $parametersEndPointer;
            }
            $typeHintEndPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, $typeHintTokenCodes, $parameterPointer - 1, $parameterStartPointer);
            if ($typeHintEndPointer === null) {
                continue;
            }
            $nextTokenNames = [\T_VARIABLE => \sprintf('parameter %s', $parameterName), \T_BITWISE_AND => \sprintf('reference sign of parameter %s', $parameterName), \T_ELLIPSIS => \sprintf('varadic parameter %s', $parameterName)];
            $nextTokenPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, \array_keys($nextTokenNames), $typeHintEndPointer + 1, $parameterEndPointer + 1);
            ...
            $typeHintStartPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousExcluding($phpcsFile, $typeHintTokenCodes, $typeHintEndPointer, $parameterStartPointer) + 1;
            $previousPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $typeHintStartPointer - 1, $parameterStartPointer);
