...
use const _PhpScoper6207116d4311\T_NULLABLE;
...
class ReturnTypeHintSpacingSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    {
        $typeHint = \SlevomatCodingStandard\Helpers\FunctionHelper::findReturnTypeHint($phpcsFile, $functionPointer);
        if ($typeHint === null) {
            return;
        }
        ...
        $colonPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousExcluding($phpcsFile, \array_merge([\T_NULLABLE], \SlevomatCodingStandard\Helpers\TokenHelper::$ineffectiveTokenCodes), $typeHintPointer - 1);
        $nextPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $colonPointer + 1);
        ...
        $spacesCountBeforeColon = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->spacesCountBeforeColon);
        ...
        if ($tokens[$colonPointer - 1]['code'] !== \T_CLOSE_PARENTHESIS && $tokens[$colonPointer - 1]['content'] !== $expectedSpaces) {
            $fix = $spacesCountBeforeColon === 0 ? $phpcsFile->addFixableError('There must be no whitespace between closing parenthesis and return type colon.', $typeHintPointer, self::CODE_WHITESPACE_BEFORE_COLON) : $phpcsFile->addFixableError(\sprintf('There must be exactly %d whitespace%s between closing parenthesis and return type colon.', $spacesCountBeforeColon, $spacesCountBeforeColon !== 1 ? 's' : ''), $typeHintPointer, self::CODE_INCORRECT_SPACES_BEFORE_COLON);
            if ($fix) {
                $phpcsFile->fixer->beginChangeset();
            ...
            $fix = $phpcsFile->addFixableError(\sprintf('There must be exactly %d whitespace%s between closing parenthesis and return type colon.', $spacesCountBeforeColon, $spacesCountBeforeColon !== 1 ? 's' : ''), $typeHintPointer, self::CODE_INCORRECT_SPACES_BEFORE_COLON);
            if ($fix) {
                $phpcsFile->fixer->beginChangeset();
