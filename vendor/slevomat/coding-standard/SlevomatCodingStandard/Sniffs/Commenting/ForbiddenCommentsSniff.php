...
class ForbiddenCommentsSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $docCommentOpenPointer) : void
    {
        ...
        $comments = \SlevomatCodingStandard\Helpers\DocCommentHelper::getDocCommentDescription($phpcsFile, $docCommentOpenPointer);
        if ($comments === null) {
            return;
        }
        foreach (\SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeArray($this->forbiddenCommentPatterns) as $forbiddenCommentPattern) {
            if (!\SlevomatCodingStandard\Helpers\SniffSettingsHelper::isValidRegularExpression($forbiddenCommentPattern)) {
