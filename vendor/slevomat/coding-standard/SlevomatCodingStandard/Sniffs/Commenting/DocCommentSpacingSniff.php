...
use function uasort;
use function usort;
use const _PhpScoperbd5fb781fe24\T_DOC_COMMENT_OPEN_TAG;
use const _PhpScoperbd5fb781fe24\T_DOC_COMMENT_STAR;
use const _PhpScoperbd5fb781fe24\T_DOC_COMMENT_STRING;
use const _PhpScoperbd5fb781fe24\T_DOC_COMMENT_WHITESPACE;
class DocCommentSpacingSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
     ...
     */
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $docCommentOpenerPointer) : void
    {
        if (\SlevomatCodingStandard\Helpers\DocCommentHelper::isInline($phpcsFile, $docCommentOpenerPointer)) {
            return;
        }
        $tokens = $phpcsFile->getTokens();
        $firstContentStartPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, [\T_DOC_COMMENT_WHITESPACE, \T_DOC_COMMENT_STAR], $docCommentOpenerPointer + 1, $tokens[$docCommentOpenerPointer]['comment_closer']);
        ...
        do {
            ...
            $actualPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, [\T_DOC_COMMENT_STAR, \T_DOC_COMMENT_WHITESPACE], $actualPointer + 1, $tokens[$docCommentOpenerPointer]['comment_closer'] + 1);
            if ($tokens[$actualPointer]['code'] !== \T_DOC_COMMENT_STRING) {
                break;
            }
            $firstContentEndPointer = $actualPointer;
        } while (\true);
        $annotations = \array_merge([], ...\array_values(\SlevomatCodingStandard\Helpers\AnnotationHelper::getAnnotations($phpcsFile, $docCommentOpenerPointer)));
        \uasort($annotations, static function (\SlevomatCodingStandard\Helpers\Annotation\Annotation $a, \SlevomatCodingStandard\Helpers\Annotation\Annotation $b) : int {
    ...
    private function checkLinesBeforeFirstContent(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, int $firstContentStartPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        $whitespaceBeforeFirstContent = \substr($tokens[$docCommentOpenerPointer]['content'], 0, \strlen('/**'));
        $whitespaceBeforeFirstContent .= \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $docCommentOpenerPointer + 1, $firstContentStartPointer - 1);
        $requiredLinesCountBeforeFirstContent = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBeforeFirstContent);
        ...
        $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines before first content, found %d.', $requiredLinesCountBeforeFirstContent, $linesCountBeforeFirstContent), $firstContentStartPointer, self::CODE_INCORRECT_LINES_COUNT_BEFORE_FIRST_CONTENT);
        if (!$fix) {
            return;
        }
        $indentation = \SlevomatCodingStandard\Helpers\IndentationHelper::getIndentation($phpcsFile, $docCommentOpenerPointer);
    ...
    private function checkLinesBetweenDescriptionAndFirstAnnotation(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, int $firstContentStartPointer, int $firstContentEndPointer, ?\SlevomatCodingStandard\Helpers\Annotation\Annotation $firstAnnotation) : void
    {
        ...
        $whitespaceBetweenDescriptionAndFirstAnnotation .= \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $firstContentEndPointer + 1, $firstAnnotation->getStartPointer() - 1);
        $requiredLinesCountBetweenDescriptionAndAnnotations = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBetweenDescriptionAndAnnotations);
        ...
        $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines between description and annotations, found %d.', $requiredLinesCountBetweenDescriptionAndAnnotations, $linesCountBetweenDescriptionAndAnnotations), $firstAnnotation->getStartPointer(), self::CODE_INCORRECT_LINES_COUNT_BETWEEN_DESCRIPTION_AND_ANNOTATIONS);
        if (!$fix) {
            return;
        }
        $indentation = \SlevomatCodingStandard\Helpers\IndentationHelper::getIndentation($phpcsFile, $docCommentOpenerPointer);
     ...
     */
    private function checkLinesBetweenDifferentAnnotationsTypes(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, array $annotations) : void
    {
        $requiredLinesCountBetweenDifferentAnnotationsTypes = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBetweenDifferentAnnotationsTypes);
        $tokens = $phpcsFile->getTokens();
        $indentation = \SlevomatCodingStandard\Helpers\IndentationHelper::getIndentation($phpcsFile, $docCommentOpenerPointer);
        ...
        foreach ($annotations as $annotation) {
            ...
            $linesCountAfterPreviousAnnotation .= \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $previousAnnotation->getEndPointer() + 1, $annotation->getStartPointer() - 1);
            ...
            $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines between different annotations types, found %d.', $requiredLinesCountBetweenDifferentAnnotationsTypes, $linesCountAfterPreviousAnnotation), $annotation->getStartPointer(), self::CODE_INCORRECT_LINES_COUNT_BETWEEN_DIFFERENT_ANNOTATIONS_TYPES);
            if (!$fix) {
                $previousAnnotation = $annotation;
                continue;
            }
     ...
     * @param Annotation[] $annotations
     */
    private function checkAnnotationsGroups(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, array $annotations) : void
    {
        $tokens = $phpcsFile->getTokens();
        $annotationsGroups = [];
        $annotationsGroup = [];
     ...
     */
    private function checkLinesBetweenAnnotationsGroups(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, array $annotationsGroups) : void
    {
        $tokens = $phpcsFile->getTokens();
        $requiredLinesCountBetweenAnnotationsGroups = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBetweenAnnotationsGroups);
        ...
        foreach ($annotationsGroups as $annotationsGroup) {
            ...
            $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines between annotations groups, found %d.', $requiredLinesCountBetweenAnnotationsGroups, $actualLinesCountBetweenAnnotationsGroups), $firstAnnotationInActualGroup->getStartPointer(), self::CODE_INCORRECT_LINES_COUNT_BETWEEN_ANNOTATIONS_GROUPS);
            if (!$fix) {
                $previousAnnotationsGroup = $annotationsGroup;
                continue;
            }
            ...
            for ($i = 1; $i <= $requiredLinesCountBetweenAnnotationsGroups; $i++) {
                $phpcsFile->fixer->addContent($lastAnnotationInPreviousGroup->getEndPointer(), \sprintf('%s *%s', $indentation, $phpcsFile->eolChar));
            }
            $phpcsFile->fixer->addContentBefore($firstAnnotationInActualGroup->getStartPointer(), $indentation . ' * ');
            $phpcsFile->fixer->endChangeset();
        }
    }
    /**
     ...
     * @param Annotation[] $annotations
     */
    private function checkAnnotationsGroupsOrder(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, array $annotationsGroups, array $annotations) : void
    {
        $equals = static function (array $firstAnnotationsGroup, array $secondAnnotationsGroup) : bool {
            $getAnnotationsPointers = static function (\SlevomatCodingStandard\Helpers\Annotation\Annotation $annotation) : int {
        ...
        foreach ($annotationsGroups as $annotationsGroupPosition => $annotationsGroup) {
            ...
            $incorrectAnnotationsGroupsExist = \true;
        ...
        foreach ($annotationsGroups as $annotationsGroupPosition => $annotationsGroup) {
            ...
            foreach ($annotationsGroup as $annotationPosition => $annotation) {
                ...
                $fix = $phpcsFile->addFixableError('Incorrent order of annotations in group.', $annotation->getStartPointer(), self::CODE_INCORRECT_ORDER_OF_ANNOTATIONS_IN_GROUP);
                break;
            }
        }
        if (!$fix) {
        ...
        foreach ($sortedAnnotationsGroups as $sortedAnnotationsGroup) {
            if ($firstGroup) {
                ...
                for ($i = 0; $i < \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBetweenAnnotationsGroups); $i++) {
                    $fixedAnnotations .= \sprintf('%s *%s', $indentation, $phpcsFile->eolChar);
                }
            }
            foreach ($sortedAnnotationsGroup as $sortedAnnotation) {
                $fixedAnnotations .= \sprintf('%s * %s%s', $indentation, \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $sortedAnnotation->getStartPointer(), $sortedAnnotation->getEndPointer()), $phpcsFile->eolChar);
            }
        }
        $endOfLineBeforeFirstAnnotation = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousContent($phpcsFile, \T_DOC_COMMENT_WHITESPACE, $phpcsFile->eolChar, $firstAnnotation->getStartPointer() - 1, $docCommentOpenerPointer);
        $endOfLineAfterLastAnnotation = \SlevomatCodingStandard\Helpers\TokenHelper::findNextContent($phpcsFile, \T_DOC_COMMENT_WHITESPACE, $phpcsFile->eolChar, $lastAnnotation->getEndPointer() + 1);
        $phpcsFile->fixer->beginChangeset();
    ...
    private function sortAnnotationsToGroups(array $annotations) : array
    {
        ...
        foreach (\array_keys($sortedAnnotationsGroups) as $annotationsGroupPosition) {
            ...
            \usort($sortedAnnotationsGroups[$annotationsGroupPosition], function (\SlevomatCodingStandard\Helpers\Annotation\Annotation $firstAnnotation, \SlevomatCodingStandard\Helpers\Annotation\Annotation $secondAnnotation) use($expectedAnnotationsGroupOrder) : int {
    ...
    {
        return \in_array(\substr($annotationNamespace, -1), ['\\', '-', ':'], \true) && \strpos($annotationName, $annotationNamespace) === 0;
    }
    private function isAnnotationMatched(\SlevomatCodingStandard\Helpers\Annotation\Annotation $annotation, string $annotationName) : bool
    {
    ...
    private function checkLinesAfterLastContent(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenerPointer, int $docCommentCloserPointer, int $lastContentEndPointer) : void
    {
        $whitespaceAfterLastContent = \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $lastContentEndPointer + 1, $docCommentCloserPointer);
        $requiredLinesCountAfterLastContent = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountAfterLastContent);
        ...
        $fix = $phpcsFile->addFixableError(\sprintf('Expected %d lines after last content, found %d.', $requiredLinesCountAfterLastContent, $linesCountAfterLastContent), $lastContentEndPointer, self::CODE_INCORRECT_LINES_COUNT_AFTER_LAST_CONTENT);
        if (!$fix) {
            return;
        }
        $indentation = \SlevomatCodingStandard\Helpers\IndentationHelper::getIndentation($phpcsFile, $docCommentOpenerPointer);
    ...
    private function getAnnotationsGroups() : array
    {
        if ($this->normalizedAnnotationsGroups === null) {
            ...
            foreach ($this->annotationsGroups as $annotationsGroup) {
                $this->normalizedAnnotationsGroups[] = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeArray(\explode(',', $annotationsGroup));
            }
        }
