...
class DisallowOneLinePropertyDocCommentSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $propertyPointer) : void
    {
        ...
        if (!\SlevomatCodingStandard\Helpers\DocCommentHelper::hasDocComment($phpcsFile, $propertyPointer)) {
            return;
        }
        /** @var int $docCommentStartPointer */
        $docCommentStartPointer = \SlevomatCodingStandard\Helpers\DocCommentHelper::findDocCommentOpenToken($phpcsFile, $propertyPointer);
        ...
        $fix = $phpcsFile->addFixableError(\sprintf('Found one-line comment for property %s, use multi-line comment instead.', \SlevomatCodingStandard\Helpers\PropertyHelper::getFullyQualifiedName($phpcsFile, $propertyPointer)), $docCommentStartPointer, self::CODE_ONE_LINE_PROPERTY_COMMENT);
        if (!$fix) {
            return;
        }
        $commentWhitespacePointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, [\T_WHITESPACE], $docCommentStartPointer);
