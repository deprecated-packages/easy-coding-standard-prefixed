...
class LanguageConstructWithParenthesesSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    {
        return [\T_BREAK, \T_CONTINUE, \T_ECHO, \T_EXIT, \T_INCLUDE, \T_INCLUDE_ONCE, \T_PRINT, \T_REQUIRE, \T_REQUIRE_ONCE, \T_RETURN, \T_THROW, \T_YIELD, \T_YIELD_FROM];
    }
    /**
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $languageConstructPointer) : void
    {
        ...
        $openParenthesisPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $languageConstructPointer + 1);
        ...
        $afterCloseParenthesisPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $closeParenthesisPointer + 1);
        if (!\in_array($tokens[$afterCloseParenthesisPointer]['code'], [\T_SEMICOLON, \T_CLOSE_PARENTHESIS, \T_CLOSE_SHORT_ARRAY], \true)) {
            return;
        }
        $containsContentBetweenParentheses = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $openParenthesisPointer + 1, $closeParenthesisPointer) !== null;
