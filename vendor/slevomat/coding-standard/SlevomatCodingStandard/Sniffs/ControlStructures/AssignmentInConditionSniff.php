...
class AssignmentInConditionSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $conditionStartPointer) : void
    {
        ...
        if ($token['code'] === \T_DO) {
            $whilePointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, \T_WHILE, $token['scope_closer'] + 1);
    ...
    private function processCondition(\PHP_CodeSniffer\Files\File $phpcsFile, int $parenthesisOpener, int $parenthesisCloser, string $conditionType) : void
    {
        $equalsTokenPointers = \SlevomatCodingStandard\Helpers\TokenHelper::findNextAll($phpcsFile, \T_EQUAL, $parenthesisOpener + 1, $parenthesisCloser);
        $tokens = $phpcsFile->getTokens();
        ...
        foreach ($equalsTokenPointers as $equalsTokenPointer) {
            ...
            $functionCall = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, \SlevomatCodingStandard\Helpers\TokenHelper::getOnlyNameTokenCodes(), $insideParenthesis, $parenthesisOpener);
    ...
    private function error(\PHP_CodeSniffer\Files\File $phpcsFile, string $conditionType, int $equalsTokenPointer) : void
    {
