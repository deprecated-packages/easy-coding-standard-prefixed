...
class SingleLineArrayWhitespaceSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
     ...
     */
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $stackPointer) : int
    {
        ...
        $content = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, \T_WHITESPACE, $arrayStart + 1, $arrayEnd + 1);
        if ($content === $arrayEnd) {
        ...
        for ($i = $arrayStart + 1; $i < $arrayEnd; $i++) {
            ...
            $next = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, \T_WHITESPACE, $i + 1, $arrayEnd);
            if ($next === null) {
                return $arrayStart + 1;
            }
    ...
    {
        return \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->spacesAroundBrackets);
    }
    private function checkWhitespaceInEmptyArray(\PHP_CodeSniffer\Files\File $phpcsFile, int $arrayStart, int $arrayEnd) : void
    {
    ...
    private function checkWhitespaceAfterOpeningBracket(\PHP_CodeSniffer\Files\File $phpcsFile, int $arrayStart) : void
    {
        $tokens = $phpcsFile->getTokens();
    ...
    private function checkWhitespaceBeforeClosingBracket(\PHP_CodeSniffer\Files\File $phpcsFile, int $arrayEnd) : void
    {
        $tokens = $phpcsFile->getTokens();
    ...
    private function checkWhitespaceBeforeComma(\PHP_CodeSniffer\Files\File $phpcsFile, int $comma) : void
    {
        $tokens = $phpcsFile->getTokens();
    ...
    private function checkWhitespaceAfterComma(\PHP_CodeSniffer\Files\File $phpcsFile, int $comma) : void
    {
        $tokens = $phpcsFile->getTokens();
        if ($tokens[$comma + 1]['code'] !== \T_WHITESPACE) {
