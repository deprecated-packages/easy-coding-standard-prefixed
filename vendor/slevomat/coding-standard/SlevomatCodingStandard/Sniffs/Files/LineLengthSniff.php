...
class LineLengthSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    private function checkLineLength(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        ...
        if (\in_array($tokens[$pointer]['code'], [\T_COMMENT, \T_DOC_COMMENT_STRING], \true)) {
        ...
        if ($this->ignoreImports) {
            $usePointer = \SlevomatCodingStandard\Helpers\UseStatementHelper::getUseStatementPointer($phpcsFile, $pointer - 1);
            if (\is_int($usePointer) && $tokens[$usePointer]['line'] === $tokens[$pointer]['line'] && !\SlevomatCodingStandard\Helpers\UseStatementHelper::isTraitUse($phpcsFile, $usePointer)) {
                return;
            }
        }
