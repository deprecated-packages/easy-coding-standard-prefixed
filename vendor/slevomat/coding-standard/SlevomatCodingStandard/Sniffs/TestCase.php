 ...
 * @codeCoverageIgnore
 */
abstract class TestCase extends \_PhpScoper666af036e800\PHPUnit\Framework\TestCase
{
    /**
     ...
     * @return File
     */
    protected static function checkFile(string $filePath, array $sniffProperties = [], array $codesToCheck = [], array $cliArgs = []) : \PHP_CodeSniffer\Files\File
    {
        if (\defined('PHP_CODESNIFFER_CBF') === \false) {
            \define('PHP_CODESNIFFER_CBF', \false);
        }
        $codeSniffer = new \PHP_CodeSniffer\Runner();
        $codeSniffer->config = new \PHP_CodeSniffer\Config(\array_merge(['-s'], $cliArgs));
        $codeSniffer->init();
        ...
        if (\count($codesToCheck) > 0) {
            foreach (static::getSniffClassReflection()->getConstants() as $constantName => $constantValue) {
                if (\strpos($constantName, 'CODE_') !== 0 || \in_array($constantValue, $codesToCheck, \true)) {
                    continue;
                }
    ...
    protected static function assertNoSniffErrorInFile(\PHP_CodeSniffer\Files\File $phpcsFile) : void
    {
        $errors = $phpcsFile->getErrors();
        self::assertEmpty($errors, \sprintf('No errors expected, but %d errors found.', \count($errors)));
    }
    protected static function assertSniffError(\PHP_CodeSniffer\Files\File $phpcsFile, int $line, string $code, ?string $message = null) : void
    {
        $errors = $phpcsFile->getErrors();
    ...
    protected static function assertNoSniffError(\PHP_CodeSniffer\Files\File $phpcsFile, int $line) : void
    {
        $errors = $phpcsFile->getErrors();
        self::assertFalse(isset($errors[$line]), \sprintf('Expected no error on line %s, but found:%s%s%s', $line, \PHP_EOL . \PHP_EOL, isset($errors[$line]) ? self::getFormattedErrors($errors[$line]) : '', \PHP_EOL));
    }
    protected static function assertAllFixedInFile(\PHP_CodeSniffer\Files\File $phpcsFile) : void
    {
        $phpcsFile->disableCaching();
        $phpcsFile->fixer->fixFile();
        self::assertStringEqualsFile(\preg_replace('~(\\.php)$~', '_PhpScoper666af036e800\\.fixed\\1', $phpcsFile->getFilename()), $phpcsFile->fixer->getContents());
    }
    protected static function getSniffName() : string
    {
        return \preg_replace(['~\\\\~', '~\\.Sniffs~', '~Sniff$~'], ['.', '', ''], static::getSniffClassName());
    }
