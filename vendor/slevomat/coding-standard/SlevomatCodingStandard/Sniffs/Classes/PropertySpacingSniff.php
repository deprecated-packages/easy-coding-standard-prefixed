...
class PropertySpacingSniff extends \SlevomatCodingStandard\Sniffs\Classes\AbstractPropertyAndConstantSpacing
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $pointer) : int
    {
        ...
        $asPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $pointer - 1);
        if ($tokens[$asPointer]['code'] === \T_AS) {
            return $pointer;
        }
        $propertyPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, [\T_VARIABLE, \T_FUNCTION, \T_CONST, \T_USE], $pointer + 1);
    ...
    protected function isNextMemberValid(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer) : bool
    {
        $nextPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, [\T_FUNCTION, \T_VARIABLE], $pointer + 1);
        if ($nextPointer === null) {
            return \false;
        }
        return $phpcsFile->getTokens()[$nextPointer]['code'] === \T_VARIABLE;
    }
    protected function addError(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer, int $minExpectedLines, int $maxExpectedLines, int $found) : bool
    {
