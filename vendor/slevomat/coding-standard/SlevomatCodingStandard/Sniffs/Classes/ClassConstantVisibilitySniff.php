...
class ClassConstantVisibilitySniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $constantPointer) : void
    {
        ...
        $visibilityPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $constantPointer - 1);
        if (\in_array($tokens[$visibilityPointer]['code'], [\T_PUBLIC, \T_PROTECTED, \T_PRIVATE], \true)) {
            return;
        }
        $message = \sprintf('Constant %s::%s visibility missing.', \SlevomatCodingStandard\Helpers\ClassHelper::getFullyQualifiedName($phpcsFile, $classPointer), $tokens[\SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $constantPointer + 1)]['content']);
        if ($this->fixable) {
