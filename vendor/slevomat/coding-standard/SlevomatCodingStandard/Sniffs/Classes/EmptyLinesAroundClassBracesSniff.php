...
use const _PhpScoperfd70a7e8e84f\T_ANON_CLASS;
use const T_CLASS;
use const T_INTERFACE;
use const T_TRAIT;
use const T_WHITESPACE;
class EmptyLinesAroundClassBracesSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    {
        return [\T_CLASS, \T_ANON_CLASS, \T_INTERFACE, \T_TRAIT];
    }
    /**
    ...
    private function processOpeningBrace(\PHP_CodeSniffer\Files\File $phpcsFile, int $stackPointer) : void
    {
        ...
        $nextPointerAfterOpeningBrace = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, \T_WHITESPACE, $openerPointer + 1);
        $nextTokenAfterOpeningBrace = $tokens[$nextPointerAfterOpeningBrace];
        $linesCountAfterOpeningBrace = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountAfterOpeningBrace);
        ...
        if ($linesCountAfterOpeningBrace === 1) {
            ...
            $fix = $phpcsFile->addFixableError(\sprintf('There must be exactly %d empty lines after %s opening brace.', $linesCountAfterOpeningBrace, $typeToken['content']), $openerPointer, self::CODE_INCORRECT_EMPTY_LINES_AFTER_OPENING_BRACE);
        }
        if (!$fix) {
            return;
        }
    ...
    private function processClosingBrace(\PHP_CodeSniffer\Files\File $phpcsFile, int $stackPointer) : void
    {
        ...
        $previousPointerBeforeClosingBrace = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousExcluding($phpcsFile, \T_WHITESPACE, $closerPointer - 1);
        $previousTokenBeforeClosingBrace = $tokens[$previousPointerBeforeClosingBrace];
        $linesCountBeforeClosingBrace = \SlevomatCodingStandard\Helpers\SniffSettingsHelper::normalizeInteger($this->linesCountBeforeClosingBrace);
        ...
        if ($linesCountBeforeClosingBrace === 1) {
            $fix = $phpcsFile->addFixableError(\sprintf('There must be one empty line before %s closing brace.', $typeToken['content']), $closerPointer, $lines === 0 ? self::CODE_NO_EMPTY_LINE_BEFORE_CLOSING_BRACE : self::CODE_MULTIPLE_EMPTY_LINES_BEFORE_CLOSING_BRACE);
        } else {
            $fix = $phpcsFile->addFixableError(\sprintf('There must be exactly %d empty lines before %s closing brace.', $linesCountBeforeClosingBrace, $typeToken['content']), $closerPointer, self::CODE_INCORRECT_EMPTY_LINES_BEFORE_CLOSING_BRACE);
        }
        if (!$fix) {
            return;
        }
