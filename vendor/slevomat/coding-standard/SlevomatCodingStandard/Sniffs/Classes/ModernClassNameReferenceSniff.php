...
class ModernClassNameReferenceSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    {
        $tokens = \SlevomatCodingStandard\Helpers\TokenHelper::getOnlyNameTokenCodes();
        $tokens[] = \T_CLASS_C;
        return $tokens;
    }
    /**
    ...
    private function checkMagicConstant(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer) : void
    {
    ...
    private function checkFunctionCall(\PHP_CodeSniffer\Files\File $phpcsFile, int $functionPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        $functionName = \ltrim(\strtolower($tokens[$functionPointer]['content']), '\\');
        $functionNames = ['get_class', 'get_parent_class', 'get_called_class'];
        if (!\in_array($functionName, $functionNames, \true)) {
            return;
        }
        $openParenthesisPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $functionPointer + 1);
        if ($tokens[$openParenthesisPointer]['code'] !== \T_OPEN_PARENTHESIS) {
            return;
        }
        $previousPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $functionPointer - 1);
        if (\in_array($tokens[$previousPointer]['code'], [\T_OBJECT_OPERATOR, \T_DOUBLE_COLON], \true)) {
            return;
        }
        $parameterPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $openParenthesisPointer + 1, $tokens[$openParenthesisPointer]['parenthesis_closer']);
        $isThisParameter = static function () use($phpcsFile, $tokens, $openParenthesisPointer, $parameterPointer) : bool {
            ...
            $pointerAfterParameterPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $parameterPointer + 1);
        ...
        if ($functionName === 'get_class') {
            ...
            if ($parameterPointer !== null) {
                ...
                $classPointer = \SlevomatCodingStandard\Helpers\FunctionHelper::findClassPointer($phpcsFile, $functionPointer);
                if (!\SlevomatCodingStandard\Helpers\ClassHelper::isFinal($phpcsFile, $classPointer)) {
                    return;
                }
            }
