...
class ConstantSpacingSniff extends \SlevomatCodingStandard\Sniffs\Classes\AbstractPropertyAndConstantSpacing
{
     ...
     */
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $constantPointer) : int
    {
        ...
        if (!\in_array($tokens[$classPointer]['code'], [\T_CLASS, \T_INTERFACE, \T_ANON_CLASS], \true)) {
            return $constantPointer;
        }
    ...
    protected function isNextMemberValid(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer) : bool
    {
        ...
        $nextPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, [\T_FUNCTION, \T_CONST, \T_VARIABLE, \T_USE], $pointer + 1);
    ...
    protected function addError(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer, int $minExpectedLines, int $maxExpectedLines, int $found) : bool
    {
        if ($minExpectedLines === $maxExpectedLines) {
            $errorMessage = $minExpectedLines === 1 ? 'Expected 1 blank line after constant, found %3$d.' : 'Expected %2$d blank lines after constant, found %3$d.';
        } else {
