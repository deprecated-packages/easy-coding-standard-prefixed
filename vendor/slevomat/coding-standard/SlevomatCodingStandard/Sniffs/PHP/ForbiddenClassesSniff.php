...
use const _PhpScoper797695bcfb1f\T_COMMA;
...
class ForbiddenClassesSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
     ...
     */
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $tokenPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        $token = $tokens[$tokenPointer];
        $nameTokens = \array_merge(\SlevomatCodingStandard\Helpers\TokenHelper::getNameTokenCodes(), \SlevomatCodingStandard\Helpers\TokenHelper::$ineffectiveTokenCodes);
        if ($token['code'] === \T_IMPLEMENTS || $token['code'] === \T_USE && \SlevomatCodingStandard\Helpers\UseStatementHelper::isTraitUse($phpcsFile, $tokenPointer)) {
            $endTokenPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, [\T_SEMICOLON, \T_OPEN_CURLY_BRACKET], $tokenPointer);
        ...
        } elseif (\in_array($token['code'], [\T_NEW, \T_EXTENDS], \true)) {
            $endTokenPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextExcluding($phpcsFile, $nameTokens, $tokenPointer + 1);
            $references = $this->getAllReferences($phpcsFile, $tokenPointer, $endTokenPointer);
            ...
            $startTokenPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousExcluding($phpcsFile, $nameTokens, $tokenPointer - 1);
     ...
     */
    private function checkReferences(\PHP_CodeSniffer\Files\File $phpcsFile, int $tokenPointer, array $references, array $forbiddenNames, bool $isFixable = \true) : void
    {
        $token = $phpcsFile->getTokens()[$tokenPointer];
        $details = [\T_NEW => ['class', self::CODE_FORBIDDEN_CLASS], \T_DOUBLE_COLON => ['class', self::CODE_FORBIDDEN_CLASS], \T_EXTENDS => ['as a parent class', self::CODE_FORBIDDEN_PARENT_CLASS], \T_IMPLEMENTS => ['interface', self::CODE_FORBIDDEN_INTERFACE], \T_USE => ['trait', self::CODE_FORBIDDEN_TRAIT]];
     ...
     */
    private function getAllReferences(\PHP_CodeSniffer\Files\File $phpcsFile, int $startPointer, int $endPointer) : array
    {
        // Always ignore first token
        $startPointer++;
        $references = [];
        while ($startPointer < $endPointer) {
            $nextComma = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, [\T_COMMA], $startPointer + 1);
            $nextSeparator = \min($endPointer, $nextComma ?? \PHP_INT_MAX);
            $reference = \SlevomatCodingStandard\Helpers\ReferencedNameHelper::getReferenceName($phpcsFile, $startPointer, $nextSeparator - 1);
            if (\strlen($reference) !== 0 && !\in_array(\strtolower($reference), self::$keywordReferences, \true)) {
                $references[] = ['fullyQualifiedName' => \SlevomatCodingStandard\Helpers\NamespaceHelper::resolveClassName($phpcsFile, $reference, $startPointer), 'startPointer' => \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $startPointer, $endPointer), 'endPointer' => \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $nextSeparator - 1, $startPointer)];
            }
    ...
    private static function normalizeClassName(?string $typeName) : ?string
    {
        ...
        return \SlevomatCodingStandard\Helpers\NamespaceHelper::getFullyQualifiedTypeName($typeName);
    }
}
