...
class UselessSemicolonSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
     ...
     */
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $semicolonPointer) : void
    {
    ...
    private function checkMultipleSemicolons(\PHP_CodeSniffer\Files\File $phpcsFile, int $semicolonPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        $previousPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $semicolonPointer - 1);
        if ($tokens[$previousPointer]['code'] !== \T_SEMICOLON) {
            return;
        }
        $possibleEndScopePointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNextLocal($phpcsFile, \T_CLOSE_PARENTHESIS, $semicolonPointer + 1);
    ...
    private function checkSemicolonAtTheBeginningOfScope(\PHP_CodeSniffer\Files\File $phpcsFile, int $semicolonPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        $previousPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $semicolonPointer - 1);
        if (!\in_array($tokens[$previousPointer]['code'], [\T_OPEN_TAG, \T_OPEN_CURLY_BRACKET], \true)) {
            return;
        }
        $fix = $phpcsFile->addFixableError('Useless semicolon.', $semicolonPointer, self::CODE_USELESS_SEMICOLON);
        if (!$fix) {
    ...
    private function checkSemicolonAfterScope(\PHP_CodeSniffer\Files\File $phpcsFile, int $semicolonPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
        $previousPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPreviousEffective($phpcsFile, $semicolonPointer - 1);
        ...
        if (\in_array($tokens[$scopeOpenerPointer]['code'], [\T_CLOSURE, \T_FN, \T_ANON_CLASS], \true)) {
            return;
        }
        $fix = $phpcsFile->addFixableError('Useless semicolon.', $semicolonPointer, self::CODE_USELESS_SEMICOLON);
        if (!$fix) {
    ...
    private function removeUselessSemicolon(\PHP_CodeSniffer\Files\File $phpcsFile, int $semicolonPointer) : void
    {
        $tokens = $phpcsFile->getTokens();
