...
use _PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\PhpDoc\InvalidTagValueNode;
use _PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagValueNode;
...
use _PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\ConstExprParser;
use _PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\PhpDocParser;
use _PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\TokenIterator;
use _PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\TypeParser;
...
class AnnotationHelper
{
     ...
     * @return string
     */
    public static function fixAnnotationType(\PHP_CodeSniffer\Files\File $phpcsFile, \SlevomatCodingStandard\Helpers\Annotation\Annotation $annotation, \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\TypeNode $typeNode, \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\TypeNode $fixedTypeNode) : string
    {
     ...
     * @return string
     */
    public static function fixAnnotationConstantFetchNode(\PHP_CodeSniffer\Files\File $phpcsFile, \SlevomatCodingStandard\Helpers\Annotation\Annotation $annotation, \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\ConstExpr\ConstFetchNode $node, \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\ConstExpr\ConstFetchNode $fixedNode) : string
    {
        if ($annotation instanceof \SlevomatCodingStandard\Helpers\Annotation\MethodAnnotation) {
            ...
            foreach (self::getAnnotationTypes($annotation) as $annotationType) {
                foreach (\SlevomatCodingStandard\Helpers\AnnotationTypeHelper::getConstantTypeNodes($annotationType) as $constTypeNode) {
                    foreach (\SlevomatCodingStandard\Helpers\AnnotationConstantExpressionHelper::getConstantFetchNodes($constTypeNode->constExpr) as $constFetchNode) {
                        ...
                        $fixedConstTypeNode = new \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\ConstTypeNode(\SlevomatCodingStandard\Helpers\AnnotationConstantExpressionHelper::change($constTypeNode->constExpr, $node, $fixedNode));
            ...
            while ($i < $tokens[$docCommentOpenToken]['comment_closer']) {
                ...
                if (\array_key_exists($annotationName, $mapping)) {
                    ...
                    if ($annotationContent !== null) {
                        ...
                        if ($parsedContent instanceof \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\PhpDoc\InvalidTagValueNode) {
                            $parsedContent = null;
                        }
                    }
    ...
    public static function isAnnotationUseless(\PHP_CodeSniffer\Files\File $phpcsFile, int $functionPointer, $typeHint, \SlevomatCodingStandard\Helpers\Annotation\Annotation $annotation, array $traversableTypeHints) : bool
    {
        ...
        if (\SlevomatCodingStandard\Helpers\AnnotationTypeHelper::isCompoundOfNull($annotation->getType())) {
            ...
            $annotationTypeHint = $annotationTypeHintNode instanceof \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode ? $annotationTypeHintNode->name : (string) $annotationTypeHintNode;
        ...
        if ($annotation->getType() instanceof \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\ConstTypeNode) {
            return \false;
        }
        if ($annotation->getType() instanceof \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\GenericTypeNode) {
            return \false;
        }
        if ($annotation->getType() instanceof \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\CallableTypeNode) {
            return \false;
        }
        /** @var GenericTypeNode|CallableTypeNode|IdentifierTypeNode|ThisTypeNode $annotationTypeNode */
        $annotationTypeNode = $annotation->getType();
        if ($annotationTypeNode instanceof \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode && \in_array(\strtolower($annotationTypeNode->name), ['true', 'false', 'class-string', 'trait-string', 'callable-string', 'numeric-string'], \true)) {
            return \false;
        }
     ...
     * @return Annotation
     */
    private static function fixAnnotation(\SlevomatCodingStandard\Helpers\Annotation\Annotation $annotation, \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\TypeNode $typeNode, \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\Type\TypeNode $fixedTypeNode) : \SlevomatCodingStandard\Helpers\Annotation\Annotation
    {
        if ($annotation instanceof \SlevomatCodingStandard\Helpers\Annotation\MethodAnnotation) {
            $fixedContentNode = clone $annotation->getContentNode();
    ...
    private static function parseAnnotationContent(string $annotationName, string $annotationContent) : \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagValueNode
    {
        $annotationContentWithoutNewLines = \preg_replace('~[\\r\\n]~', ' ', $annotationContent);
        $tokens = new \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\TokenIterator(self::getPhpDocLexer()->tokenize($annotationContentWithoutNewLines));
        return self::getPhpDocParser()->parseTagValue($tokens, $annotationName);
    }
    private static function getPhpDocLexer() : \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Lexer\Lexer
    {
        static $phpDocLexer;
        if ($phpDocLexer === null) {
            $phpDocLexer = new \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Lexer\Lexer();
        }
        return $phpDocLexer;
    }
    private static function getPhpDocParser() : \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\PhpDocParser
    {
        static $phpDocParser;
        if ($phpDocParser === null) {
            $constantExpressionParser = new \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\ConstExprParser();
            $phpDocParser = new \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\PhpDocParser(new \_PhpScoper6224e3b16fcc\PHPStan\PhpDocParser\Parser\TypeParser($constantExpressionParser), $constantExpressionParser);
        }
        return $phpDocParser;
    }
}
