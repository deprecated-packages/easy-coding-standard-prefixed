...
{
    public static function isUsedInScope(\PHP_CodeSniffer\Files\File $phpcsFile, int $scopeOwnerPointer, int $variablePointer) : bool
    {
        return self::isUsedInScopeInternal($phpcsFile, $scopeOwnerPointer, $variablePointer, null);
    }
    public static function isUsedInScopeAfterPointer(\PHP_CodeSniffer\Files\File $phpcsFile, int $scopeOwnerPointer, int $variablePointer, int $startCheckPointer) : bool
    {
        return self::isUsedInScopeInternal($phpcsFile, $scopeOwnerPointer, $variablePointer, $startCheckPointer);
    }
    public static function isUsedAsVariable(\PHP_CodeSniffer\Files\File $phpcsFile, int $variablePointer, int $variableToCheckPointer) : bool
    {
        ...
        if ($tokens[$variableToCheckPointer - 1]['code'] === \T_DOUBLE_COLON) {
            $pointerAfterVariable = \SlevomatCodingStandard\Helpers\TokenHelper::findNextEffective($phpcsFile, $variableToCheckPointer + 1);
            return $tokens[$pointerAfterVariable]['code'] === \T_OPEN_PARENTHESIS;
        }
        return !\SlevomatCodingStandard\Helpers\ParameterHelper::isParameter($phpcsFile, $variableToCheckPointer);
    }
    public static function isUsedInCompactFunction(\PHP_CodeSniffer\Files\File $phpcsFile, int $variablePointer, int $stringPointer) : bool
    {
        $tokens = $phpcsFile->getTokens();
    ...
    public static function isUsedInScopeInString(\PHP_CodeSniffer\Files\File $phpcsFile, string $variableName, int $stringPointer) : bool
    {
        $tokens = $phpcsFile->getTokens();
        $stringContent = $tokens[$stringPointer]['content'];
    ...
    private static function isUsedInScopeInternal(\PHP_CodeSniffer\Files\File $phpcsFile, int $scopeOwnerPointer, int $variablePointer, ?int $startCheckPointer) : bool
    {
        $tokens = $phpcsFile->getTokens();
        $scopeCloserPointer = $tokens[$scopeOwnerPointer]['code'] === \T_OPEN_TAG ? \count($tokens) - 1 : $tokens[$scopeOwnerPointer]['scope_closer'] - 1;
        ...
        for ($i = $startCheckPointer; $i <= $scopeCloserPointer; $i++) {
            ...
            if (\in_array($tokens[$i]['code'], [\T_DOUBLE_QUOTED_STRING, \T_HEREDOC], \true) && self::isUsedInScopeInString($phpcsFile, $tokens[$variablePointer]['content'], $i)) {
                return \true;
            }
        }
        return \false;
    }
    private static function isGetDefinedVarsCall(\PHP_CodeSniffer\Files\File $phpcsFile, int $stringPointer) : bool
    {
        $tokens = $phpcsFile->getTokens();
        $stringContent = $tokens[$stringPointer]['content'];
