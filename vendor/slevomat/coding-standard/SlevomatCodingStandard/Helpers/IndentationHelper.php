...
use const _PhpScoperaba240c3d5f1\T_END_NOWDOC;
use const T_START_HEREDOC;
use const _PhpScoperaba240c3d5f1\T_START_NOWDOC;
/**
 * @internal
 */
class IndentationHelper
{
    ...
    public static function getIndentation(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer) : string
    {
        $firstPointerOnLine = \SlevomatCodingStandard\Helpers\TokenHelper::findFirstTokenOnLine($phpcsFile, $pointer);
        return \SlevomatCodingStandard\Helpers\TokenHelper::getContent($phpcsFile, $firstPointerOnLine, $pointer - 1);
    }
    ...
    {
        return $identation === '' ? self::TAB_INDENT : ($identation[0] === self::TAB_INDENT ? self::TAB_INDENT : self::SPACES_INDENT);
    }
    /**
     ...
     * @return string
     */
    public static function fixIndentation(\PHP_CodeSniffer\Files\File $phpcsFile, array $codePointers, string $defaultIndentation) : string
    {
        ...
        foreach ($codePointers as $no => $codePointer) {
            ...
            if (\in_array($tokens[$codePointer]['code'], [\T_START_HEREDOC, \T_START_NOWDOC], \true)) {
                $inHeredoc = \true;
            } elseif (\in_array($tokens[$codePointer]['code'], [\T_END_HEREDOC, \T_END_NOWDOC], \true)) {
                $inHeredoc = \false;
            }
            $code .= $content;
        }
    ...
    public static function convertTabsToSpaces(\PHP_CodeSniffer\Files\File $phpcsFile, string $code) : string
    {
