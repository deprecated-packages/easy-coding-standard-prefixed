...
use const _PhpScoperfcf15c26e033\T_CLOSE_PARENTHESIS;
use const _PhpScoperfcf15c26e033\T_CLOSE_SHORT_ARRAY;
use const T_COALESCE;
use const _PhpScoperfcf15c26e033\T_COLON;
...
use const _PhpScoperfcf15c26e033\T_FALSE;
use const _PhpScoperfcf15c26e033\T_INLINE_ELSE;
use const _PhpScoperfcf15c26e033\T_INLINE_THEN;
use const T_INT_CAST;
use const T_LNUMBER;
use const T_LOGICAL_AND;
use const T_LOGICAL_OR;
use const T_LOGICAL_XOR;
use const _PhpScoperfcf15c26e033\T_MINUS;
use const T_NS_SEPARATOR;
use const _PhpScoperfcf15c26e033\T_NULL;
use const T_OBJECT_CAST;
use const _PhpScoperfcf15c26e033\T_OPEN_PARENTHESIS;
use const _PhpScoperfcf15c26e033\T_OPEN_SHORT_ARRAY;
use const T_OPEN_TAG;
use const _PhpScoperfcf15c26e033\T_PLUS;
use const T_RETURN;
use const _PhpScoperfcf15c26e033\T_SEMICOLON;
use const T_STRING;
use const T_STRING_CAST;
use const _PhpScoperfcf15c26e033\T_TRUE;
...
class YodaHelper
{
    ...
    public static function getLeftSideTokens(array $tokens, int $comparisonTokenPointer) : array
    {
        ...
        return self::trimWhitespaceTokens(\array_reverse($sideTokens, \true));
    }
    /**
     * @param array<int, array<string, array<int, int|string>|int|string>> $tokens
     * @param int $comparisonTokenPointer
    ...
    public static function getDynamismForTokens(array $tokens, array $sideTokens) : ?int
    {
        $sideTokens = \array_values(\array_filter($sideTokens, static function (array $token) : bool {
            return !\in_array($token['code'], [\T_WHITESPACE, \T_COMMENT, \T_DOC_COMMENT, \T_NS_SEPARATOR, \T_PLUS, \T_MINUS, \T_INT_CAST, \T_DOUBLE_CAST, \T_STRING_CAST, \T_ARRAY_CAST, \T_OBJECT_CAST, \T_BOOL_CAST, \T_UNSET_CAST], \true);
        }));
    ...
    public static function trimWhitespaceTokens(array $tokens) : array
    {
        ...
        foreach (\array_reverse($tokens, \true) as $pointer => $token) {
            if ($token['code'] !== \T_WHITESPACE) {
                break;
            }
            unset($tokens[$pointer]);
    ...
    private static function getStopTokenCodes() : array
    {
        ...
        if ($stopTokenCodes === null) {
            $stopTokenCodes = [\T_BOOLEAN_AND => \true, \T_BOOLEAN_OR => \true, \T_SEMICOLON => \true, \T_OPEN_TAG => \true, \T_INLINE_THEN => \true, \T_INLINE_ELSE => \true, \T_LOGICAL_AND => \true, \T_LOGICAL_OR => \true, \T_LOGICAL_XOR => \true, \T_COALESCE => \true, \T_CASE => \true, \T_COLON => \true, \T_RETURN => \true, \T_COMMA => \true, \T_CLOSE_CURLY_BRACKET => \true];
            $stopTokenCodes += \array_fill_keys(\array_keys(\PHP_CodeSniffer\Util\Tokens::$assignmentTokens), \true);
        }
