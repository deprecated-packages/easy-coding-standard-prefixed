...
class TypeHintHelper
{
    ...
    {
        return \in_array($typeHint, self::getSimpleTypeHints(), \true);
    }
    public static function isSimpleIterableTypeHint(string $typeHint) : bool
    {
        return \in_array($typeHint, self::getSimpleIterableTypeHints(), \true);
    }
    public static function convertLongSimpleTypeHintToShort(string $typeHint) : string
    {
        $longToShort = ['integer' => 'int', 'boolean' => 'bool'];
        return \array_key_exists($typeHint, $longToShort) ? $longToShort[$typeHint] : $typeHint;
    }
    public static function isTemplate(\PHP_CodeSniffer\Files\File $phpcsFile, int $docCommentOpenPointer, string $typeHint) : bool
    {
        ...
        if ($templateAnnotationNames === null) {
            foreach (['template', 'template-covariant'] as $annotationName) {
                ...
                foreach (\SlevomatCodingStandard\Helpers\AnnotationHelper::PREFIXES as $prefixAnnotatioName) {
        ...
        $docCommentOwnerPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findNext($phpcsFile, \array_merge([\T_FUNCTION], \SlevomatCodingStandard\Helpers\TokenHelper::$typeKeywordTokenCodes), $tokens[$docCommentOpenPointer]['comment_closer'] + 1);
        if ($docCommentOwnerPointer !== null && $tokens[$tokens[$docCommentOpenPointer]['comment_closer']]['line'] + 1 === $tokens[$docCommentOwnerPointer]['line']) {
        ...
        if ($docCommentOwnerPointer === null) {
            $functionPointer = \SlevomatCodingStandard\Helpers\TokenHelper::findPrevious($phpcsFile, \T_FUNCTION, $docCommentOpenPointer - 1);
            if ($functionPointer !== null) {
                $pointerToFindClass = $functionPointer;
            }
        }
        $classPointer = \SlevomatCodingStandard\Helpers\ClassHelper::getClassPointer($phpcsFile, $pointerToFindClass);
        if ($classPointer === null) {
            return \false;
        }
        $classDocCommentOpenPointer = \SlevomatCodingStandard\Helpers\DocCommentHelper::findDocCommentOpenToken($phpcsFile, $classPointer);
    ...
    public static function getFullyQualifiedTypeHint(\PHP_CodeSniffer\Files\File $phpcsFile, int $pointer, string $typeHint) : string
    {
        ...
        return \SlevomatCodingStandard\Helpers\NamespaceHelper::resolveClassName($phpcsFile, $typeHint, $pointer);
    }
    /**
     * @return string[]
     */
    public static function getSimpleTypeHints() : array
    {
        ...
        if ($simpleTypeHints === null) {
            $simpleTypeHints = ['int', 'integer', 'float', 'string', 'bool', 'boolean', 'callable', 'self', 'array', 'iterable', 'void'];
        }
    ...
    public static function isSimpleUnofficialTypeHints(string $typeHint) : bool
    {
        ...
        if ($simpleUnofficialTypeHints === null) {
            $simpleUnofficialTypeHints = ['null', 'mixed', 'scalar', 'numeric', 'true', 'false', 'object', 'resource', 'static', '$this', 'class-string', 'trait-string', 'callable-string', 'numeric-string', 'array-key', 'list', 'empty'];
        }
        return \in_array($typeHint, $simpleUnofficialTypeHints, \true);
    }
    /**
     * @param string $type
    ...
    {
        return self::isSimpleIterableTypeHint($type) || \in_array($type, $traversableTypeHints, \true);
    }
    public static function typeHintEqualsAnnotation(\PHP_CodeSniffer\Files\File $phpcsFile, int $functionPointer, string $typeHint, string $typeHintInAnnotation) : bool
    {
