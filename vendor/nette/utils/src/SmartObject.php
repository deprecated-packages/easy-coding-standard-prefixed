...
namespace _PhpScoper890197fe38f7\Nette;
...
trait SmartObject
{
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (ObjectHelpers::hasProperty($class, $name) === 'event') {
            ...
            if (\is_iterable($handlers)) {
                ...
                throw new \_PhpScoper890197fe38f7\Nette\UnexpectedValueException("Property {$class}::\${$name} must be iterable or null, " . \gettype($handlers) . ' given.');
            }
        } else {
    ...
    public function &__get(string $name)
    {
        ...
        if ($prop = ObjectHelpers::getMagicProperties($class)[$name] ?? null) {
            // property getter
            if (!($prop & 0b1)) {
                throw new \_PhpScoper890197fe38f7\Nette\MemberAccessException("Cannot read a write-only property {$class}::\${$name}.");
            }
    ...
    public function __set(string $name, $value)
    {
        ...
        if (ObjectHelpers::hasProperty($class, $name)) {
            ...
            // property setter
            if (!($prop & 0b1000)) {
                throw new \_PhpScoper890197fe38f7\Nette\MemberAccessException("Cannot write to a read-only property {$class}::\${$name}.");
            }
            $this->{'set' . $name}($value);
        } else {
    ...
    public function __unset(string $name)
    {
        $class = static::class;
        if (!ObjectHelpers::hasProperty($class, $name)) {
            throw new \_PhpScoper890197fe38f7\Nette\MemberAccessException("Cannot unset the property {$class}::\${$name}.");
        }
    }
