...
class Validators
{
    ...
    protected static $validators = [
        ...
        'list' => [\_PhpScopera238de2e9b5a\Nette\Utils\Arrays::class, 'isList'],
    ...
    protected static $counters = ['string' => 'strlen', 'unicode' => [\_PhpScopera238de2e9b5a\Nette\Utils\Strings::class, 'length'], 'array' => 'count', 'list' => 'count', 'alnum' => 'strlen', 'alpha' => 'strlen', 'digit' => 'strlen', 'lower' => 'strlen', 'space' => 'strlen', 'upper' => 'strlen', 'xdigit' => 'strlen'];
    /**
    ...
    public static function assert($value, string $expected, string $label = 'variable') : void
    {
        if (!static::is($value, $expected)) {
            ...
            throw new \_PhpScopera238de2e9b5a\Nette\Utils\AssertionException("The {$label} expects to be {$expected}, {$type} given.");
        }
    }
    /**
    ...
    public static function assertField(array $arr, $field, string $expected = null, string $label = "item '%' in array") : void
    {
        if (!\array_key_exists($field, $arr)) {
            throw new \_PhpScopera238de2e9b5a\Nette\Utils\AssertionException('Missing ' . \str_replace('%', $field, $label) . '.');
        } elseif ($expected) {
    ...
    public static function is($value, string $expected) : bool
    {
        foreach (\explode('|', $expected) as $item) {
            ...
            if (isset(static::$validators[$type])) {
                ...
                if (\_PhpScopera238de2e9b5a\Nette\Utils\Strings::match($value, '|^' . ($item[1] ?? '') . '$|D')) {
                    return \true;
                }
                continue;
    ...
    {
        return \_PhpScopera238de2e9b5a\Nette\Utils\Arrays::isList($value);
    }
    /**
