...
final class FileSystem
{
    ...
    public static function createDir(string $dir, int $mode = 0777) : void
    {
        if (!\is_dir($dir) && !@\mkdir($dir, $mode, \true) && !\is_dir($dir)) {
            ...
            throw new Nette\IOException("Unable to create directory '{$dir}' with mode " . \decoct($mode) . '. ' . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
        }
    }
    /**
    ...
    public static function copy(string $origin, string $target, bool $overwrite = \true) : void
    {
        if (\stream_is_local($origin) && !\file_exists($origin)) {
            ...
            if (($s = @\fopen($origin, 'rb')) && ($d = @\fopen($target, 'wb')) && @\stream_copy_to_stream($s, $d) === \false) {
                ...
                throw new Nette\IOException("Unable to copy file '{$origin}' to '{$target}'. " . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
            }
        }
    }
    /**
    ...
    public static function delete(string $path) : void
    {
        if (\is_file($path) || \is_link($path)) {
            ...
            if (!@$func($path)) {
                // @ is escalated to exception
                throw new Nette\IOException("Unable to delete '{$path}'. " . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
            }
        } elseif (\is_dir($path)) {
            ...
            if (!@\rmdir($path)) {
                // @ is escalated to exception
                throw new Nette\IOException("Unable to delete directory '{$path}'. " . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
            }
        }
    }
    /**
    ...
    public static function rename(string $origin, string $target, bool $overwrite = \true) : void
    {
        if (!$overwrite && \file_exists($target)) {
            ...
            if (!@\rename($origin, $target)) {
                // @ is escalated to exception
                throw new Nette\IOException("Unable to rename file or directory '{$origin}' to '{$target}'. " . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
            }
        }
    }
    /**
    ...
    public static function read(string $file) : string
    {
        ...
        // @ is escalated to exception
        if ($content === \false) {
            throw new Nette\IOException("Unable to read file '{$file}'. " . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
        }
        return $content;
    }
    /**
    ...
    public static function write(string $file, string $content, ?int $mode = 0666) : void
    {
        ...
        if (@\file_put_contents($file, $content) === \false) {
            // @ is escalated to exception
            throw new Nette\IOException("Unable to write file '{$file}'. " . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
        }
        if ($mode !== null && !@\chmod($file, $mode)) {
            // @ is escalated to exception
            throw new Nette\IOException("Unable to chmod file '{$file}' to mode " . \decoct($mode) . '. ' . \_PhpScopercae9e6ab5cea\Nette\Utils\Helpers::getLastError());
        }
    }
    /**
