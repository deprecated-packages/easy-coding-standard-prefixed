...
class Arrays
{
    ...
    public static function get(array $arr, $key, $default = null)
    {
        foreach (\is_array($key) ? $key : [$key] as $k) {
            if (\is_array($arr) && \array_key_exists($k, $arr)) {
                ...
                if (\func_num_args() < 3) {
                    throw new \_PhpScopere4fa57261c04\Nette\InvalidArgumentException("Missing item '{$k}'.");
                }
                return $default;
            }
        }
    ...
    public static function &getRef(array &$arr, $key)
    {
        foreach (\is_array($key) ? $key : [$key] as $k) {
            if (\is_array($arr) || $arr === null) {
                ...
                throw new \_PhpScopere4fa57261c04\Nette\InvalidArgumentException('Traversed item is not an array.');
            }
        }
        return $arr;
    }
    ...
    {
        return \_PhpScopere4fa57261c04\Nette\Utils\Strings::pcre('preg_grep', [$pattern, $arr, $flags]);
    }
    /**
    ...
    public static function associate(array $arr, $path)
    {
        ...
        if (!$parts || $parts[0] === '=' || $parts[0] === '|' || $parts === ['->']) {
            throw new \_PhpScopere4fa57261c04\Nette\InvalidArgumentException("Invalid path '{$path}'.");
        }
    ...
    public static function pick(array &$arr, $key, $default = null)
    {
        if (\array_key_exists($key, $arr)) {
            ...
            throw new \_PhpScopere4fa57261c04\Nette\InvalidArgumentException("Missing item '{$key}'.");
        } else {
            return $default;
        }
    }
