...
class Image
{
    ...
    public static function fromFile(string $file, int &$type = null)
    {
        ...
        if (!$type) {
            throw new \_PhpScopera22bb3f4d7b7\Nette\Utils\UnknownImageFileException(\is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
        $method = 'imagecreatefrom' . self::FORMATS[$type];
        return new static(\_PhpScopera22bb3f4d7b7\Nette\Utils\Callback::invokeSafe($method, [$file], function (string $message) : void {
            throw new \_PhpScopera22bb3f4d7b7\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromString(string $s, int &$type = null)
    {
        ...
        if (!$type) {
            throw new \_PhpScopera22bb3f4d7b7\Nette\Utils\UnknownImageFileException('Unknown type of image.');
        }
        return new static(\_PhpScopera22bb3f4d7b7\Nette\Utils\Callback::invokeSafe('imagecreatefromstring', [$s], function (string $message) : void {
            throw new \_PhpScopera22bb3f4d7b7\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    {
        if (!$image instanceof \_PhpScopera22bb3f4d7b7\GdImage && !(\is_resource($image) && \get_resource_type($image) === 'gd')) {
    ...
    {
        return \_PhpScopera22bb3f4d7b7\Nette\Utils\Helpers::capture(function () use($type, $quality) {
    ...
    private function output(int $type, ?int $quality, string $file = null) : void
    {
        ...
        if (!$success) {
            throw new \_PhpScopera22bb3f4d7b7\Nette\Utils\ImageException(\_PhpScopera22bb3f4d7b7\Nette\Utils\Helpers::getLastError() ?: 'Unknown error');
        }
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!\function_exists($function)) {
            \_PhpScopera22bb3f4d7b7\Nette\Utils\ObjectHelpers::strictCall(static::class, $name);
        }
        ...
        return $res instanceof \_PhpScopera22bb3f4d7b7\GdImage || \is_resource($res) && \get_resource_type($res) === 'gd' ? $this->setImageResource($res) : $res;
    }
