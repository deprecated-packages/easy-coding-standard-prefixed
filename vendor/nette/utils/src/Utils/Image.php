...
class Image
{
    ...
    public static function fromFile(string $file, int &$detectedFormat = null)
    {
        if (!\extension_loaded('gd')) {
            throw new \_PhpScoper21763e6c7ac4\Nette\NotSupportedException('PHP extension GD is not loaded.');
        }
        $detectedFormat = @\getimagesize($file)[2];
        // @ - files smaller than 12 bytes causes read error
        if (!isset(self::FORMATS[$detectedFormat])) {
            $detectedFormat = null;
            throw new \_PhpScoper21763e6c7ac4\Nette\Utils\UnknownImageFileException(\is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
        return new static(\_PhpScoper21763e6c7ac4\Nette\Utils\Callback::invokeSafe('imagecreatefrom' . \image_type_to_extension($detectedFormat, \false), [$file], function (string $message) : void {
            throw new \_PhpScoper21763e6c7ac4\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromString(string $s, int &$detectedFormat = null)
    {
        ...
        return new static(\_PhpScoper21763e6c7ac4\Nette\Utils\Callback::invokeSafe('imagecreatefromstring', [$s], function (string $message) : void {
            throw new \_PhpScoper21763e6c7ac4\Nette\Utils\ImageException($message);
        }));
    }
    /**
    ...
    public static function fromBlank(int $width, int $height, array $color = null)
    {
        ...
        if ($width < 1 || $height < 1) {
            throw new \_PhpScoper21763e6c7ac4\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
        }
    ...
    protected function setImageResource($image)
    {
        if (!\is_resource($image) || \get_resource_type($image) !== 'gd') {
            throw new \_PhpScoper21763e6c7ac4\Nette\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
    ...
    public static function calculateSize(int $srcWidth, int $srcHeight, $newWidth, $newHeight, int $flags = self::FIT) : array
    {
        ...
        if ($flags & self::STRETCH) {
            ...
            if (empty($newWidth) || empty($newHeight)) {
                throw new \_PhpScoper21763e6c7ac4\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            // proportional
            if (empty($newWidth) && empty($newHeight)) {
                throw new \_PhpScoper21763e6c7ac4\Nette\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
            if ($newWidth > 0) {
                // fit width
    ...
    public function save(string $file, int $quality = null, int $type = null) : void
    {
        if ($type === null) {
            ...
            if (!isset($extensions[$ext])) {
                throw new \_PhpScoper21763e6c7ac4\Nette\InvalidArgumentException("Unsupported file extension '{$ext}'.");
            }
            $type = $extensions[$ext];
        }
    ...
    {
        return \_PhpScoper21763e6c7ac4\Nette\Utils\Helpers::capture(function () use($type, $quality) {
    ...
    public function send(int $type = self::JPEG, int $quality = null) : void
    {
        if (!isset(self::FORMATS[$type])) {
            throw new \_PhpScoper21763e6c7ac4\Nette\InvalidArgumentException("Unsupported image type '{$type}'.");
        }
    ...
    private function output(int $type, ?int $quality, string $file = null) : void
    {
        ...
        if (!$success) {
            throw new \_PhpScoper21763e6c7ac4\Nette\Utils\ImageException(\_PhpScoper21763e6c7ac4\Nette\Utils\Helpers::getLastError() ?: 'Unknown error');
        }
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!\function_exists($function)) {
            \_PhpScoper21763e6c7ac4\Nette\Utils\ObjectHelpers::strictCall(\get_class($this), $name);
        }
    ...
    {
        throw new \_PhpScoper21763e6c7ac4\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
    }
}
