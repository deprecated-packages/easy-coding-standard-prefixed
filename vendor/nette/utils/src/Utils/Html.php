...
use _PhpScopercae9e6ab5cea\Nette\HtmlStringable;
...
class Html implements \ArrayAccess, \Countable, \IteratorAggregate, HtmlStringable
{
    ...
    public static function el(string $name = null, $attrs = null)
    {
        ...
        if (isset($parts[1])) {
            foreach (\_PhpScopercae9e6ab5cea\Nette\Utils\Strings::matchAll($parts[1] . ' ', '#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\\s))?#i') as $m) {
        ...
        foreach ($attrs as $key => $value) {
            if ($value === null || $value === \false) {
                ...
                if (\strncmp($key, 'data-', 5) === 0) {
                    $value = \_PhpScopercae9e6ab5cea\Nette\Utils\Json::encode($value);
                } else {
                    $tmp = null;
