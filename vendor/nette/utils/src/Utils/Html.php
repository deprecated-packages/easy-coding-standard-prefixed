 ...
 */
class Html implements \ArrayAccess, \Countable, \IteratorAggregate, \_PhpScoperf77bffce0320\Nette\Utils\IHtmlString
{
    ...
    public static function el(string $name = null, $attrs = null)
    {
        ...
        if (isset($parts[1])) {
            foreach (\_PhpScoperf77bffce0320\Nette\Utils\Strings::matchAll($parts[1] . ' ', '#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\\s))?#i') as $m) {
    ...
    {
        if (!$text instanceof \_PhpScoperf77bffce0320\Nette\Utils\IHtmlString) {
        ...
        foreach ($attrs as $key => $value) {
            if ($value === null || $value === \false) {
                ...
                if (\strncmp($key, 'data-', 5) === 0) {
                    $value = \_PhpScoperf77bffce0320\Nette\Utils\Json::encode($value);
                } else {
                    $tmp = null;
