...
final class Decoder
{
    ...
    public function decode(string $input)
    {
        ...
        if ($this->tokens === \false) {
            throw new \_PhpScoperc5bee3a837bb\Nette\Neon\Exception('Invalid UTF-8 sequence.');
        }
        ...
        for (; $n < $count; $n++) {
            ...
            if ($t === ',') {
                ...
                if ($hasValue) {
                    ...
                    $n++;
                    if ($value instanceof \_PhpScoperc5bee3a837bb\Nette\Neon\Entity && $value->value === \_PhpScoperc5bee3a837bb\Nette\Neon\Neon::CHAIN) {
                        \end($value->attributes)->attributes = $this->parse(\false, []);
                    } else {
                        $value = new \_PhpScoperc5bee3a837bb\Nette\Neon\Entity($value, $this->parse(\false, []));
                    }
                } else {
                    $n++;
                ...
                if ($hasValue) {
                    if ($value instanceof \_PhpScoperc5bee3a837bb\Nette\Neon\Entity) {
                        // Entity chaining
                        if ($value->value !== \_PhpScoperc5bee3a837bb\Nette\Neon\Neon::CHAIN) {
                            $value = new \_PhpScoperc5bee3a837bb\Nette\Neon\Entity(\_PhpScoperc5bee3a837bb\Nette\Neon\Neon::CHAIN, [$value]);
                        }
                        $value->attributes[] = new \_PhpScoperc5bee3a837bb\Nette\Neon\Entity($converted);
                    } else {
                        $this->error();
                    }
                } else {
    ...
    private function error(string $message = "Unexpected '%s'")
    {
        ...
        throw new \_PhpScoperc5bee3a837bb\Nette\Neon\Exception(\str_replace('%s', $token, $message) . " on line {$line}, column {$col}.");
    }
}
