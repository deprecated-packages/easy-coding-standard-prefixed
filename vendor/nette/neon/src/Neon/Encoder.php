...
final class Encoder
{
    ...
    public function encode($var, int $flags = 0) : string
    {
        ...
        } elseif ($var instanceof \_PhpScopera88a8b9f064a\Nette\Neon\Entity) {
            if ($var->value === \_PhpScopera88a8b9f064a\Nette\Neon\Neon::CHAIN) {
        ...
        } elseif (\is_string($var) && !\preg_match('~[\\x00-\\x1F]|^[+-.]?\\d|^(true|false|yes|no|on|off|null)$~Di', $var) && \preg_match('~^' . \_PhpScopera88a8b9f064a\Nette\Neon\Decoder::PATTERNS[1] . '$~Dx', $var)) {
            return $var;
        } elseif (\is_string($var)) {
            if (!\preg_match('~[\\x00-\\x1F]|^[+-.]?\\d|^(true|false|yes|no|on|off|null)$~Di', $var) && \preg_match('~^' . \_PhpScopera88a8b9f064a\Nette\Neon\Decoder::PATTERNS[1] . '$~Dx', $var)) {
                return $var;
            }
            $res = \json_encode($var, \JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES);
            if ($res === \false) {
                throw new \_PhpScopera88a8b9f064a\Nette\Neon\Exception('Invalid UTF-8 sequence: ' . $var);
            }
            if (\strpos($var, "\n") !== \false) {
