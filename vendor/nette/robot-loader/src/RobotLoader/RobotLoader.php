...
namespace _PhpScoperd8b0b9452568\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \_PhpScoperd8b0b9452568\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
            ...
            foreach ($iterator as $fileInfo) {
                ...
                foreach ($foundClasses as $class) {
                    if (isset($this->classes[$class])) {
                        throw new \_PhpScoperd8b0b9452568\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$this->classes[$class][0]} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \_PhpScoperd8b0b9452568\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \_PhpScoperd8b0b9452568\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \_PhpScoperd8b0b9452568\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    private function updateFile(string $file) : void
    {
        ...
        foreach ($foundClasses as $class) {
            ...
            if (isset($prevFile)) {
                throw new \_PhpScoperd8b0b9452568\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$prevFile} and in {$file}.");
            }
    ...
    {
        \_PhpScoperd8b0b9452568\Nette\Utils\FileSystem::createDir($dir);
