...
namespace _PhpScoper57c79939064b\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \_PhpScoper57c79939064b\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
            ...
            foreach ($iterator as $fileInfo) {
                ...
                foreach ($foundClasses as $class) {
                    if (isset($this->classes[$class])) {
                        throw new \_PhpScoper57c79939064b\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$this->classes[$class][0]} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \_PhpScoper57c79939064b\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \_PhpScoper57c79939064b\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \_PhpScoper57c79939064b\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    private function updateFile(string $file) : void
    {
        ...
        foreach ($foundClasses as $class) {
            ...
            if (isset($prevFile)) {
                throw new \_PhpScoper57c79939064b\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$prevFile} and in {$file}.");
            }
    ...
    {
        \_PhpScoper57c79939064b\Nette\Utils\FileSystem::createDir($dir);
