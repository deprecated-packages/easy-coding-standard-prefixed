...
namespace _PhpScoperc8fea59b0cb1\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \_PhpScoperc8fea59b0cb1\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
    /**
     * Register autoloader.
        ...
        foreach ($this->scanPaths as $path) {
            ...
            foreach ($iterator as $file) {
                ...
                foreach ($classes as $class) {
                    ...
                    if (isset($info['file'])) {
                        throw new \_PhpScoperc8fea59b0cb1\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$info['file']} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \_PhpScoperc8fea59b0cb1\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \_PhpScoperc8fea59b0cb1\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \_PhpScoperc8fea59b0cb1\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    {
        \_PhpScoperc8fea59b0cb1\Nette\Utils\FileSystem::createDir($dir);
