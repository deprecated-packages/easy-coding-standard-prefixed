...
namespace _PhpScoper3e1e0e5bb8ef\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \_PhpScoper3e1e0e5bb8ef\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
    /**
     * Register autoloader.
        ...
        foreach ($this->scanPaths as $path) {
            ...
            foreach ($iterator as $file) {
                ...
                foreach ($classes as $class) {
                    ...
                    if (isset($info['file'])) {
                        throw new \_PhpScoper3e1e0e5bb8ef\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$info['file']} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \_PhpScoper3e1e0e5bb8ef\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \_PhpScoper3e1e0e5bb8ef\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \_PhpScoper3e1e0e5bb8ef\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    {
        \_PhpScoper3e1e0e5bb8ef\Nette\Utils\FileSystem::createDir($dir);
