...
namespace _PhpScoperb6d4bd368bd9\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \_PhpScoperb6d4bd368bd9\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
    /**
     * Register autoloader.
        ...
        foreach ($this->scanPaths as $path) {
            ...
            foreach ($iterator as $file) {
                ...
                foreach ($classes as $class) {
                    ...
                    if (isset($info['file'])) {
                        throw new \_PhpScoperb6d4bd368bd9\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$info['file']} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \_PhpScoperb6d4bd368bd9\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \_PhpScoperb6d4bd368bd9\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \_PhpScoperb6d4bd368bd9\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    {
        \_PhpScoperb6d4bd368bd9\Nette\Utils\FileSystem::createDir($dir);
