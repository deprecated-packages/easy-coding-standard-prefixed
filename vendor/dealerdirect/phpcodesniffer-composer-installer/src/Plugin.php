 ...
 * @license MIT
 */
namespace _PhpScoperbd5fb781fe24\Dealerdirect\Composer\Plugin\Installers\PHPCodeSniffer;

use _PhpScoperbd5fb781fe24\Composer\Composer;
use _PhpScoperbd5fb781fe24\Composer\EventDispatcher\EventSubscriberInterface;
use _PhpScoperbd5fb781fe24\Composer\IO\IOInterface;
use _PhpScoperbd5fb781fe24\Composer\Package\AliasPackage;
use _PhpScoperbd5fb781fe24\Composer\Package\PackageInterface;
use _PhpScoperbd5fb781fe24\Composer\Package\RootPackageInterface;
use _PhpScoperbd5fb781fe24\Composer\Plugin\PluginInterface;
use _PhpScoperbd5fb781fe24\Composer\Script\Event;
use _PhpScoperbd5fb781fe24\Composer\Script\ScriptEvents;
use _PhpScoperbd5fb781fe24\Composer\Util\Filesystem;
use _PhpScoperbd5fb781fe24\Composer\Util\ProcessExecutor;
use _PhpScoperbd5fb781fe24\Symfony\Component\Finder\Finder;
...
use _PhpScoperbd5fb781fe24\Symfony\Component\Process\PhpExecutableFinder;
/**
 * PHP_CodeSniffer standard installation manager.
 *
 * @author Franck Nijhof <franck.nijhof@dealerdirect.com>
 */
class Plugin implements \_PhpScoperbd5fb781fe24\Composer\Plugin\PluginInterface, \_PhpScoperbd5fb781fe24\Composer\EventDispatcher\EventSubscriberInterface
{
     ...
     * @throws RuntimeException
     */
    public static function run(\_PhpScoperbd5fb781fe24\Composer\Script\Event $event)
    {
        $io = $event->getIO();
     ...
     * @throws RuntimeException
     */
    public function activate(\_PhpScoperbd5fb781fe24\Composer\Composer $composer, \_PhpScoperbd5fb781fe24\Composer\IO\IOInterface $io)
    {
     ...
     * {@inheritDoc}
     */
    public function deactivate(\_PhpScoperbd5fb781fe24\Composer\Composer $composer, \_PhpScoperbd5fb781fe24\Composer\IO\IOInterface $io)
    {
    }
    /**
     * {@inheritDoc}
     */
    public function uninstall(\_PhpScoperbd5fb781fe24\Composer\Composer $composer, \_PhpScoperbd5fb781fe24\Composer\IO\IOInterface $io)
    {
    }
    /**
    ...
    private function init()
    {
        ...
        $this->processExecutor = new \_PhpScoperbd5fb781fe24\Composer\Util\ProcessExecutor($this->io);
        $this->filesystem = new \_PhpScoperbd5fb781fe24\Composer\Util\Filesystem($this->processExecutor);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return array(\_PhpScoperbd5fb781fe24\Composer\Script\ScriptEvents::POST_INSTALL_CMD => array(array('onDependenciesChangedEvent', 0)), \_PhpScoperbd5fb781fe24\Composer\Script\ScriptEvents::POST_UPDATE_CMD => array(array('onDependenciesChangedEvent', 0)));
    }
    /**
    ...
    public function onDependenciesChangedEvent()
    {
        ...
        if ($this->isPHPCodeSnifferInstalled() === \true) {
            $this->loadInstalledPaths();
            $installPathCleaned = $this->cleanInstalledPaths();
            $installPathUpdated = $this->updateInstalledPaths();
            if ($installPathCleaned === \true || $installPathUpdated === \true) {
                $exitCode = $this->saveInstalledPaths();
            } elseif ($isVerbose) {
                $io->write(\sprintf('<info>%s</info>', self::MESSAGE_NOTHING_TO_INSTALL));
            }
        } else {
            $pluginPackage = $this->composer->getRepositoryManager()->getLocalRepository()->findPackages(self::PLUGIN_NAME);
    ...
    {
        if ($this->isPHPCodeSnifferInstalled() === \true) {
            $this->processExecutor->execute(\sprintf('phpcs --config-show %s', self::PHPCS_CONFIG_KEY), $output, $this->composer->getConfig()->get('bin-dir'));
            ...
            if (\preg_match($regex, $output, $match) === 1) {
                ...
                $phpcsInstalledPaths = \trim($phpcsInstalledPaths);
    ...
    private function saveInstalledPaths()
    {
        ...
        if (\file_exists($phpcsPath . '/bin/phpcs') === \true) {
            // PHPCS 3.x.
        ...
        // Okay, lets rock!
        $command = \vsprintf('%s %s %s', array('php executable' => $this->getPhpExecCommand(), 'phpcs executable' => $phpcsExecutable, 'arguments' => \implode(' ', $arguments)));
    ...
    {
        $finder = new \_PhpScoperbd5fb781fe24\Symfony\Component\Process\PhpExecutableFinder();
        ...
        $command = \_PhpScoperbd5fb781fe24\Composer\Util\ProcessExecutor::escape($phpPath) . $phpArgs . ' -d allow_url_fopen=' . \_PhpScoperbd5fb781fe24\Composer\Util\ProcessExecutor::escape(\ini_get('allow_url_fopen')) . ' -d disable_functions=' . \_PhpScoperbd5fb781fe24\Composer\Util\ProcessExecutor::escape(\ini_get('disable_functions')) . ' -d memory_limit=' . \_PhpScoperbd5fb781fe24\Composer\Util\ProcessExecutor::escape(\ini_get('memory_limit'));
        return $command;
    }
    /**
    ...
    private function cleanInstalledPaths()
    {
        ...
        foreach ($this->installedPaths as $key => $path) {
            ...
            if ((\is_dir($path) === \false || \is_readable($path) === \false) && (\is_dir($alternativePath) === \false || \is_readable($alternativePath) === \false)) {
    ...
    private function updateInstalledPaths()
    {
        ...
        foreach ($codingStandardPackages as $package) {
            ...
            if ($this->filesystem->isAbsolutePath($installPath) === \false) {
                $installPath = $this->filesystem->normalizePath($this->cwd . \DIRECTORY_SEPARATOR . $installPath);
            }
            $searchPaths[] = $installPath;
        }
        $finder = new \_PhpScoperbd5fb781fe24\Symfony\Component\Finder\Finder();
        ...
        foreach ($finder as $ruleset) {
            ...
            if ($this->isRunningGlobally() === \false) {
                $standardsPath = $this->filesystem->findShortestPath($this->getPHPCodeSnifferInstallPath(), $standardsPath, \true);
            }
            // De-duplicate and add when directory is not configured.
            if (\in_array($standardsPath, $this->installedPaths, \true) === \false) {
    ...
    {
        $codingStandardPackages = \array_filter($this->composer->getRepositoryManager()->getLocalRepository()->getPackages(), function (\_PhpScoperbd5fb781fe24\Composer\Package\PackageInterface $package) {
            if ($package instanceof \_PhpScoperbd5fb781fe24\Composer\Package\AliasPackage) {
                return \false;
            }
            return $package->getType() === \_PhpScoperbd5fb781fe24\Dealerdirect\Composer\Plugin\Installers\PHPCodeSniffer\Plugin::PACKAGE_TYPE;
        });
        if (!$this->composer->getPackage() instanceof \_PhpScoperbd5fb781fe24\Composer\Package\RootPackageInterface && $this->composer->getPackage()->getType() === self::PACKAGE_TYPE) {
    ...
    {
        $packages = $this->composer->getRepositoryManager()->getLocalRepository()->findPackages(self::PACKAGE_NAME, $versionConstraint);
    ...
    private function getMaxDepth()
    {
        ...
        if (\array_key_exists(self::KEY_MAX_DEPTH, $extra)) {
            ...
            if ((string) (int) $maxDepth !== (string) $maxDepth || $maxDepth <= $minDepth || \is_float($maxDepth) === \true) {
                $message = \vsprintf(self::MESSAGE_ERROR_WRONG_MAX_DEPTH, array('key' => self::KEY_MAX_DEPTH, 'min' => $minDepth, 'given' => \var_export($maxDepth, \true)));
    ...
    {
        if ($this->isPHPCodeSnifferInstalled('>= 3.0.0') !== \true) {
            return 1;
        }
        return 0;
    }
