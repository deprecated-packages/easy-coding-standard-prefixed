...
class File
{
    ...
    public function getMethodProperties($stackPtr)
    {
        ...
        return ['scope' => $scope, 'scope_specified' => $scopeSpecified, 'return_type' => $returnType, 'return_type_token' => $returnTypeToken, 'nullable_return_type' => $nullableReturnType, 'is_abstract' => $isAbstract, 'is_final' => $isFinal, 'is_static' => $isStatic, 'has_body' => $hasBody];
    }
    ...
    public function getMemberProperties($stackPtr)
    {
        ...
        $scopeSpecified = \false;
        $isStatic = \false;
        $startOfStatement = $this->findPrevious([T_SEMICOLON, T_OPEN_CURLY_BRACKET, T_CLOSE_CURLY_BRACKET], $stackPtr - 1);
    ...
    {
        $endTokens = \PHP_CodeSniffer\Util\Tokens::$blockOpeners;
        $endTokens[T_COLON] = \true;
        $endTokens[T_COMMA] = \true;
        $endTokens[\T_DOUBLE_ARROW] = \true;
        $endTokens[T_SEMICOLON] = \true;
        $endTokens[\T_OPEN_TAG] = \true;
        $endTokens[\T_CLOSE_TAG] = \true;
        $endTokens[T_OPEN_SHORT_ARRAY] = \true;
        if ($ignore !== null) {
            $ignore = (array) $ignore;
        ...
        $lastNotEmpty = $start;
        for ($i = $start; $i >= 0; $i--) {
            if (isset($endTokens[$this->tokens[$i]['code']]) === \true) {
