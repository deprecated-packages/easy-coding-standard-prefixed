...
class ClassDeclarationSniff extends PEARClassDeclarationSniff
{
    ...
    public function processOpen($phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr - 1]['code'] === \T_WHITESPACE) {
            ...
            if ($prevContent !== $phpcsFile->eolChar) {
                ...
                if (\in_array($tokens[$stackPtr - 2]['code'], [\T_ABSTRACT, \T_FINAL], \true) === \true && $spaces !== 1) {
                    $prevContent = \strtolower($tokens[$stackPtr - 2]['content']);
                    $error = 'Expected 1 space between %s and %s keywords; %s found';
                    $data = [$prevContent, $stackPtrType, $spaces];
        ...
        $find = [\T_STRING, $keywordTokenType];
        if ($className !== null) {
            $start = $className;
        } else {
            ...
            if ($checkingImplements === \true && $multiLineImplements === \true && ($tokens[$className - 1]['code'] !== \T_NS_SEPARATOR || $tokens[$className - 2]['code'] !== \T_STRING)) {
                ...
                if ($tokens[$className - 1]['code'] !== \T_NS_SEPARATOR || $tokens[$className - 2]['code'] !== \T_STRING) {
                    ...
                    if ($tokens[$className - 1]['code'] === T_COMMA || $tokens[$className - 1]['code'] === \T_NS_SEPARATOR && $tokens[$className - 2]['code'] === T_COMMA) {
                        ...
                        $last = $phpcsFile->findPrevious(\T_WHITESPACE, $prev, null, \true);
                        $content = $phpcsFile->getTokensAsString($last + 1, $prev - $last);
                        if ($content !== ' ') {
                            $found = \strlen($content);
                            $error = 'Expected 1 space before "%s"; %s found';
                            $data = [$tokens[$className]['content'], $found];
            ...
            if ($checkingImplements === \true && $tokens[$className + 1]['code'] !== \T_NS_SEPARATOR && $tokens[$className + 1]['code'] !== T_COMMA) {
                if ($n !== $classCount - 1) {
                    ...
                    if ($tokens[$className + 2]['code'] !== $keywordTokenType) {
                        ...
                        $data = [$tokens[$className]['content'], $tokens[$className + 1]['length']];
    ...
    public function processClose($phpcsFile, $stackPtr)
    {
        ...
        $prevContent = $phpcsFile->findPrevious(\T_WHITESPACE, $closeBrace - 1, null, \true);
