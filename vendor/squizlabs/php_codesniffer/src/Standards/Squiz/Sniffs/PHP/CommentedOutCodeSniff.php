...
class CommentedOutCodeSniff implements Sniff
{
    ...
    public function process($phpcsFile, $stackPtr)
    {
        ...
        for ($i = $stackPtr; $i < $phpcsFile->numTokens; $i++) {
            ...
            if ($commentStyle === 'line' && $lastLineSeen + 1 <= $tokens[$i]['line'] && \strpos($tokens[$i]['content'], '/*') === 0) {
                // First non-whitespace token on a new line is start of a different style comment.
                break;
            }
            if ($commentStyle === 'line' && $lastLineSeen + 1 < $tokens[$i]['line']) {
            ...
            */
            $tokenContent = \trim($tokens[$i]['content']);
            $break = \false;
