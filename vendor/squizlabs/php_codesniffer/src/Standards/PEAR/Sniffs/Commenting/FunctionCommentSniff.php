...
class FunctionCommentSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$commentEnd]['code'] !== T_DOC_COMMENT_CLOSE_TAG && $tokens[$commentEnd]['code'] !== \T_COMMENT) {
            ...
            $phpcsFile->addError('Missing doc comment for function %s()', $stackPtr, 'Missing', [$function]);
    ...
    protected function processParams(\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, $commentStart)
    {
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $pos => $tag) {
            ...
            if ($tokens[$tag + 2]['code'] === T_DOC_COMMENT_STRING) {
                ...
                if (isset($matches[2]) === \true) {
                    $var = $matches[2];
                    $varLen = \strlen($var);
                    if ($varLen > $maxVar) {
                        $maxVar = $varLen;
                    }
                    if (isset($matches[4]) === \true) {
                        $varSpace = \strlen($matches[3]);
                        $comment = $matches[4];
                        // Any strings until the next tag belong to this comment.
                        if (isset($tokens[$commentStart]['comment_tags'][$pos + 1]) === \true) {
            ...
            //end if
            $params[] = ['tag' => $tag, 'type' => $type, 'var' => $var, 'comment' => $comment, 'comment_end' => $commentEnd, 'comment_tokens' => $commentTokens, 'type_space' => $typeSpace, 'var_space' => $varSpace];
        }
        //end foreach
