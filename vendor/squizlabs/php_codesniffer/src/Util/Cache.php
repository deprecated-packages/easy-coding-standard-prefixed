...
class Cache
{
    ...
    public static function load(\PHP_CodeSniffer\Ruleset $ruleset, \PHP_CodeSniffer\Config $config)
    {
        ...
        $filter = new \RecursiveCallbackFilterIterator($di, function ($file, $key, $iterator) {
            ...
            $filePath = \PHP_CodeSniffer\Util\Common::realpath($file->getPathname());
            if ($filePath === \false) {
                return \false;
            }
            if (\is_dir($filePath) === \true && ($filename === 'Standards' || $filename === 'Exceptions' || $filename === 'Reports' || $filename === 'Generators')) {
                return \false;
            }
            return \true;
        });
