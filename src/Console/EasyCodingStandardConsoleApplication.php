...
use _PhpScoper5e5ebcdbebdf\Composer\XdebugHandler\XdebugHandler;
...
final class EasyCodingStandardConsoleApplication extends \Symplify\SymplifyKernel\Console\AbstractSymplifyConsoleApplication
{
    ...
    public function doRun(\_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputInterface $input, \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        // @fixes https://github.com/rectorphp/rector/issues/2205
        $isXdebugAllowed = $input->hasParameterOption('--xdebug');
        if (!$isXdebugAllowed && !\defined('PHPUNIT_COMPOSER_INSTALL')) {
            $xdebugHandler = new \_PhpScoper5e5ebcdbebdf\Composer\XdebugHandler\XdebugHandler('ecs', '--ansi');
            $xdebugHandler->check();
            unset($xdebugHandler);
        }
    ...
    public function renderThrowable(\Throwable $throwable, \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
    ...
    protected function getDefaultInputDefinition() : \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputDefinition
    {
        $inputDefinition = parent::getDefaultInputDefinition();
        $this->addExtraOptions($inputDefinition);
        return $inputDefinition;
    }
    private function shouldPrintMetaInformation(\_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputInterface $input) : bool
    {
    ...
    private function addExtraOptions(\_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputDefinition $inputDefinition) : void
    {
        $inputDefinition->addOption(new \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputOption(\Symplify\EasyCodingStandard\ValueObject\Option::XDEBUG, null, \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Allow running xdebug'));
        $inputDefinition->addOption(new \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputOption(\Symplify\EasyCodingStandard\ValueObject\Option::DEBUG, null, \_PhpScoper5e5ebcdbebdf\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Run in debug mode (alias for "-vvv")'));
    }
}
