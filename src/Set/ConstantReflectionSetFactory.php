...
final class ConstantReflectionSetFactory
{
    ...
    public function createSetsFromClass(string $setClassName) : array
    {
        ...
        foreach ($constants as $name => $setPath) {
            ...
            $setName = \_PhpScoper4e2df00556a9\Nette\Utils\Strings::replace($setName, self::REMOVE_DASH_BEFORE_NUMBER_REGEX, '$1$2');
    ...
    private function constantToDashes(string $string) : string
    {
        ...
        return \_PhpScoper4e2df00556a9\Nette\Utils\Strings::replace($string, self::UNDERSCORE_REGEX, '-');
    }
}
