...
final class ConstantReflectionSetFactory
{
    ...
    public function createSetsFromClass(string $setClassName) : array
    {
        ...
        foreach ($constants as $name => $setPath) {
            ...
            $setName = \_PhpScoper8e2d8a2760d1\Nette\Utils\Strings::replace($setName, self::REMOVE_DASH_BEFORE_NUMBER_REGEX, '$1$2');
    ...
    private function constantToDashes(string $string) : string
    {
        ...
        return \_PhpScoper8e2d8a2760d1\Nette\Utils\Strings::replace($string, self::UNDERSCORE_REGEX, '-');
    }
}
